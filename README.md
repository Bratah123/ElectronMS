...
What this means is that ElectronMS is a more complete product than [**SoulGirlJP**'s repository](https://github.com/SoulGirlJP/AzureV316).  
Unlike the official open-source repository made available by **SoulGirlJP**, this is not an "abandoned" project, and will continue to receive support from its maintainers in the forseeable future.  
...
The v352.3 pre-alpha launch brought many new features to the table (courtesy of Brandon), including the use of Auth Hook, Discord RPC, and auto-registration upon login. However, the launch was also a mild disaster with numerous critical issues plaguing the gameplay. This included a faulty damage calculation system that prevented players from killing monsters (unless the player has SP put into a mastery skill, has a working weapon, **and** can one-shot the monster), poorly handled weapons/equipment, lack of methods to obtain equipment, lack of methods to obtain potions, as well as obvious signs of memory leaks. As a consequence of the latter issue, the game gradually slowed down and grinded to a halt less than 24 hours after its launch. This was circumvented (or in **SoulGirlJP**'s words "fixed") by **SoulGirlJP** by specifying to the process to use more memory.  

![SoulGirlJP increasing memory](https://media.discordapp.net/attachments/731528944402300977/890312904467947550/unknown.png)
