...
import static tools.BitTools.multiplyBytes;
...
public class MapleAESOFB {
    ...
    private static void Shuffle(byte inputValue, byte[] newIV) {
        byte elina = newIV[1];
        byte moritz = funnyBytes[(int) elina & 0xFF];
        moritz -= inputValue;
        newIV[0] += moritz;
        moritz = newIV[2];
        moritz ^= funnyBytes[(int) inputValue & 0xFF];
        ...
        moritz ^= newIV[2];
        newIV[2] = moritz;
        elina += (int) funnyBytes[(int) inputValue & 0xFF] & 0xFF;
        newIV[3] = elina;
