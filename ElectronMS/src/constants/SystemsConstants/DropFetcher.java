...
public class DropFetcher {
    ...
    private static void crawlPage(final String url) { // Recursive method bitches
        try {
            ...
            while (s.hasNext()) {
                temp_data.append(s.nextLine()).append("\n");
            }
            s.close();
            is.close();
            while (temp_data.toString().contains("class=\"mobImage\"")) {
                try {
                    String monster_section;
                    if (!temp_data.toString().contains("<div class=\"entityBox\">")) {
                        monster_section = getStringBetween(temp_data.toString(), "class=\"mobImage\"",
                    ...
                    temp_data = new StringBuilder(Objects.requireNonNull(trimUntil(temp_data.toString(), "<div class=\"entityBox\">")));
                    if (temp_data == null) {
                        break;
                    }
