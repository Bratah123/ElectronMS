...
import client.ItemInventory.Items.StructSetItem;
import client.ItemInventory.Items.StructSetItem.SetItem;
...
public class PlayerStats {
    ...
    private final transient Map<Integer, Integer> demonForce = new HashMap<>();
    ...
    public transient int percent_hp, percent_mp, percent_str, percent_dex, percent_int, percent_luk, percent_acc,
            percent_atk, percent_matk, speed, jump, ignoreTargetDEF, lvHp, lvMp, percent_min_crit;
    private int Morph = 0;
        ...
        if (chra != null) {
            if (hp <= 0) {
                if (chra.getSkillLevel(14110030) > 0 && !chra.skillisCooling(14110030)) {
                    this.hp = localmaxhp;
                    this.mp = localmaxmp;
                    chra.updateSingleStat(PlayerStatList.HP, this.hp);
                    chra.updateSingleStat(PlayerStatList.MP, this.mp);
                    ...
                    chra.cancelEffect(Objects.requireNonNull(SkillFactory.getSkill(12111023)).getEffect(chra.getSkillLevel(12111023)), false,
                            -1);
                    return true;
                    ...
                    int percentage = chra.getBuffedValue(BuffStats.StackBuff, 24111002);
                    ...
                    int percentage = chra.getBuffedValue(BuffStats.SoulStone);
                    this.hp = (int) (localmaxhp * (percentage / 100.0D));
                    this.mp = (int) (localmaxmp * (percentage / 100.0D));
                    chra.updateSingleStat(PlayerStatList.HP, this.hp);
                    chra.updateSingleStat(PlayerStatList.MP, this.mp);
                    ...
                    Objects.requireNonNull(SkillFactory.getSkill(1320019)).getEffect(chra.getSkillLevel(1320016)).applyTo(chra);
    ...
    private void handlePotential(StructPotentialItem pot, MapleCharacter player) {
        ...
        percent_min_crit += pot.incCriticaldamageMin;
        passive_sharpeye_rate += pot.incCriticaldamageMax;
        passive_sharpeye_min_percent += pot.incCriticaldamageMin;
        if (!pot.boss) {
        ...
        if (pot.skillID > 0 && player.getSkillLevel(pot.skillID) <= 0) {
            player.changeSkillLevel(SkillFactory.getSkill(getSkillByJob(pot.skillID, player.getJob())),
                    (byte) 1, (byte) 1);
        }
    }
        ...
        try {
            ...
            if (chra.getKeyValue("Skill_111_Used") == null) {
                if (chra.getSkillLevel(111) > 0) {
                    byte slot = (byte) Objects.requireNonNull(SkillFactory.getSkill(112)).getEffect(1).getX();
            ...
            for (IItem item : chra.getInventory(MapleInventoryType.EQUIPPED)) {
                ...
                if (setItemId != null && setItemId > 0) {
                    ...
                    if (setHandling.containsKey(setItemId)) {
                        value += setHandling.get(setItemId);
                    }
            ...
            for (String strid : ServerConstants.hp_skillid_real) {
                ...
                if (!GameConstants.isNotPassiveLoading(skillid)) {
                    if (chra.getSkillLevel(skillid) > 0 && Objects.requireNonNull(SkillFactory.getSkill(skillid)).getEffect(chra.getSkillLevel(skillid)).getLv2Mhp() > 0) {
                        lvHp += Objects.requireNonNull(SkillFactory.getSkill(skillid)).getEffect(chra.getSkillLevel(skillid)).getLv2Mhp() * chra.getLevel();
                    }
                    if (chra.getSkillLevel(skillid) > 0 && Objects.requireNonNull(SkillFactory.getSkill(skillid)).getEffect(chra.getSkillLevel(skillid)).getLv2Mmp() > 0) {
                        lvMp += (Objects.requireNonNull(SkillFactory.getSkill(skillid)).getEffect(chra.getSkillLevel(skillid)).getLv2Mmp()) * chra.getLevel();
                    }
                    if (chra.getSkillLevel(skillid) > 0 && Objects.requireNonNull(SkillFactory.getSkill(skillid)).getEffect(chra.getSkillLevel(skillid)).getMhpR() > 0) {
                        percent_hp += Objects.requireNonNull(SkillFactory.getSkill(skillid)).getEffect(chra.getSkillLevel(skillid)).getMhpR();
                    }
                    if (chra.getSkillLevel(skillid) > 0 && Objects.requireNonNull(SkillFactory.getSkill(skillid)).getEffect(chra.getSkillLevel(skillid)).getMmpR() > 0) {
                        percent_mp += Objects.requireNonNull(SkillFactory.getSkill(skillid)).getEffect(chra.getSkillLevel(skillid)).getMmpR();
                    }
                    if (chra.getSkillLevel(skillid) > 0 && Objects.requireNonNull(SkillFactory.getSkill(skillid)).getEffect(chra.getSkillLevel(skillid)).getMhpX() > 0) {
                        localmaxhp += Objects.requireNonNull(SkillFactory.getSkill(skillid)).getEffect(chra.getSkillLevel(skillid)).getMhpX();
                    }
                    if (chra.getSkillLevel(skillid) > 0 && Objects.requireNonNull(SkillFactory.getSkill(skillid)).getEffect(chra.getSkillLevel(skillid)).getMmpX() > 0) {
                        localmaxmp += Objects.requireNonNull(SkillFactory.getSkill(skillid)).getEffect(chra.getSkillLevel(skillid)).getMmpX();
                    }
                }
            }

            if (chra.getBuffedValue(BuffStats.CombatOrders) != null) {
                incAllskill += chra.getBuffedValue(BuffStats.CombatOrders);
            }
...
*/
    ...
    private void calcPassiveSharpEye(final MapleCharacter player) {
        if (GameConstants.isResistance(player.getJob())) {
            ISkill critSkill = SkillFactory.getSkill(30000022);
            int critlevel = player.getSkillLevel(critSkill);
            if (critlevel > 0) {
                assert critSkill != null;
        ...
        switch (player.getJob()) { // Apply passive Critical bonus
            ...
            case 422: {
                ...
                final ISkill bx = SkillFactory.getSkill(4110012);
            ...
            case 112: {
                SkillStatEffect eff;
                final ISkill bx = SkillFactory.getSkill(1110009);
                final int bof = player.getSkillLevel(bx);
                if (bof > 0) {
                    assert bx != null;
                    eff = bx.getEffect(bof);
                ...
                ISkill bx2 = SkillFactory.getSkill(1120012);
            ...
            case 322: { // Bowman
                ...
                SkillStatEffect eff;
                final ISkill bx2 = SkillFactory.getSkill(3120011);
                ...
                final ISkill bx3 = SkillFactory.getSkill(3220009);
            ...
            case 3111:
            case 3112: {
                final ISkill critSkill = SkillFactory.getSkill(31100006); // TODO LEGEND, not final
                final int critlevel = player.getSkillLevel(critSkill);
                if (critlevel > 0) {
                    assert critSkill != null;
            ...
            case 2311:
            case 2312: {
                final ISkill critSkill = SkillFactory.getSkill(23000003);
                final int critlevel = player.getSkillLevel(critSkill);
                if (critlevel > 0) {
                    assert critSkill != null;
                    this.passive_sharpeye_rate += (short) (critSkill.getEffect(critlevel).getCr());
            ...
            case 3211:
            case 3212: {
                final ISkill critSkill = SkillFactory.getSkill(32100006);
                final int critlevel = player.getSkillLevel(critSkill);
                if (critlevel > 0) {
                    assert critSkill != null;
                    this.passive_sharpeye_rate += (short) (critSkill.getEffect(critlevel).getCr());
            ...
            case 434: {
                final ISkill critSkill = SkillFactory.getSkill(4340010);
                final int critlevel = player.getSkillLevel(critSkill);
                if (critlevel > 0) {
                    assert critSkill != null;
                    this.passive_sharpeye_rate += (short) (critSkill.getEffect(critlevel).getProb());
            ...
            case 521:
            case 522: {
                final ISkill critSkill1 = SkillFactory.getSkill(5000007);
                final int critlevel1 = player.getSkillLevel(5000007);
                if (critlevel1 > 0) {
                    assert critSkill1 != null;
                ...
                final ISkill critSkill = SkillFactory.getSkill(5200007);
                final int critlevel = player.getSkillLevel(critSkill);
                if (critlevel > 0) {
                    assert critSkill != null;
                    this.passive_sharpeye_rate += (short) (critSkill.getEffect(critlevel).getCr());
            ...
            case 1211:
            case 1212: {
                final ISkill critSkill = SkillFactory.getSkill(12110000);
                final int critlevel = player.getSkillLevel(critSkill);
                if (critlevel > 0) {
                    assert critSkill != null;
                    this.passive_sharpeye_rate += (short) (critSkill.getEffect(critlevel).getCr());
            ...
            case 531:
            case 532: {
                final ISkill critSkill = SkillFactory.getSkill(5300004);
                final int critlevel = player.getSkillLevel(critSkill);
                if (critlevel > 0) {
                    assert critSkill != null;
                    this.passive_sharpeye_rate += (short) (critSkill.getEffect(critlevel).getCr());
            ...
            case 512: { // Buccaner, Viper
                final ISkill critSkill = SkillFactory.getSkill(5000007);
                ...
                final ISkill critSkill2 = SkillFactory.getSkill(5110011);
                final int critlevel2 = player.getSkillLevel(5110011);
                if (critlevel2 > 0) {
                    assert critSkill2 != null;
            ...
            case 1511:
            case 1512: {
                final ISkill critSkill2 = SkillFactory.getSkill(15110009);
            ...
            case 212: {
                SkillStatEffect eff;
                final ISkill id = SkillFactory.getSkill(2120010);
            ...
            case 222: {
                SkillStatEffect eff;
                final ISkill id2 = SkillFactory.getSkill(2220010);
            ...
            case 232: { // bishop
                SkillStatEffect eff;
                final ISkill id3 = SkillFactory.getSkill(2220010);
            ...
            case 2411:
            case 2412: { // ฦาลา
                final ISkill critSkill = SkillFactory.getSkill(24110007);
                final int critlevel = player.getSkillLevel(critSkill);
                if (critlevel > 0) {
                    assert critSkill != null;
                    this.passive_sharpeye_rate += (short) (critSkill.getEffect(critlevel).getCr());
        ...
        if (player.getBuffedValue(BuffStats.Dice) != null) {
            int rand1;
            int rand2;
            rand1 = player.getBuffedValue(BuffStats.Dice) & 0xF;
            rand2 = player.getBuffedValue(BuffStats.Dice) & 0xF0;
            if (rand1 == 4 && rand2 == 4) {
    ...
    public int getincRewardProp() {
        return Math.min(incRewardProp, 400);
    }
}
