...
public class MapleClient {
	...
	private final transient MapleCrypto send;
	private final transient MapleCrypto receive;
	...
	/* IoSession */
	public MapleClient(Channel session, MapleCrypto send, MapleCrypto receive) {
	...
	public Map<Integer, Integer> getCharacterCard() {
		Map<Integer, Integer> chrcard = new HashMap<>();
		Connection con = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		try {
	...
	public final boolean deleteCharacter(final int cid) {
		...
		try {
			...
			if (rs.getInt("guildid") > 0) { // is in a guild when deleted
				...
				CommunityHandler.getInstance().deleteGuildCharacter(mgc);
			}
	...
	public final void pongReceived(ReadingMaple slea) {
		...
		if (ServerConstants.AHT_VERSION >= 100) {
			...
			long dwChk1 = (slea.readInt() ^ dwSeed) & 0xFFFFFFFFL;
			long dwChk2 = (slea.readInt() ^ dwSeed2) & 0xFFFFFFFFL;
			long dwChk3 = (slea.readInt() ^ dwSeed3) & 0xFFFFFFFFL;
			...
			long dwCrcVal = (slea.readInt() ^ 0xF0C47ADA ^ dwSeed3) & 0xFFFFFFFFL;
			...
			long nErrorCode = (slea.readInt() ^ dwSeed2) & 0xFFFFFFFFL;
	...
	public final void sendPing() {
		...
		PingTimer.getInstance().schedule(() -> sendPing(), 60000); // note: idletime gets added to this too
	}
