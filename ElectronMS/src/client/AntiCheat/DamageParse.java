...
import handlers.Global.FinalAttackHandler;
...
//        }
        ...
        //}
        FinalAttackHandler.FinalAttack(attack, player);
        ...
        if (GameConstants.isArk(player.getJob())) {
            ...
            if (player.isActiveBuffedValue(155101008) && attack.skill != 155100009 && attack.allDamage.size() > 0 && player.isActiveBuffedValue(155101006)) {
                ...
                final List<MapleMapObject> objs = player.getMap().getMapObjectsInRange(player.getTruePosition(), 200000, Collections.singletonList(MapleMapObjectType.MONSTER));
        ...
        if (attack.skill == 3301008) {
            final List<MapleMapObject> objs = player.getMap().getMapObjectsInRange(player.getTruePosition(), 4000000, Collections.singletonList(MapleMapObjectType.MONSTER));
            final List<MapleMonster> monsters = new ArrayList<>();
            int i = 0;
            for (MapleMapObject o : objs) {
                monsters.add((MapleMonster) o);
        ...
        if (attack.skill == 3321005) {
            if (player.shape == 1 && Randomizer.isSuccess(60)) {
                final List<MapleMapObject> objs = player.getMap().getMapObjectsInRange(player.getTruePosition(), 400000, Collections.singletonList(MapleMapObjectType.MONSTER));
                final List<MapleMonster> monsters = new ArrayList<>();
                int i = 0;
                for (MapleMapObject o : objs) {
                    monsters.add((MapleMonster) o);
        ...
        for (final AttackPair mob : attack.allDamage) {
            ...
            /*  Add damage  */
            if (player.getKeyValue("AddDamageAccess") != null) {
        ...
        for (final AttackPair oned : attack.allDamage) {
            ...
            if (monster != null) {
             ...
             if (player.getKeyValue("AddDamageAccess") != null) {
                 if (curTime > 500 + player.getLastCombo()) {
                     if (player.getAddDamage() > 0) {
                         long dam = player.getAddDamage() * (player.getDamageHit() + player.getDamageHit2()) / 100;
                ...
                if (totDamageToOneMonster > 0) {
                            ...
                            if (modeChange) {
                                ...
                                player.getClient().getSession()
                                        .writeAndFlush(UIPacket.showWZEffect(
                                                ...
                                                + "/" + (player.quivercount[player.quivermode - 1]),
                                                1));
                                player.getMap().broadcastMessage(player,
                    ...
                    if (player.getBuffedValue(BuffStats.NightWalkerBat, 14001027) != null) {
                        ...
                        final List<MapleMapObject> mobs_ = player.getMap().getMapObjectsInRange(player.getPosition(), 800 * 800, Collections.singletonList(MapleMapObjectType.MONSTER));
                        final List<Integer> mobList = new LinkedList<>();
                        int i = 0;
                    ...
                    if (player.getSkillLevel(4120011) > 0) {
                        monster.applyStatus(player, new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.POISON, Randomizer.rand(100, 500)), SkillFactory.getSkill(4120011), player.getSkillLevel(4120011), null, false), (long) 10 * 1000);
                    }

                    if (player.getSkillLevel(4220011) > 0) {
                        monster.applyStatus(player, new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.POISON, Randomizer.rand(100, 500)), SkillFactory.getSkill(4220011), player.getSkillLevel(4220011), null, false), (long) 10 * 1000);
                    }
                    if (player.isActiveBuffedValue(12101024)) {
                        monster.applyStatus(player, new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.POISON, Randomizer.rand(100, 500)), SkillFactory.getSkill(12101024), player.getSkillLevel(12101024), null, false), (long) 10 * 1000);
                        if (player.getSkillLevel(400021042) > 0 && !player.skillisCooling(400021042) && attack.skill != 400021045) {
                    ...
                    if (player.isActiveBuffedValue(4121054)) {
                        monster.applyStatus(player, new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.POISON, Randomizer.rand(100, 500)), SkillFactory.getSkill(4121054), player.getSkillLevel(4121054), null, false), (long) 10 * 1000);
                    }
        ...
        if (player.getSkillLevel(1200014) > 0) {
            ...
            if (player.GetCount() < 5) {
                for (int value : skillid) {
                    if (attack.skill == value) {
                        if (player.GetSkillid() != value) {
        ...
        if (effect != null) {
            if (attack.skill != 0 && !effect.isMist()) {
                if (attack.skill != 400051013 && attack.skill != 400051006 && attack.skill != 400021041 && attack.skill != 400021049
        ...
        if (player.isActiveBuffedValue(65121011) && attack.skill != 65111007 && attack.targets > 0) {
            ...
            objs = player.getMap().getMapObjectsInRange(player.getTruePosition(), 100000, Collections.singletonList(MapleMapObjectType.MONSTER));
            if (!objs.isEmpty()) {
    ...
    public static void applyAttackMagic(AttackInfo attack, ISkill theSkill, MapleCharacter player,
        ...
        long totDamage = 0, totDamageToOneMonster;
        int last = 0;
        int lastE = 0;
        Point lastP = null;
        MapleMonsterStats monsterstats;
...
//                        }
                ...
                if (totDamageToOneMonster > 0) {
                    ...
                    if (attack.skill == 142001000 || attack.skill == 142100000 || attack.skill == 142110000) {
                        monster.applyStatus(player, new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.POISON, Randomizer.rand(100, 500)), SkillFactory.getSkill(attack.skill), player.getSkillLevel(attack.skill), null, false), (long) 10 * 1000);
