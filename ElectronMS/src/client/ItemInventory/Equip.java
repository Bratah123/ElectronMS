...
public class Equip extends Item implements IEquip, Serializable {
    ...
    public void renewPotential() {
        ...
        if (getState() == 17 && Randomizer.nextInt(100) <= epic) {
            ...
            setState((byte) 3);
            return;
        } else if (getState() == 19 && Randomizer.nextInt(100) <= legendary) {
            setState((byte) 4);
            return;
        }
    ...
    public String getFireStatToString() {
        ...
        for (int value : fireStat) {
            t.append(t.toString().equals("") ? "" : ",").append(value);
        }
        return t.toString();
    }
...
//		반지, 방패, 어깨장식, 훈장, 심장, 보조무기, 엠블렘, 뱃지, 안드로이드, 펫장비, 심볼은 추가옵션이 붙지 않음
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            int randomValue; // Usually 3 ~ 7-> 1 ~ 5 commonly called
