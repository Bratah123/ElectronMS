...
import client.Skills.VMatrix.VCoreInfo;
...
import client.ItemInventory.Items.StructRewardItem;
...
public class InventoryHandler {
    ...
    private static List<IItem> sortItems(final List<IItem> passedMap) {
        ...
        final List<IItem> sortedList = new LinkedList<>();
    ...
    public static void UseItem(ReadingMaple rh, MapleClient c, MapleCharacter chr) {
        ...
        if (chr.getBuffedSkillEffect(BuffStats.ExpBuffRate) != null && Objects.requireNonNull(ItemInformation.getInstance().getItemEffect(toUse.getItemId())).getStatups() != null && ItemInformation.getInstance().getItemEffect(toUse.getItemId()).getStatups().contains(BuffStats.ExpBuffRate)) {
            chr.cancelEffectFromBuffStat(BuffStats.ExpBuffRate);
        }
        if (!FieldLimitType.PotionUse.check(chr.getMap().getFieldLimit())) {
            if (Objects.requireNonNull(ItemInformation.getInstance().getItemEffect(toUse.getItemId())).applyTo(chr)) {
    ...
    public static Equip Reincarnation(Equip eq) {
        return eq;
    }
    ...
    public static void UseReturnScroll(ReadingMaple rh, MapleClient c, MapleCharacter chr) {
        ...
        if (!FieldLimitType.PotionUse.check(chr.getMap().getFieldLimit())) {
            if (Objects.requireNonNull(ItemInformation.getInstance().getItemEffect(toUse.getItemId())).applyReturnScroll(chr)) {
                InventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, slot, (short) 1, false);
            } else {
                c.getSession().writeAndFlush(MainPacketCreator.resetActions(c.getPlayer()));
            }
    ...
    public static boolean UseUpgradeScroll(byte slot, byte dst, MapleClient c, MapleCharacter chr) {
        ...
        if (!GameConstants.isSpecialScroll(scroll.getItemId()) && !GameConstants.isCleanSlate(scroll.getItemId())
            ...
            for (int value : itemId) {
                if (toScroll.getItemId() == value) {
        ...
        if (zeroScroll != null) {
            ...
            zeroEquip.setDurability(zeroEquip.getDurability() + srcEquip.getDurability() - check.getDurability());
            ...
            zeroEquip.setItemEXP(zeroEquip.getItemEXP() + srcEquip.getItemEXP() - check.getItemEXP());
            ...
            if (Math.floor(CurrentLoopedSkillId / 100000) == chr.getJob() / 10) {
                ...
                if (chr.getOriginSkillLevel(CurrSkillData) >= ReqSkillLevel && chr.getMasterLevel(CurrSkillData) < MasterLevel) {
                    ...
                    if (Randomizer.nextInt(99) <= SuccessRate && SuccessRate != 0) {
                        successs = true;
                        chr.changeSkillLevel(CurrSkillData, chr.getOriginSkillLevel(CurrSkillData), (byte) MasterLevel);
                    } else {
                        successs = false;
                    }
                    InventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, slot, (short) 1, false);
        ...
        if (toUse != null && toUse.getQuantity() >= 1 && toUse.getItemId() == itemId) {
            switch (toUse.getItemId()) {
                ...
                //Damage skin up to version 316
                {
                    ...
                    queststatus.setCustomData(skinString);
                    ...
                    c.getPlayer().setKeyValue("SaveDamageSkin",
                            dKey == null ? String.valueOf(itemid) : (dKey + "," + itemid));
    ...
    public static void UseSummonBag(ReadingMaple rh, MapleClient c, MapleCharacter chr) {
        ...
        if (toUse != null && toUse.getQuantity() >= 1 && toUse.getItemId() == itemId) {
            ...
            if (c.getPlayer().isGM() || !FieldLimitType.SummoningBag.check(chr.getMap().getFieldLimit())) {
                ...
                MapleMonster ht;
                int type = 0;
                for (Pair<Integer, Integer> integerIntegerPair : toSpawn) {
                    if (Randomizer.nextInt(99) <= integerIntegerPair.getRight()) {
                        ht = MapleLifeProvider.getMonster(integerIntegerPair.getLeft());
    ...
    public static void UseGoldenHammer(ReadingMaple rh, MapleClient c) { // Golden Hammer Method
        ...
        success = false;
        c.getSession().writeAndFlush(CashPacket.GoldenHammer(true, false));
        ...
        switch (itemId) { // Golden Hammer
         case 2470000:
            ...
            InventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, slot, (short) 1, true);
            c.getSession().writeAndFlush(MainPacketCreator.resetActions(c.getPlayer()));
            break;
    ...
    public static void useItem(ReadingMaple rh, MapleClient c) {
        ...
        if (itemid == 2711003) {// Miracle Cube
            IItem item;
    ...
    public static void UseMannequin(final ReadingMaple rh, final MapleClient c) {
        ...
        switch (value) {
            ...
            case 1: // Swapping
                ...
                c.getPlayer().setHair(c.getPlayer().getSecondHair());
                statups.add(new Pair<>(PlayerStatList.HAIR, (long) c.getPlayer().getHair()));
    ...
    public static void UseCashItem(ReadingMaple rh, MapleClient c) {
        ...
        switch (itemId) {
            ...
            case 5050000: { // AP Reset
                List<Pair<PlayerStatList, Long>> statupdate = new ArrayList<>(2);
                int apto = rh.readInt();
                ...
                if (used) {
                    switch (apto) { // AP to
                        case 64: { // str
                            ...
                            statupdate.add(new Pair<>(PlayerStatList.STR, (long) toSet));
                            break;
                        }
                        case 128: { // dex
                            int toSet = playerst.getDex() + 1;
                            playerst.setDex(toSet);
                            statupdate.add(new Pair<>(PlayerStatList.DEX, (long) toSet));
                            break;
                        }
                        case 256: { // int
                            int toSet = playerst.getInt() + 1;
                            playerst.setInt(toSet);
                            statupdate.add(new Pair<>(PlayerStatList.INT, (long) toSet));
                            break;
                        }
                        case 512: { // luk
                            int toSet = playerst.getLuk() + 1;
                            playerst.setLuk(toSet);
                            statupdate.add(new Pair<>(PlayerStatList.LUK, (long) toSet));
                            break;
                        }
                    }
            ...
            case 5050004: { // SP Reset (4th job)
                ...
                assert skillSPTo != null;
                assert skillSPFrom != null;
            ...
            case 5051001: {
                ...
                List<Pair<ISkill, Byte>> toRemove = new ArrayList<>();
                for (Entry<ISkill, SkillEntry> e : skills.entrySet()) {
                    if (!GameConstants.isBeginnerJob(e.getKey().getId() / 10000) && (e.getKey().getId() / 10000 <= 5112)) {
                        ...
                        toRemove.add(new Pair<>(e.getKey(), e.getValue().masterlevel));
                    }
                }
            ...
            case 5050100: {
                ...
                stats.add(new Pair<>(PlayerStatList.STR, (long) 4));
                stats.add(new Pair<>(PlayerStatList.DEX, (long) 4));
                stats.add(new Pair<>(PlayerStatList.INT, (long) 4));
                stats.add(new Pair<>(PlayerStatList.LUK, (long) 4));
                ...
                for (MaplePet pet : c.getPlayer().getPets()) {
                    if (!pet.canConsume(itemId)) {
                        ...
                        if (pet.getCloseness() < 30000) {
                            pet.setCloseness(Math.min(pet.getCloseness() + 100, 30000));
                ...
                if (itemId / 10000 == 512) {
                    ...
                    if (buff != 0) {
                        for (MapleCharacter mChar : c.getPlayer().getMap().getCharacters()) {
                            Objects.requireNonNull(ii.getItemEffect(buff)).applyTo(mChar);
                        }
                    }
                    used = true;
        ...
        if ((itemId == 5060002 || itemId == 5060022)) { // Premium Incubator, Faerie Water Bottle
            ...
            rh.readInt();
            IItem item2 = c.getPlayer().getInventory(Objects.requireNonNull(MapleInventoryType.getByType(inventory2))).getItem(slot2);
            if (item2 != null) {
                try {
    ...
    public static void Pickup_Player(ReadingMaple rh, MapleClient c, MapleCharacter chr) {
        ...
        if (mapitem.getMeso() > 0) {
            if (chr.getParty() != null && mapitem.getOwner() != chr.getId()) {
                ...
                for (final MapleCharacter m : toGive) {
                    ...
                    if (m.getSkillLevel(70000050) > 0) {
                        int rate = Objects.requireNonNull(SkillFactory.getSkill(70000050)).getEffect(m.getSkillLevel(70000050)).getSkillStats()
                                .getStats("mesoR");
                ...
                if (chr.getSkillLevel(70000050) > 0) {
                    int rate = Objects.requireNonNull(SkillFactory.getSkill(70000050)).getEffect(chr.getSkillLevel(70000050)).getSkillStats()
                            .getStats("mesoR");
                    mesoSize += (int) (mesoSize * (rate / 100.0D));
                }
            ...
            if (InventoryManipulator.checkSpace(c, mapitem.getItemId(), mapitem.getItem().getQuantity(),
                ...
                if (InventoryManipulator.addFromDrop(c, mapitem.getItem(), true, false, 0, false) != -1) {
                    removeItem(chr, mapitem, ob);
        ...
        try {
            ...
            if ((toUse.getItemId() >= 2048305 && toUse.getItemId() <= 2048310)) {
                ...
                if (item != null) {
                    ...
                    if (succes) {
                        Equip eq = (Equip) item;
                        int option, option2, option3_sbal, level = 2;
            ...
            if (mapitem.getMeso() > 0) {
                if (chr.getParty() != null && mapitem.getOwner() != chr.getId()) {
                    ...
                    for (final MapleCharacter m : toGive) {
                        ...
                        if (m.getSkillLevel(70000050) > 0) {
                            int rate = Objects.requireNonNull(SkillFactory.getSkill(70000050)).getEffect(m.getSkillLevel(70000050)).getSkillStats().getStats("mesoR");
                            mesoSize += (int) (mesoSize * (rate / 100.0D));
                        }
                        m.gainMeso(mesoSize, true);
                    }
                    int mesoSize = mapitem.getMeso() - splitMeso;
                    if (chr.getSkillLevel(70000050) > 0) {
                        int rate = Objects.requireNonNull(SkillFactory.getSkill(70000050)).getEffect(chr.getSkillLevel(70000050)).getSkillStats().getStats("mesoR");
                        mesoSize += (int) (mesoSize * (rate / 100.0D));
                    }
                    chr.gainMeso(mesoSize, true);
                } else {
    ...
    public static void removeItem_Pet(final MapleCharacter chr, final MapleWorldMapItem mapitem, int pet) {
            ...
            if (consumeval > 0) {
                if (c.getPlayer().getMapId() == 109090300) {
                    for (MapleCharacter chr : c.getPlayer().getMap().getCharacters()) {
                        if (chr != null && ((id == 2022163 && c.getPlayer().isCatched == chr.isCatched)
                            ...
                            if (id == 2022163) {
                                Objects.requireNonNull(ii.getItemEffect(id)).applyTo(chr);
                            } else if (id == 2022166) {
                ...
                if (consumeval == 2) {
                    if (c.getPlayer().getParty() != null) {
                        ...
                        Objects.requireNonNull(ii.getItemEffect(id)).applyTo(c.getPlayer());
                    }
                } else {
                    Objects.requireNonNull(ii.getItemEffect(id)).applyTo(c.getPlayer());
                }
    ...
    public static void headTitle(ReadingMaple rh, MapleClient ha) {
        int itemid = rh.readInt();
    ...
    public static void UseSoulEnchanter(ReadingMaple rh, MapleClient c, MapleCharacter chr) {
        rh.skip(4);
    ...
    public static void UseSoulScroll(ReadingMaple rh, MapleClient c, MapleCharacter chr) {
