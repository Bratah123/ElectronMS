...
public class ItemInformation {
    ...
    protected final Map<Integer, SkillStatEffect> itemEffects = new HashMap<>();
    ...
    protected final Map<Integer, Map<Integer, StructEquipLevel>> equipLevelCache = new HashMap<>();
    protected final Map<Integer, Integer> equipLevelMaxLevel = new HashMap<>();
    ...
    protected final Map<Integer, Byte> runOnPickupCache = new HashMap<>();
    protected final Map<Integer, Boolean> dropRestrictionCache = new HashMap<>();
    protected final Map<Integer, Boolean> expirationOnLogOutCache = new HashMap<>();
    protected final Map<Integer, Boolean> pickupRestrictionCache = new HashMap<>();
    protected final Map<Integer, Integer> stateChangeCache = new HashMap<>(40);
    ...
    public final Map<Integer, List<StructPotentialItem>> potentialCache = new HashMap<>();
    ...
    protected Map<Integer, Integer> androidCache = new HashMap<>();
    protected Map<Integer, Integer> bagTypeCache = new HashMap<>();
    protected List<Integer> nonLevelEquip = new LinkedList<>();
    protected List<Integer> incSkill = null;
    protected Map<Integer, BasicAndroid> androidBasicCache = new HashMap<>();
    protected Map<Integer, String> scriptedItemScriptCache = new HashMap<>();
    ...
    protected Map<Integer, Integer> mipCache = new HashMap<>();
...
* Bottom 54: Gloves 55: Shoes
    ...
    public void cachePotentialOption() {
        ...
        for (MapleData data : potsData) {
            ...
            int level = 0;
            Map<Integer, List<Integer>> option = new HashMap<>();
            ...
            potentialOpCache.add(new Pair<>(potentialID, type));
            option.put(type, id);
        }
    }
    ...
    public void cachePotentialItems() {
        ...
        for (MapleData data : potsData) {
            ...
            for (MapleData level : data.getChildByPath("level")) {
                ...
                switch (item.potentialID) {// item.skillID = (short) (item.potentialID - 23001);
                    default:
                        item.skillID = 0;
                        break;
                }
    ...
    public final Integer getSetItemID(final int itemId) {
        if (getEquipStats(itemId) == null || !Objects.requireNonNull(getEquipStats(itemId)).containsKey("setItemID")) {
            return 0;
        }
        return Objects.requireNonNull(getEquipStats(itemId)).get("setItemID");
    }
    ...
    public final int getReqLevel(final int itemId) {
        try {
             ...
             return Objects.requireNonNull(getEquipStats(itemId)).get("reqLevel");
    ...
    public final List<Integer> getScrollReqs(final int itemId) {
        ...
        final MapleData data = Objects.requireNonNull(getItemData(itemId)).getChildByPath("req");
        ...
        if (equip.getType() == 1) {
            ...
            switch (scrollId.getItemId()) {
                ...
                default: {
                    if (GameConstants.isChaosScroll(scrollId.getItemId())) {
                    ...
                    } else if (scrollId.getItemId() == 2049371 || scrollId.getItemId() == 2049370) {
                        ...
                        if (nEquip.getFire() == -1) {
                            if (Randomizer.isSuccess(getSuccess(scrollId.getItemId(), chr, nEquip))) {
                                ...
                                nEquip = randomizeStatsFire(nEquip, true);
                                if (nZero != null) {
                                    nEquip = randomizeStatsFire(nEquip, true);
                                }
                            }
                        } else if (Randomizer.isSuccess(getSuccess(scrollId.getItemId(), chr, nEquip))) {
                            nEquip = randomizeStatsFire(nEquip, false);
                        }
                        break;
                    } else {
                        if (!Randomizer.isSuccess(getSuccess(scrollId.getItemId(), chr, nEquip), chr)) {
    ...
    public final SkillStatEffect getItemEffect(final int itemId) {
        SkillStatEffect ret = itemEffects.get(itemId);
        if (ret == null) {
            final MapleData item = getItemData(itemId);
            if (item == null) {
                return null;
    ...
    public final List<Pair<Integer, Integer>> getSummonMobs(final int itemId) {
        ...
        final MapleData data = Objects.requireNonNull(getItemData(itemId)).getChildByPath("mob");
        if (data == null) {
            return null;
        }
    ...
    public final Pair<Integer, List<StructRewardItem>> getRewardItem(final int itemid) {
        ...
        // 100
        ArrayList<StructRewardItem> all = new ArrayList<>();
    ...
    public final List<Integer> petsCanConsume(final int itemId) {
        ...
        int count = 0;
        while (true) {
            curPetId = MapleDataTool.getInt("spec/" + count, data, 0);
            if (curPetId == 0) {
                break;
            }
            ret.add(curPetId);
    ...
    public final boolean isCash(final int itemid) {
        ...
        return GameConstants.getInventoryType(itemid) == MapleInventoryType.CASH
                || Objects.requireNonNull(getEquipStats(itemid)).get("cash") > 0;
    }
    ...
    public Equip randomizeStatsFire(final Equip equip, final boolean isFirst) { //Flame of Eternal Reincarnation
        ...
        if (!GameConstants.isWeapon(equip.getItemId())) {
            if (Randomizer.rand(1, 2) == 1) {
                equip.setAllStatP(def.getAllStatP());
            } else {
    ...
    public void addCrystalEffect(Equip equip, int crystalId) {
        switch (crystalId) {
            case 4250400:
            case 4250401:
            case 4250402:
    ...
    public int getETCMonsLvl(int itemid) {
        if (getItemData(itemid) != null) {
            final MapleData lvl = Objects.requireNonNull(getItemData(itemid)).getChildByPath("info/lv");
            if (lvl == null) {
                return -1;
            }
            ...
            if (potype == 0 || potype == 10 || potype == 11 || potype == 20 || potype == 40 || potype == 54) { 
                ...
                if (isEditionalWeaponOption(potential)) {
                    ...
                    if (isEditionalGloveOption(potential)) {
                        ...
                        return !isEditionalArmorOption(potential);
                    }
                }
            } else {
                return true;
