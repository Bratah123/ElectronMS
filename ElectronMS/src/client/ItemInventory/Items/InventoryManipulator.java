...
import client.ItemInventory.Items.CashItem.CashItemInfo;
...
public class InventoryManipulator {
    ...
    public static boolean addbyItem(final MapleClient c, IItem item) {
        ...
        if (GameConstants.환생의불꽃아이템(item.getItemId())) {
            if (item.getOwner().equals("") || item.getOwner() == null) {
                item = InventoryHandler.Reincarnation((Equip) item);
            }
        }
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            if (quantity == 1) {
                ...
                if (GameConstants.환생의불꽃아이템(nEquip.getItemId())) {
                    if (nEquip.getOwner().equals("") || nEquip.getOwner() == null) {
                        nEquip = InventoryHandler.Reincarnation((Equip) nEquip);
                    }
                }
    ...
    public static short addFromDrop(final MapleClient c, IItem item, final boolean show, final boolean enhance) {
        return addFromDrop(c, item, show, false, 0);
    }

    public static short addFromDrop(final MapleClient c, IItem item, final boolean show, final boolean enhance,
            long period) {
        return addFromDrop(c, item, show, false, 0, false);
    }
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            if (!GameConstants.isThrowingStar(item.getItemId()) && !GameConstants.isBullet(item.getItemId())) {
                        ...
                        if (i.hasNext()) {
                            ...
                            if (GameConstants.환생의불꽃아이템(eItem.getItemId())) {
                                if (eItem.getOwner() == "" || eItem.getOwner() == null) {
                                    eItem = InventoryHandler.Reincarnation((Equip) eItem);
                                }
                            }
                            ...
                            if (oldQ < slotMax && item.getOwner().equals(eItem.getOwner())
                                ...
                                if (!c.isCS && !c.isAuction) {
                                    c.getSession().writeAndFlush(
                                            MainPacketCreator.updateInventorySlot(type, eItem, !petloot));
                                }
                                if (eItem.isCash()) {
                ...
                while (quantity > 0) {
                    ...
                    if (!c.isCS && !c.isAuction) {
                        c.getSession()
                                .writeAndFlush(MainPacketCreator.addInventorySlot(type, nItem, !petloot));
                    }
                }
            } else {
            ...
            if (quantity == 1) {
                ...
                if (!c.isCS && !c.isAuction) {
                    c.getSession()
                            .writeAndFlush(MainPacketCreator.addInventorySlot(type, item, !petloot));
                }
            } else {
    ...
    public static void removeFromSlot(final MapleClient c, final MapleInventoryType type, final short slot,
        ...
        if (item.getQuantity() == 0 && !allowZero) {
            ...
            c.getSession().writeAndFlush(MainPacketCreator.updateInventorySlot(type, item, fromDrop));
        }
    }
            ...
            if ((olddstQ + oldsrcQ) > slotMax) {
                ...
                c.getSession()
                        .writeAndFlush(MainPacketCreator.moveAndMergeInventoryItem(type, src, dst,
                                c.getPlayer().getInventory(type).getItem(dst).getQuantity(), bag, switchSrcDst,
                                bothBag));
            }
        } else {
    ...
    public static void equip(final MapleClient c, final short src, short dst) {
        ...
        if (GameConstants.isEvanDragonItem(source.getItemId()) && (chr.getJob() < 2200 || chr.getJob() > 2218)) {
            ...
            assert stats != null;
            if ((dst <= -1200 || (dst >= -999 && dst < -99)) && !stats.containsKey("cash")) {
                c.getSession().writeAndFlush(MainPacketCreator.resetActions(c.getPlayer()));
                return;
        ...
        if (stats.get("charmEXP") > 0 || stats.get("willEXP") > 0 || stats.get("charismaEXP") > 0
                || ItemInformation.getInstance().getCashCharmExp(source) > 0) {
            short flag = source.getFlag();
            if (!ItemFlag.CHARM_EQUIPPED.check(flag)) {
                ...
                if (ItemInformation.getInstance().getCashCharmExp(source) > 0) {
                    c.getPlayer().addCharm(ItemInformation.getInstance().getCashCharmExp(source));
                }
                c.getSession().writeAndFlush(MainPacketCreator.addInventorySlot(MapleInventoryType.EQUIP, source));
            }
        }
    ...
    public static IItem BuyMonsterLifeWeapon(int boxid) {
        return InventoryHandler.Reincarnation((Equip) ItemInformation.getInstance().getEquipById(boxid));
    }
}
