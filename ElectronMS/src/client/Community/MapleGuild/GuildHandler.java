...
public class GuildHandler {
    ...
    public static void Sendrequest(ReadingMaple rh, MapleClient c) {
        ...
        final MapleGuild guild = CommunityHandler.getInstance().getGuild(guildId);
        if (guild == null) {
    ...
    public static void GuildOpertion(ReadingMaple rh, MapleClient c) {
        ...
        switch (action) {
            ...
            case 0x25:
                ...
                if (SkilliD == 91001019) {
                    String sb = "< " + c.getPlayer().getGuild().getName() + " > " + c.getPlayer().getName() +
                            " : " +
                            rh.readMapleAsciiString();
                    c.getPlayer().getMap().broadcastMessage(MainPacketCreator.serverNotice(3, sb));
            ...
            case 0x2D: {
                ...
                if (textSearch) {
                    ...
                    int minGuildLevel = rh.readByte() & 0xFF;
                    int maxGuildLevel = rh.readByte() & 0xFF;
                    int minMemberSize = rh.readByte() & 0xFF;
                    int maxMemberSize = rh.readByte() & 0xFF;
                    int minAvgLevel = rh.readByte() & 0xFF;
                    int maxAvgLevel = rh.readByte() & 0xFF;
    ...
    public static void joinRequest(ReadingMaple rh, MapleCharacter chr) {
        ...
        if (removeTime != null && !removeTime.isEmpty()) {
            if (Long.parseLong(removeTime) + 60 * 1000 > System.currentTimeMillis()) {
    ...
    public static void removeJoinRequest(ReadingMaple rh, MapleCharacter chr) {
        ...
        assert guildid != null;
        final MapleGuild guild = CommunityHandler.getInstance().getGuild(Integer.parseInt(guildid));
        if (guild == null) {
