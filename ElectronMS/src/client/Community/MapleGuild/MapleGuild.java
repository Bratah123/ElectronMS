...
public class MapleGuild {
    ...
    private final List<Integer> notifications = new ArrayList<>();
    private boolean bDirty = true;
    private int allianceid, invitedid;
    private MapleAlliance ally;
    private Lock lock = new ReentrantLock();
    private byte level = 1;
    private final Map<Integer, GuildSkills> guildSkills = new HashMap<>();
    ...
    public boolean activateSkill(int skill, String name) {
        ...
        final SkillStatEffect skillid = Objects.requireNonNull(SkillFactory.getSkill(skill)).getEffect(ourSkill.level);
    ...
    public void insertJoinRequester(MapleCharacter p) {
        ...
        p.updateInfoQuest(26015, "guild=" + id + ";" + "name=" + getName() + ";");
    ...
    public final void leaveGuild(final MapleGuildCharacter mgc) {
        ...
        lock.lock();
        try {
            members.removeIf(omgc -> omgc.getId() == mgc.getId());
            bDirty = true;
        } finally {
            lock.unlock();
        }
    ...
    public final void changeARank(final boolean leader) {
        ...
        for (final MapleGuildCharacter mgc : members) {
            ...
            if (mgc.isOnline()) {
                ...
                setOfflineGuildStatus(this.id, (byte) mgc.getGuildRank(), 3, newRank, mgc.getId());
            }
    ...
    public final void changeGuildLeader(final int cid) {
        if (changeRank(cid, 1) && changeRank(leader, 2)) {
            ...
            if (allianceid > 0) {
                int aRank = Objects.requireNonNull(getMGC(leader)).getAllianceRank();
                if (aRank == 1) {
    ...
    public final void addMemberData(final WritingPacket packet) {
        ...
        rs.sort((r1, r2) -> r2.level - r1.level);
    ...
    private final ConcurrentHashMap<Integer, JoinRequester> requesters = new ConcurrentHashMap<>();
