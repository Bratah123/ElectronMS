...
public class PartyHandler {
    ...
    public static void DenyPartyRequest(final ReadingMaple rh, final MapleClient c) {
        ...
        if (c.getPlayer().getParty() == null) {
            MapleParty party = CommunityHandler.getInstance().getParty(partyid);
            if (party != null) {
    ...
    public static void PartyOperatopn(final ReadingMaple rh, final MapleClient c) {
                    ...
                    if (partyplayer.equals(party.getLeader())) {
                        if (party.getExpedition() == null) { // disband
                            ...
                            if (party.getExpedition().getLeader() == partyplayer.getId()) {
                                CommunityHandler.getInstance().disbandExpedition(party.getExpedition(), partyplayer);
                            } else {
                                CommunityHandler.getInstance().leftExpedition(party.getExpedition(), party, partyplayer);
        ...
        switch (action) {
                ...
                if (subAction == 0x07) { // Per invitation
                    ...
                    if (hp == null) {
                        ...
                        if (exp.getAllMemberSize() < exp.getType().maxplayer) {
                            CommunityHandler.getInstance().joinExpedition(exp, c.getPlayer());
                        } else {
                            c.getPlayer().message(1, "The expedition is already full.");
                        }
                    }
            ...
            case 82: { // Expedition Expedition
                ...
                if (exp.getLeader() == c.getPlayer().getId()) {
                    ...
                    CommunityHandler.getInstance().expelExpedition(exp, hpcParty, found);
                    if (c.getPlayer().getEventInstance() != null) {
                        c.getPlayer().getEventInstance().disbandParty();
                    }
                }
            ...
            case 83: {// Expedition
                ...
                if (exp.getLeader() == c.getPlayer().getId()) {
                    ...
                    CommunityHandler.getInstance().changeLeaderExpedition(exp, hpcParty, found);
                }
                break;
            }
            case 84: {// Expedition Party Leader Giving
                ...
                if (exp.getLeader() == c.getPlayer().getId()) {
                    ...
                    CommunityHandler.getInstance().changeLeaderPartyInExpedition(exp, hpcParty, found);
                }
                break;
            }
            case 85: {// Expedition Party Moves
                ...
                if (exp.getLeader() == c.getPlayer().getId()) {
                    ...
                    CommunityHandler.getInstance().movePlayerExpedition(exp, targetchr.getParty(), index, targetchr);
                }
                break;
            }
        }
