...
import client.Stats.Buffs.MapleBuffStorage;
...
public class CommunityHandler {
    ...
    private final Map<Integer, MapleExpedition> expeditions = new HashMap<>();
    private final AtomicInteger runningExpeditionId = new AtomicInteger();
    private final Map<Integer, MapleMultiChat> messengers = new HashMap<>();
    ...
    private static CommunityHandler instance = new CommunityHandler();
    ...
    public void setGuildMemberOnline(final MapleGuildCharacter mgc, final boolean bOnline, final int channel) {
        Objects.requireNonNull(getGuild(mgc.getGuildId())).setOnline(mgc.getId(), bOnline, channel);
    }
    ...
    public void deleteGuildCharacter(MapleGuildCharacter mgc) {
        CommunityHandler.getInstance().setGuildMemberOnline(mgc, false, -1);
        if (mgc.getGuildRank() > 1) { // not leader
            CommunityHandler.getInstance().leaveGuild(mgc);
        } else {
            CommunityHandler.getInstance().disbandGuild(mgc.getGuildId());
        }
    }
        ...
        if (alliance != null) {
            for (int i = 0; i < alliance.getNoGuilds(); i++) {
                ...
                if (gid > 0 && gid != exceptionId) {
                    Objects.requireNonNull(getGuild(gid)).broadcast(packet);
                }
            }
        }
    }
    ...
    public boolean createAlliance(final MapleClient client, final String alliancename, final int cid, final int cid2, final int gid, final int gid2) {
        final int allianceid = Objects.requireNonNull(getGuild(gid)).createAlliance(client, alliancename, gid2);
        if (allianceid <= 0) {
            return false;
        }
    ...
    public List<MapleGuild> getGuildForSearch(int worldid, byte mode, String text, boolean likeSearch) {
        ...
        if (mode == 1 || mode == 3) {
            List<MapleGuild> guild;
            if (likeSearch) {
                guild = guilds.values().stream().filter(g -> Objects.requireNonNull(g.getLeaderName()).contains(text)).collect(Collectors.toList());
            } else {
                guild = guilds.values().stream().filter(g -> Objects.equals(g.getLeaderName(), text)).collect(Collectors.toList());
            }
            if (guild.size() > 0) {
                guild.forEach(g -> {
                    if (ret.stream().noneMatch(gg -> gg.getId() == g.getId())) {
                        ret.add(g);
                    }
                });
            }
