                ...
                if (map != null) {
                    ...
                    for (final MapleCharacter chr : map.getCharacters()) {
                        ...
                        if (chr.isActiveBuffedValue(400011010)) {
                            chr.addHP(-Objects.requireNonNull(SkillFactory.getSkill(400011010)).getEffect(chr.getSkillLevel(400011010)).getY());
                        }
                        ...
                        if (chr.getBuffedValue(BuffStats.DevilishPower, 400051015) != null && !(chr.getJob() >= 510 && chr.getJob() <= 512)) {
                            Objects.requireNonNull(SkillFactory.getSkill(400051015)).getEffect(1).cerpentScrew(chr);
                        }
                        if (chr.getCooldowns().size() > 0) {
                            ...
                            if (!coolList.isEmpty()) {
                                for (CoolDownValueHolder coolDownValueHolder : coolList) {
                                    chr.getClient().sendPacket(MainPacketCreator.skillCooldown(coolDownValueHolder.skillId, 0));
                                    chr.cancelCooldown(coolDownValueHolder.skillId);
                                }
                            }
                        }
                        ...
                        if (chr.isActiveBuffedValue(400011010)) {
                            ...
                            if (time - chr.lastDemonicFrenzyTime >= effect.getU() * 1000) {
                                if (chr.getStat().getHp() > chr.getMaxHp() * (effect.getStat("q2") / 100.0D)) {
                                    ...
                                    // int xMax = xMin + 500;
                                    Objects.requireNonNull(SkillFactory.getSkill(400010010)).getEffect(chr.getSkillLevel(400011010)).applyTo(chr, new Point(x, y));
                                }
                        ...
                        if (chr.getJob() / 100 == 155 && !chr.isActiveBuffedValue(155101006) && chr.getArkGage() < 1000) {
                            ...
                            if (chr.getArkGage() <= 0) {
                                ...
                                chr.getEffects().entrySet().stream().forEach((Entry<BuffStats, List<BuffStatsValueHolder>> effect1) -> effect1.getValue().stream().filter((bsvh) -> (!bsvhs.containsKey(bsvh.effect) && bsvh.effect.getSourceId() == 155101006)).forEach((bsvh) -> {
                    ...
                    if (map.getAllMonster().size() > 0) {
                        for (final MapleMapObject obj : map.getAllMonster()) {
                            ...
                            if (mob.isAlive()) {
                                ...
                                for (MonsterStatusEffect status : cancelStatus) {
