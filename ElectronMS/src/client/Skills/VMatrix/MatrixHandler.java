    ...
    public static void EquipCore(ReadingMaple rm, MapleClient c) {
        ...
        switch (v1) {
            ...
            case 6: { // 여러개 분해
                ...
                int quantity = 0;
                for (VCore vCore : list) {
                    quantity += VCoreFactory.getCoreQuantity(vCore.getCoreId());
                    c.getPlayer().cores.remove(vCore);
                }
                ...
                if (coreId / 10000000 == 2) {
                    switch (c.getPlayer().getJob()) {
                        case 112: {
                            ...
                            Triple<Integer, Integer, Integer> coreConnectOne = VCoreFactory.getCoreConnects(coreId);
                            ...
                            Core = new VCore(Count, coreId, c.getPlayer().getId(), 1, 0, 1, coreConnectOne.first, coreConnectTwo.first, coreConnectThree.first);
                        }
                        break;
                        case 122: {
            ...
            case 10: {
                ...
                int coreup = getValue2(c, "upcore" + wheref);
                if (coreup != 0) {
                    if (coreup >= 5) {
