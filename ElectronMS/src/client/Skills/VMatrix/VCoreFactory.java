...
public class VCoreFactory {
    ...
    private static boolean CheckUseableJobs(List<String> jobz, List<String> list) {
        for (String job : jobz) {
            for (String jobs : list) {
                ...
                if (jobs.equals("warrior") && GameConstants.isWarrior(Short.parseShort(job))) {
                    return true;
                }
                if (jobs.equals("magician") && GameConstants.isMagician(Short.parseShort(job))) {
                    return true;
                }
                if (jobs.equals("archer") && GameConstants.isArcher(Short.parseShort(job))) {
                    return true;
                }
                if (jobs.equals("rogue") && GameConstants.isThief(Short.parseShort(job))) {
                    return true;
                }
                if (jobs.equals("pirate") && GameConstants.isPirate(Short.parseShort(job))) {
                    return true;
                }
                if (job.equals("warrior") && GameConstants.isWarrior(Short.parseShort(jobs))) {
                    return true;
                }
                if (job.equals("magician") && GameConstants.isMagician(Short.parseShort(jobs))) {
                    return true;
                }
                if (job.equals("archer") && GameConstants.isArcher(Short.parseShort(jobs))) {
                    return true;
                }
                if (job.equals("rogue") && GameConstants.isThief(Short.parseShort(jobs))) {
                    return true;
                }
                if (job.equals("pirate") && GameConstants.isPirate(Short.parseShort(jobs))) {
                    return true;
                }
                if (GameConstants.JobCodeCheck(Short.parseShort(job), Short.parseShort(jobs))) {
                    return true;
                }
                if (GameConstants.JobCodeCheck(Short.parseShort(jobs), Short.parseShort(job))) {
                    return true;
                }
            }
        }
    ...
    public static boolean CheckOwnUseableJobs(VCoreInfo data, MapleClient c) {
        ...
        for (String jobs : list) {
            if (isNumeric(jobs)) {
                if (GameConstants.JobCodeCheck(Short.parseShort(jobs), jobcode)) {
                    return true;
                }
            }
        }
    ...
    public static boolean ResetCore(MapleClient c, VCoreInfo origin, int id1, VCoreInfo fresh, int id2, boolean checkjob) {
        ...
        return !CheckUseableJobs(fresh, c);
    }
