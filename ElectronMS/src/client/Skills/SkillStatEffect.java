...
import client.Stats.PlayerStats.PlayerStats;
...
import client.Stats.PlayerStats.PlayerStatList;

import constants.GameConstants;
import constants.ServerConstants;
import handlers.Global.MapleMechDoor;
import handlers.Global.PlayerHandler.PlayerHandler;
...
import client.ItemInventory.Items.InventoryManipulator;
import server.LifeEntity.Mob.MonsterEntity.MapleMonster;
import server.LifeEntity.Mob.MobSkillFactory;
...
public class SkillStatEffect {
    ...
    public static SkillStatEffect loadItemEffectFromData(final MapleData source, final int itemid) {
        return loadFromData(source, itemid, false, false, 0);
    }

    public static SkillStatEffect loadSkillEffectFromData(final MapleData source, final int skillid, final boolean overtime) {
        return loadFromData(source, skillid, true, overtime, 1);
    }

    public static SkillStatEffect loadSkillEffectFromData(final MapleData source, final int skillid, final boolean overtime, final int level) {
        return loadFromData(source, skillid, true, overtime, level);
    }

    private static void addBuffStatPairToListIfNotZero(final List<Triple<BuffStats, Integer, Boolean>> list, final BuffStats buffstat, final Integer val, boolean overlap) {
        if (val != 0) {
            ...
            if (!isExist) {
                list.add(new Triple<>(buffstat, val, overlap));
            }
        }
    }
    ...
    private static SkillStatEffect loadFromData(final MapleData source, final int sourceid, final boolean skill, final boolean overTime, final int level) {
        ...
        final ArrayList<Triple<BuffStats, Integer, Boolean>> statups = new ArrayList<>();

        List<DiseaseStats> cure = new ArrayList<>(5);
        ...
        if (skill) {
            switch (sourceid) {
                case 1000003: // Iron body
                    statups.add(new Triple<>(BuffStats.PDD, ret.effects.getStats("pddX"), false));
                    statups.add(new Triple<>(BuffStats.MHPCutR, ret.effects.getStats("mhpR"), false));
                    statups.add(new Triple<>(BuffStats.DamAbsorbShield, ret.effects.getStats("damAbsorbShieldR"), false));
                    break;
                case 2001002: // Magic guard
                    statups.add(new Triple<>(BuffStats.MagicGuard, ret.effects.getStats("x"), false));
                    break;
                case 2001003: // Magic armor
                    statups.add(new Triple<>(BuffStats.PDD, ret.effects.getStats("pdd"), false));
                    break;
                case 4001005: // Haste
                    statups.add(new Triple<>(BuffStats.Speed, ret.effects.getStats("speed"), false));
                    statups.add(new Triple<>(BuffStats.Jump, ret.effects.getStats("jump"), false));
                    break;
                ...
                case 1101006: // anger
                    ...
                    statups.add(new Triple<>(BuffStats.PowerGuard, ret.effects.getStats("y"), false));
                    break;
                case 1101013: // Combo Attack
                    statups.add(new Triple<>(BuffStats.ComboCounter, 1, false));
                    ret.time = Integer.MAX_VALUE;
                    break;
                case 1200014: // Elemental charge
                    statups.add(new Triple<>(BuffStats.ElementalCharge, 1, false));
                    break;
                case 1301006: // Iron wall
                    statups.add(new Triple<>(BuffStats.PDD, ret.effects.getStats("pdd"), false));
                    break;
                case 1301007: // Hyper body
                    statups.add(new Triple<>(BuffStats.MaxHP, ret.effects.getStats("x"), false));
                    statups.add(new Triple<>(BuffStats.MaxMP, ret.effects.getStats("x"), false));
                    break;
                case 1310013: // Beholder Holder Dominant
                    statups.add(new Triple<>(BuffStats.Beholder, ret.effects.getStats("x"), false));
                    break;
                case 1301013: // Beholder
                    statups.add(new Triple<>(BuffStats.Beholder, 1, false));
                    break;
                case 1310016: // Beholder's Buff
                    statups.clear();
                    statups.add(new Triple<>(BuffStats.IndieCr, ret.effects.getStats("indieCr"), true));
                    statups.add(new Triple<>(BuffStats.EPAD, ret.effects.getStats("epad"), false));
                    statups.add(new Triple<>(BuffStats.EPDD, ret.effects.getStats("epdd"), false));
                    break;
                ...
                case 1311015: // Crossover chain
                    statups.add(new Triple<>(BuffStats.CrossOverChain, ret.effects.getStats("x"), false));
                    break;
                case 1320019: // Rebirth
                    statups.add(new Triple<>(BuffStats.IndieDamR, ret.effects.getStats("indieDamR"), true));
                    statups.add(new Triple<>(BuffStats.Reincarnation, 1, false));
			...
			case 1105: // Ice knight
                ...
                case 1210016: // Blessing Armor
                    statups.add(new Triple<>(BuffStats.BlessingArmorIncPAD, ret.effects.getStats("epad"), false));
                    statups.add(new Triple<>(BuffStats.BlessingArmor, ret.effects.getStats("x"), false));
                    break;
                ...
                case 2101005: // Poison breath
                    ...
                    break;
                case 2101010: // Ignite
                    statups.add(new Triple<>(BuffStats.WizardIgnite, 1, false));
                    break;
                case 2101001: // Meditation
                    statups.add(new Triple<>(BuffStats.IndieMAD, ret.effects.getStats("indieMad"), true));
                    break;
                case 2111010: // Slime virus
                    monsterStatus.put(MonsterStatus.POISON, 1);
                    break;
                case 2111011: // Elemental Adaptering (Fire, Poison)
                    statups.add(new Triple<>(BuffStats.StackBuff, ret.effects.getStats("y"), false));
                    ret.overTime = true;
                    break;
                case 2111007: // Teleport Mastery (Fire, Poison)
                    statups.add(new Triple<>(BuffStats.TeleportMasteryOn, ret.effects.getStats("x"), false));
                    monsterStatus.put(MonsterStatus.Burned, 1);
                    monsterStatus.put(MonsterStatus.STUN, 1);
                    ret.time = Integer.MAX_VALUE;
                    break;
                ...
                case 2221045: // Teleport Mastery-Range (Sun, Call)
                    statups.add(new Triple<>(BuffStats.TeleportMasteryRange, ret.effects.getStats("x"), false));
                    ret.time = Integer.MAX_VALUE;
                    break;
...
//                    monsterStatus.put(MonsterStatus.WDEF, ret.effects.getStats("x"));
                ...
                case 2221054: // Ice aura
                    statups.add(new Triple<>(BuffStats.IndieStance, ret.effects.getStats("x"), true));
                    statups.add(new Triple<>(BuffStats.IceAura, 1, false));
                    ret.time = Integer.MAX_VALUE;
                    break;
...
//                    monsterStatus.put(MonsterStatus.WDEF, -ret.effects.getStats("w"));
                ...
                case 31121003: // Devil Cry
                    statups.add(new Triple<>(BuffStats.DevilCry, 1, false));
                ...
                case 35121055: // Bomber Time
                    statups.add(new Triple<>(BuffStats.BombTime, 1, false));
                    break;
                ...
                case 51121006: // Soul rage
                    statups.add(new Triple<>(BuffStats.Enrage, ret.effects.getStats("x") * 100 + ret.effects.getStats("mobCount"), false));
                    statups.add(new Triple<>(BuffStats.EnrageCr, ret.effects.getStats("z"), false));
                    statups.add(new Triple<>(BuffStats.EnrageCrDamMin, ret.effects.getStats("y"), false));
                    break;
                ...
                case 13101022: // Tripling 윔 I
                    statups.add(new Triple<>(BuffStats.TriflingWhimOnOff, 1, false));
                    break;
                case 22171073: // Onyx Blessing
                    statups.add(new Triple<>(BuffStats.EMAD, ret.effects.getStats("emad"), false));
                    statups.add(new Triple<>(BuffStats.EPDD, ret.effects.getStats("epdd"), false));
                    break;
                ...
                case 14001022: // Haste
                    ...
                    break;
                case 2301004: // Bless
                    statups.add(new Triple<>(BuffStats.Bless, ret.effects.getStats("x"), false));
                    break;
                case 2300009: // Blessing Ensemble.
                    statups.add(new Triple<>(BuffStats.BlessEnsenble, ret.effects.getStats("x"), false));
                    break;
                ...
                case 14001027: // Shadow bat
                    statups.add(new Triple<>(BuffStats.NightWalkerBat, 2, false));
                    statups.add(new Triple<>(BuffStats.Speed, ret.effects.getStats("speed"), false));
                    ret.time = Integer.MAX_VALUE;
                    break;
                ...
                case 4211003: // Pick Pocket
                    statups.add(new Triple<>(BuffStats.PickPocket, ret.effects.getStats("x"), false));
                    ret.time = Integer.MAX_VALUE;
                    break;
                case 4201011: // Meso Guard
                    statups.add(new Triple<>(BuffStats.MesoGuard, ret.effects.getStats("x"), false));
                    break;
                case 4111002: // Shadow Partner
                case 4211008: // Shadow Partner
                case 36111006: // Virtual projection
                    statups.add(new Triple<>(BuffStats.ShadowPartner, ret.effects.getStats("x"), false));
                    break;
                case 15101006: // Lightning Charge
                case 21101006: // Aran-Snow Charge
                    statups.add(new Triple<>(BuffStats.WeaponCharge, ret.effects.getStats("x"), false));
                    break;
                case 21120021: // Swing Study II
                    statups.add(new Triple<>(BuffStats.AranSmashSwing, ret.effects.getStats("w"), false));
                    break;
                case 1311008: // Dragon strength
                    statups.add(new Triple<>(BuffStats.STR, ret.effects.getStats("str"), false));
                    break;
                case 33101004: // Mine
                    break;
                ...
                case 1001003: // Iron body
                    statups.add(new Triple<>(BuffStats.IndiePDD, ret.effects.getStats("indiePdd"), true));
                    break;
                case 3101004: // Soul Arrow: Bow
                case 3201004: // Soul Arrow: Crossbow
                case 13101003: // Soul Arrow: Bow
                    statups.add(new Triple<>(BuffStats.EPAD, ret.effects.getStats("epad"), false));
                    statups.add(new Triple<>(BuffStats.SoulArrow, ret.effects.getStats("x"), false));
                    break;
                ...
                case 400001002: // Useful Sharp Eyes
                    ...
                    statups.add(new Triple<>(BuffStats.SharpEyes, value, false));
                    break;
                case 3121007: // Illusion Staff
                case 3221006: // Illusion Staff
                    statups.add(new Triple<>(BuffStats.DEX, ret.effects.getStats("dex"), false));
                    statups.add(new Triple<>(BuffStats.IllusionStep, ret.effects.getStats("x"),
                            false));
                    break;
                case 3121016: // Advanced Quiver
                    statups.add(new Triple<>(BuffStats.AdvancedQuiver, 1, false));
                    break;
                case 2111008: // Elemental Reset
                case 2211008: // Elemental Reset
                case 12101005: // Elemental Reset
                case 22141016: // Elemental Reset
                    statups.add(new Triple<>(BuffStats.ElementalReset, ret.effects.getStats("x"), false));
                    break;
                ...
                case 5120018: // Ultra charge
                    statups.add(new Triple<>(BuffStats.RWMovingEvar, sourceid != 5100015 ? 0 : 1, false));
                    break;
                case 142001007: // Psychic Instinct
                    statups.add(new Triple<>(BuffStats.KinesisPsychicEnergeShield, 1, false));
                    break;
                case 142001003: // ESP Booster
                    statups.add(new Triple<>(BuffStats.IndieBooster, ret.effects.getStats("indieBooster"), true));
                    break;
                case 142101004: // Psychic shield
                    statups.add(new Triple<>(BuffStats.KinesisPsychicShield, ret.effects.getStats("er"), false));
                    statups.add(new Triple<>(BuffStats.IndiePDD, ret.effects.getStats("indiePdd"), true));
                    break;
                ...
                case 142111008: // Mental reinforcement
                    statups.add(new Triple<>(BuffStats.IndieMADR, ret.effects.getStats("indieMadR"), true));
                    break;
                ...
                case 64101003: // Weapon Booster
                    statups.add(new Triple<>(BuffStats.Booster, ret.effects.getStats("x"), false));
                    break;
                case 21001003: // Pole arm booster
                    statups.add(new Triple<>(BuffStats.Booster, -(ret.effects.getStats("y")), false));
                    break;
                case 152101007: // Magic Gauntlet Booster
                    statups.add(new Triple<>(BuffStats.Booster, -2, false));
                    break;
                case 12120043: // Flame Orbita Range
                case 12121043:
                    statups.add(new Triple<>(BuffStats.AddRangeOnOff, ret.effects.getStats("range") + 500, true));
                    break;
                case 12000022: // Element: Flame 1
                case 12100026: // Element: Flame 2
                case 12110024: // Element: Flame 3
                case 12120007: // Element: Flame 4
                    statups.add(new Triple<>(BuffStats.MAD, ret.effects.getStats("x"), false));
                    break;
                ...
                case 12120014: // Spirit of Flame
                    statups.add(new Triple<>(BuffStats.IgnoreMobDamR, ret.effects.getStats("y"), false));
                    break;
                case 12121003: // Flame barrier
                    statups.add(new Triple<>(BuffStats.FireBarrier, ret.effects.getStats("x"), false));
                    break;
                ...
                case 14111024: // Shadow Servant
                    statups.add(new Triple<>(BuffStats.ShadowServant, 1, false));
                    break;
                case 14121054: // Shadow Illusion
                // case 14121055: // Shadow Illusion
                // case 14121056: // Shadow Illusion
                	statups.add(new Triple<>(BuffStats.ShadowIllusion, 1, false));
                	break;
                ...
                case 20031205: // Phantom Shroud
                    statups.add(new Triple<>(BuffStats.Invisible, ret.effects.getStats("x"), false));
                    break;
                case 24111002: // Luck of Phantom Sheep
                    statups.add(new Triple<>(BuffStats.StackBuff, 1, false));
                    break;
                case 24111003: // Missposham Protection
                    statups.add(new Triple<>(BuffStats.IndieMHPR, ret.effects.getStats("indieMhpR"), true));
                    statups.add(new Triple<>(BuffStats.IndieMMPR, ret.effects.getStats("indieMmpR"), true));
                    break;
                case 24111005: // Moonlight
                    statups.clear();
                    statups.add(new Triple<>(BuffStats.IndiePAD, ret.effects.getStats("indiePad"), true));
                    statups.add(new Triple<>(BuffStats.IndieACC, ret.effects.getStats("indieAcc"), true));
                    break;
                ...
                case 21111001: // Mite
                    statups.add(new Triple<>(BuffStats.EPAD, ret.effects.getStats("epad"), false));
                    statups.add(new Triple<>(BuffStats.EPDD, ret.effects.getStats("epdd"), false));
                    statups.add(new Triple<>(BuffStats.KnockBack, ret.effects.getStats("x"), false));
                    break;
                ...
                case 30018004: // Useful Combat Orders
                    statups.add(new Triple<>(BuffStats.CombatOrders, ret.effects.getStats("x"), false));
                    break;
                case 5121009: // Wind booster
                case 15111005: // Wind booster
                case 15121005: // Wind booster
                    statups.add(new Triple<>(BuffStats.PartyBooster, ret.effects.getStats("x"), false));
                    break;
                case 64100004:
                case 64110005:
                case 64120006:
                    statups.add(new Triple<>(BuffStats.KadenaStack, 1, false));
                    break;
                case 400001006: // Useful Wind Booster
                    statups.add(new Triple<>(BuffStats.Speed, ret.effects.getStats("x"), true));
                    break;
                case 5001005: // Dash
                    statups.add(new Triple<>(BuffStats.Dash_Speed, ret.effects.getStats("x"), false));
                    statups.add(new Triple<>(BuffStats.Dash_Jump, ret.effects.getStats("y"), false));
                    break;
                case 4321000: // Tornado Spin
                    ret.time = 1000;
                    statups.add(new Triple<>(BuffStats.Speed, 100 + ret.effects.getStats("x"), false));
                    statups.add(new Triple<>(BuffStats.Jump, ret.effects.getStats("y"), false)); // always																										// there
                    break;
                case 1101007: // Power reflection
                case 1201007: // Power reflection
                    statups.add(new Triple<>(BuffStats.ACC, ret.effects.getStats("x"), false));
                    break;
                case 31101003: // Dark Revenge
                    statups.add(new Triple<>(BuffStats.PowerGuard, 1, false));
                    monsterStatus.put(MonsterStatus.STUN, 1);
                    break;
                ...
                case 9001008: // Hyper body
                    ...
                    break;
                case 1001: // Recovery
                    statups.add(new Triple<>(BuffStats.DotHealHPPerSecond, ret.effects.getStats("x"), false));
                    break;
                ...
                case 4120045: // Showdown - Enhance
                    statups.add(new Triple<>(BuffStats.IndieEXP, ret.effects.getStats("indieExp"), false));
                    statups.add(new Triple<>(BuffStats.DropRIncrease, ret.effects.getStats("DropRIncrease"), false));
                    break;
                case 4341002: // Final cut
                    //ret.time = 60 * 1000;
                    //ret.overTime = true;
                    //ret.effects.setStats("hpR", -ret.effects.getStats("x"));
                    statups.add(new Triple<>(BuffStats.FinalCut, ret.effects.getStats("w") + 100, false));
                    statups.add(new Triple<>(BuffStats.NotDamaged, 1, false));
                    break;
                ...
                case 27110007: // Life Tidal
                    statups.add(new Triple<>(BuffStats.LifeTidal, 1, false));
                    break;
                case 1121000:
                case 1221000:
                case 1321000:
                ...
                case 155121008:
                case 164121009:
                    statups.add(new Triple<>(BuffStats.BasicStatUp, ret.effects.getStats("x"), false));
                    break;
                case 64120007:
                    statups.add(new Triple<>(BuffStats.CriticalBuff, ret.effects.getStats("w"), false));
                    statups.add(new Triple<>(BuffStats.EnrageCrDamMin, ret.effects.getStats("x"), false));
                    break;
                case 3111011: // Extreme Archery: Bow
                    statups.add(new Triple<>(BuffStats.PDD, -ret.effects.getStats("x"), false));
                    statups.add(new Triple<>(BuffStats.PAD, ret.effects.getStats("padX"), false));
                    statups.add(new Triple<>(BuffStats.IndiePMdR, ret.effects.getStats("indiePMdR"), true));
                    break;
                case 3211012: // Extreme Archery: Crossbow
                    statups.add(new Triple<>(BuffStats.ExtremeArchery, ret.effects.getStats("x") / 5 * 2, false));
                    break;
                case 37121054: // Maximaise Cannon
                    statups.add(new Triple<>(BuffStats.RWMaximizeCannon, ret.effects.getStats("y"), false));
                    break;
                case 37101001: // Ducking
                    statups.add(new Triple<>(BuffStats.RWMovingEvar, 90, false));
                    break;
                case 25121209: // Soul
                    statups.add(new Triple<>(BuffStats.SpiritGuard, 3, false));
                    break;
                case 25121131: // Maximize Elemental Bond
                    statups.clear();
                    statups.add(0, new Triple<>(BuffStats.IndiePAD, ret.effects.getStats("indiePad"), true));
                    statups.add(1, new Triple<>(BuffStats.IndieBooster, ret.effects.getStats("indieBooster"), true));
                    statups.add(2, new Triple<>(BuffStats.IndieDamR, ret.effects.getStats("indieDamR"), true));
                    statups.add(3, new Triple<>(BuffStats.IndieBDR, ret.effects.getStats("indieBDR"), true));
                    statups.add(4, new Triple<>(BuffStats.IndieIgnoreMobpdpR, ret.effects.getStats("indieIgnoreMobpdpR"), true));
                    break;
                case 65121004: // Soul something gays???? Wtf is this
                    statups.add(new Triple<>(BuffStats.SoulGazeCriDamR, ret.effects.getStats("x"), false));
                    break;
                case 22151003: // Magic Resistance
                    statups.add(new Triple<>(BuffStats.MagicResistance, ret.effects.getStats("x"), false));
                    break;
                case 21000000: // Aran Combo
                    statups.add(new Triple<>(BuffStats.ComboAbilityBuff, ret.effects.getStats("x"), false));
                    break;
                case 21101005: // Drain
                    statups.add(new Triple<>(BuffStats.ComboDrain, ret.effects.getStats("x"), false));
                    break;
                case 31110004: // Dark Endure
                    statups.add(new Triple<>(BuffStats.PDD, ret.effects.getStats("pddR"), false));
                    statups.add(new Triple<>(BuffStats.IndieAsrR, ret.effects.getStats("asrR"), true));
                    statups.add(new Triple<>(BuffStats.IndieTerR, ret.effects.getStats("terR"), true));
                    break;
                case 51111004: // Soul Indure
                    statups.add(new Triple<>(BuffStats.DefenseAtt, ret.effects.getStats("x"), false));
                    statups.add(new Triple<>(BuffStats.IndieAsrR, ret.effects.getStats("y"), true));
                    statups.add(new Triple<>(BuffStats.IndieTerR, ret.effects.getStats("z"), true));
                    break;
                ...
                case 31121005: // Metamorphosis
                    ...
                    statups.add(new Triple<>(BuffStats.IndieMHPR, ret.effects.getStats("indieMhpR"), true));
                    statups.add(new Triple<>(BuffStats.DamR, ret.effects.getStats("damR"), false));
                    statups.add(new Triple<>(BuffStats.DevilishPower, dam < 0 ? -dam : dam, false));
                    ret.overTime = true;
                    break;
                case 31121007: // Infinity force
                    statups.add(new Triple<>(BuffStats.InfinityForce, 1, false));
                    break;
                case 31121002: // Python touch
                    statups.add(new Triple<>(BuffStats.VampiricTouch, ret.effects.getStats("x"), false));
                    break;
                case 22131001: // Magic shield
                    statups.add(new Triple<>(BuffStats.PDD, ret.effects.getStats("pdd"), false));
                    statups.add(new Triple<>(BuffStats.PartyBarrier, ret.effects.getStats("x"), false));
                    break;
                case 22181003: // Soulstone
                    statups.add(new Triple<>(BuffStats.SoulStone, ret.effects.getStats("x"), false));
                    break;
                ...
                case 23111004: // Ignis Roar
                    statups.add(new Triple<>(BuffStats.IndiePAD, ret.effects.getStats("indiePad"), true));
                    statups.add(new Triple<>(BuffStats.StackBuff, ret.effects.getStats("x"), true));
                    break;
                case 23121004: // Ancient Spirit
                    statups.add(new Triple<>(BuffStats.EMHP, ret.effects.getStats("emhp"), false));
                    statups.add(new Triple<>(BuffStats.IndiePADR, ret.effects.getStats("indiePadR"), true));
                    break;
                ...
                case 33001015: // Summon Jaguar
                    statups.add(new Triple<>(BuffStats.JaguarSummoned, 3870, false));
                    ret.time = Integer.MAX_VALUE;
                    break;
                case 33101005: // Howling
                    statups.add(new Triple<>(BuffStats.HowlingDefence, ret.effects.getStats("x"), false));
                    statups.add(new Triple<>(BuffStats.HowlingMaxMP, ret.effects.getStats("x"), false));
                    statups.add(new Triple<>(BuffStats.HowlingEvasion, ret.effects.getStats("x"), false));
                    statups.add(new Triple<>(BuffStats.HowlingCritical, ret.effects.getStats("x"), false));
                    statups.add(new Triple<>(BuffStats.HowlingAttackDamage, ret.effects.getStats("z"), false));
                    break;
                case 33111007: // Beast form
                    statups.clear();
                    statups.add(new Triple<>(BuffStats.Speed, ret.effects.getStats("x"), false));
                    statups.add(new Triple<>(BuffStats.BeastFormDamageUp, ret.effects.getStats("z"), false));
                    statups.add(new Triple<>(BuffStats.Booster, ret.effects.getStats("w"), false));
                    break;
                case 2311009: // Holy Magic Shell
                    statups.add(new Triple<>(BuffStats.HolyMagicShell, ret.effects.getStats("x"), false));
                    ret.overTime = true;
                    break;
                case 11001022: // Element: Soul
                    statups.add(new Triple<>(BuffStats.ElementSoul, 1, false));
                    monsterStatus.put(MonsterStatus.STUN, 1);
                    break;
                case 11101022: // Polling doors
                    statups.add(new Triple<>(BuffStats.IndieCr, ret.effects.getStats("indieCr"), true));
                    statups.add(new Triple<>(BuffStats.BuckShot, ret.effects.getStats("x"), false));
                    statups.add(new Triple<>(BuffStats.PoseType, 1, false));
                    break;
                case 11101023: // Inner trust
                    statups.add(new Triple<>(BuffStats.IndiePAD, ret.effects.getStats("indiePad"), true));
                    break;
                case 11111022: // Rising Sun
                    statups.add(new Triple<>(BuffStats.PoseType, 2, false));
                    statups.add(new Triple<>(BuffStats.IndieBooster, ret.effects.getStats("indieBooster"), true));
                    statups.add(new Triple<>(BuffStats.IndiePMdR, ret.effects.getStats("indiePMdR"), true)); //IndieDamR
                    break;
                ...
                case 11111024: // Soul guardian
                    statups.add(new Triple<>(BuffStats.IndiePDD, ret.effects.getStats("indiePdd"), true));
                    statups.add(new Triple<>(BuffStats.IndieMHP, ret.effects.getStats("indieMhp"), true));
                    break;
                ...
                case 2022994:
                    if (sourceid == 5310008) { // Monkey Wave Top Output
                        statups.add(new Triple<>(BuffStats.KeyDownTimeIgnore, 1, false));
                    }
                ...
                case 2321010: // Master magic
                    statups.add(new Triple<>(BuffStats.MasterMagicOn, 1, false));
                    ret.time = Integer.MAX_VALUE;
                    break;
                case 12101001: // Slow
                    monsterStatus.put(MonsterStatus.SPEED, ret.effects.getStats("x"));
				...
				case 400011077:
                    monsterStatus.put(MonsterStatus.STUN, 1);
                    statups.add(new Triple<>(BuffStats.SUMMON, 1, false));
                    break;
                case 1085:
                case 1087:
                case 1090:
                ...
                case 60011087:
                case 400001017:
                    statups.add(new Triple<>(BuffStats.IndieStatR,ret.effects.getStats("indieStatRBasic"),true));
                    monsterStatus.put(MonsterStatus.MAGIC_DEFENSE_UP,ret.effects.getStats("w"));
                    break;
                case 60011090:
                case 60011179:
                    ret.time = Integer.MAX_VALUE;
                ...
                case 400001020: // Usable Holy Symbol
                    statups.add(new Triple<>(BuffStats.HolySymbol, ret.effects.getStats("x"), false));
                    break;
                case 2211004: // Seal
                case 2111004: // Seal
                case 12111002: // Seal
                ...
                case 5321010: // Pirate Spirit
                    statups.add(new Triple<>(BuffStats.Stance, ret.effects.getStats("x"), false));
                    break;
                case 1005:
                case 10001005:
                case 20001005:
                ...
                case 50001005:
                case 100001005:
                    statups.add(new Triple<>(BuffStats.MaxLevelBuff, ret.effects.getStats("x"), false));
                    break;
                case 2121002: // Mana Reflection
                case 2221002: // Mana Reflection
                case 2321002: // Mana Reflection
                    statups.add(new Triple<>(BuffStats.ManaReflection, 1, false));
                    break;
                case 2321005: // Advanced Bless
                    statups.add(new Triple<>(BuffStats.AdvancedBless, level, false));
                    statups.add(new Triple<>(BuffStats.ACC, ret.effects.getStats("x"), false));
                    statups.add(new Triple<>(BuffStats.IndieMHP, ret.effects.getStats("indieMhp"), true));
                    statups.add(new Triple<>(BuffStats.IndieMMP, ret.effects.getStats("indieMmp"), true));
                    break;
                ...
                case 9001003: // Operator's blessing
                    statups.add(new Triple<>(BuffStats.Speed, ret.effects.getStats("speed"), false));
                    statups.add(new Triple<>(BuffStats.PDD, ret.effects.getStats("pdd"), false));
                    statups.add(new Triple<>(BuffStats.EVA, ret.effects.getStats("eva"), false));
                    statups.add(new Triple<>(BuffStats.IndiePAD, ret.effects.getStats("indiePad"), true));
                    statups.add(new Triple<>(BuffStats.IndieMAD, ret.effects.getStats("indieMad"), true));
                    statups.add(new Triple<>(BuffStats.IndieMHPR, ret.effects.getStats("indieMhpR"), true));
                    statups.add(new Triple<>(BuffStats.IndieMMPR, ret.effects.getStats("indieMmpR"), true));
                ...
                case 5221018: // Pyret style
                    statups.add(new Triple<>(BuffStats.IndieEVA, ret.effects.getStats("indieEva"), true));
                    statups.add(new Triple<>(BuffStats.IndieAsrR, 30, true));
                    statups.add(new Triple<>(BuffStats.IndieStance, 60, true));
                    statups.add(new Triple<>(BuffStats.IndiePADR, 20, true));
                    break;
                case 5301003: // Monkey Magic
                case 5320008: // Hyper Monkey Spell
                    statups.clear();
                    statups.add(0, new Triple<>(BuffStats.IndieMHP, ret.effects.getStats("indieMhp"), true));
                    statups.add(1, new Triple<>(BuffStats.IndieMMP, ret.effects.getStats("indieMmp"), true));
                    statups.add(2, new Triple<>(BuffStats.IndieJump, ret.effects.getStats("indieJump"), true));
                    statups.add(3, new Triple<>(BuffStats.IndieSpeed, ret.effects.getStats("indieSpeed"), true));
                    statups.add(4, new Triple<>(BuffStats.IndieAllStat, ret.effects.getStats("indieAllStat"), true));
                    break;
                case 5311004: // Oak barrel roulette
                    statups.add(new Triple<>(BuffStats.Roulette, 4, false));
                    break;
                ...
                case 80001036: // Saint Saber Stage 3
                    ...
                    statups.add(new Triple<>(BuffStats.IndiePAD, ret.effects.getStats("indiePad"), true));
                    statups.add(new Triple<>(BuffStats.IndieMAD, ret.effects.getStats("indieMad"), true));
                    statups.add(new Triple<>(BuffStats.IndieJump, ret.effects.getStats("indieJump"), true));
                    statups.add(new Triple<>(BuffStats.IndieSpeed, ret.effects.getStats("indieSpeed"), true));
                    break;
                case 5111007:
                case 5120012:
                case 5211007:
                ...
                case 5320007: // Lucky dice
                    ret.overTime = true;
                    statups.add(new Triple<>(BuffStats.Dice, 0, false));
                    break;
                case 23101003: // Spirit Infusion
                    statups.add(new Triple<>(BuffStats.DamageReduce, ret.effects.getStats("damage"), false));
                    statups.add(new Triple<>(BuffStats.CriticalBuff, ret.effects.getStats("x"), false));
                    break;
                case 23111005: // Water shield
                    statups.add(new Triple<>(BuffStats.IndieAsrR, ret.effects.getStats("terR"), true));
                    statups.add(new Triple<>(BuffStats.IndieTerR, ret.effects.getStats("terR"), true));
                    statups.add(new Triple<>(BuffStats.DamAbsorbShield, 1, false));
                    break;
                case 400031007:
                case 400051022:
                    statups.add(new Triple<>(BuffStats.SUMMON, 1, false));
                    break;
                case 400041008:
                    statups.add(new Triple<>(BuffStats.ShadowSpear, 1, false));
                    break;
                case 80000169: // Speaking life
                case 20050286: // Speaking life
                    // statups.add(new Triple<>(BuffStats.Stance, 1, false)); Disabled for now it causes lag
                    break;
                case 25101009: // Spirit Fox
                    statups.add(new Triple<>(BuffStats.HiddenPossession, 1, false));
                    break;
                case 35111016: // Overtuning
                    statups.add(new Triple<>(BuffStats.IndieDamR, ret.effects.getStats("indieDamR"), true));
                    break;
                case 35121010: // Amplifier
                    statups.add(new Triple<>(BuffStats.AmplifyDamage, ret.effects.getStats("x"), false));
                    ret.time = 60000;
                    break;
                ...
                case 35111008: // Healing Robot
                    statups.add(new Triple<>(BuffStats.MaxHP, ret.effects.getStats("hp"), false));
                    statups.add(new Triple<>(BuffStats.MaxHP, ret.effects.getStats("hcHp"), false));
                    break;
                ...
                case 32001016: // Yellow aura
                    statups.add(new Triple<>(BuffStats.BMageAura, level, false));
                    statups.add(new Triple<>(BuffStats.IndieSpeed, ret.effects.getStats("indieSpeed"), true));
                    statups.add(new Triple<>(BuffStats.IndieBooster, ret.effects.getStats("indieBooster"), true));
                    ret.time = Integer.MAX_VALUE;
                    break;
                ...
                case 32111016: // Dark lightning
                    statups.add(new Triple<>(BuffStats.DarkLighting, 1, false));
                    ret.time = Integer.MAX_VALUE;
                    break;
                ...
                case 32120019: // Death Contract 3
                    statups.add(new Triple<>(BuffStats.BMageDeath, 0, false));
                    ret.time = Integer.MAX_VALUE;
                    break;
                ...
                case 33111011: // Draw bag
                    statups.add(new Triple<>(BuffStats.DrawBack, ret.effects.getStats("x"), false));
                    ret.time = Integer.MAX_VALUE;
                    break;
                ...
                case 2120010: // Arcane Aim
                    ret.time = 5000;
                    statups.add(new Triple<>(BuffStats.ArcaneAim, ret.effects.getStats("x"), false));
                    break;
                case 4101011:
                ...
                case 3220005: // Spirit Link: Freezer
                    ret.overTime = true;
                    statups.add(new Triple<>(BuffStats.SpiritLink, ret.effects.getStats("x"), false));
                    break;
                case 13111005: // Albatross
                    ...
                    statups.add(new Triple<>(BuffStats.Jump, ret.effects.getStats("jump"), false));
                    statups.add(new Triple<>(BuffStats.EPDD, ret.effects.getStats("epdd"), false));
                    statups.add(new Triple<>(BuffStats.Albatross, 1, false));
                    break;
                case 27001004: // Extended Mana
                    statups.add(new Triple<>(BuffStats.IndieMMPR, ret.effects.getStats("indieMmpR"), true));
                    break;
                case 27101202: // Body Pressure
                    statups.add(new Triple<>(BuffStats.KeyDownAreaMoving, ret.effects.getStats("x"), false));
                    break;
                case 400021071:
                    statups.add(new Triple<>(BuffStats.LightOrDark, 1, false));
                    break;
                case 27100003: // Bless of Darkness
                    statups.add(new Triple<>(BuffStats.BlessOfDarkness, ret.effects.getStats("y"), false));
                    ret.overTime = true;
                    break;
                ...
                case 20040216: // Sunfire
                    statups.add(new Triple<>(BuffStats.Larkness, ret.effects.getStats("x"), false));
                    break;
                case 20040217: // Eclipse
                    statups.add(new Triple<>(BuffStats.Larkness, ret.effects.getStats("y"), false));
                    break;
                case 20040219: // Equality
                case 20040220: // Equality
                    statups.add(new Triple<>(BuffStats.Larkness, 2, false));
                    ret.time = 1000;
                    break;
                case 27121005: // Dark Crescendo
                    statups.add(new Triple<>(BuffStats.StackBuff, ret.effects.getStats("x"), false));
                    break;
                case 27121006: // Darkness Sorcerer
                    statups.add(new Triple<>(BuffStats.ElementalReset, ret.effects.getStats("y"), false));
                    break;
                case 31121053:
                case 31221053:
                case 32121053:
                ...
                case 32121056: // Battle master
                    statups.add(new Triple<>(BuffStats.MASTER_OF_DEATH, 1, false));
                    break;
                case 33121054: // Silent Lampi
                    statups.add(new Triple<>(BuffStats.IndieDamR, ret.effects.getStats("indieDamR"), true));
                    statups.add(new Triple<>(BuffStats.FinalAttackProp, ret.effects.getStats("x"), false));
                    break;
                ...
                case 60001216: // Reshuffle Switch: Defense Mode
                    statups.add(new Triple<>(BuffStats.ReshuffleSwitch, 1, false));
                    ret.time = Integer.MAX_VALUE;
                    ret.overTime = true;
                    break;
                case 60001217: // Reshuffle Switch: Attack Mode
                    statups.add(new Triple<>(BuffStats.ReshuffleSwitch, 0, false));
                    ret.time = Integer.MAX_VALUE;
                    ret.overTime = true;
                    break;
                ...
                case 61121217: // Advanced Will of Sword (Transfiguration)
                    statups.add(new Triple<>(BuffStats.StopForceAtomInfo, ret.effects.getStats("cooltime"), false));
                ...
                case 61121009: // Robust Armor
                    statups.add(new Triple<>(BuffStats.PartyBarrier, ret.effects.getStats("v"), false));
                    break;
                ...
                case 65101002: // Power transfer
                    statups.add(new Triple<>(BuffStats.PowerTransferGauge, 1000, false));
                    break;
                ...
                case 65121053: // Final contract
                    statups.add(new Triple<>(BuffStats.CriticalBuff, ret.effects.getStats("x"), false));
                    statups.add(new Triple<>(BuffStats.IndieAsrR, ret.effects.getStats("asrR"), true));
                    statups.add(new Triple<>(BuffStats.TerR, ret.effects.getStats("terR"), false));
                    statups.add(new Triple<>(BuffStats.IndieStance, ret.effects.getStats("indieStance"), true));
                    ret.overTime = true;
                    break;
                ...
                case 31211003: // Lift Evil
                    statups.add(new Triple<>(BuffStats.IndieAsrR, ret.effects.getStats("y"), true));
                    statups.add(new Triple<>(BuffStats.IndieTerR, ret.effects.getStats("z"), true));
                    statups.add(new Triple<>(BuffStats.DamAbsorbShield, ret.effects.getStats("x"), false));
                    break;
                case 31211004: // Diabolik Recovery
                    statups.add(new Triple<>(BuffStats.DiabolikRecovery, ret.effects.getStats("x"), false));
                    statups.add(new Triple<>(BuffStats.IndieMHPR, ret.effects.getStats("indieMhpR"), true));
                    break;
                ...
                case 51121054: // Sacred Cube [Hyper]
                    statups.clear();
                    statups.add(0, new Triple<>(BuffStats.IndieMHPR, ret.effects.getStats("indieMhpR"), true));
                    statups.add(1, new Triple<>(BuffStats.IndieDamR, ret.effects.getStats("indieDamR"), true));
                    ret.overTime = true;
                    break;
                ...
                case 31221000: // Exceed: Execution
                    statups.add(new Triple<>(BuffStats.Exceed, 1, false));
                    break;
                case 36001002: // Incline Power
                    statups.add(new Triple<>(BuffStats.IndiePAD, ret.effects.getStats("indiePad"), true));
                    break;
                case 36111003: // Dual Breed Defensive
                    statups.add(new Triple<>(BuffStats.StackBuff, ret.effects.getStats("prop"), false));
                    statups.add(new Triple<>(BuffStats.DamageReduce, ret.effects.getStats("z"), false));
                    break;
                case 36111004: // Aegis system
                    statups.add(new Triple<>(BuffStats.XenonAegisSystem, 1, false));
                    break;
                case 36001005: // Pinpoint rocket
                    statups.add(new Triple<>(BuffStats.PinPointRocket, 1, false));
                    break;
                ...
                case 36121004: // Offensive Matrix
                    statups.add(new Triple<>(BuffStats.Stance, ret.effects.getStats("stanceProp"), false));
                    statups.add(new Triple<>(BuffStats.IgnoreMobpdpR, ret.effects.getStats("ignoreMobpdpR"), false));
                    break;
                ...
                case 30021237: // Aviation Liberty
                    ...
                    break;
                case 4341052: // Asura
                    statups.add(new Triple<>(BuffStats.Asura, 100, false));
                    break;
                case 4341054: // Hidden Blade
                    statups.add(new Triple<>(BuffStats.WindBreakerFinal, ret.effects.getStats("damage") - 100, false));
                    statups.add(new Triple<>(BuffStats.IndieDamR, 10, true));
                    ret.overTime = true;
                    break;
                case 3110012: // Concentratio
                    statups.add(new Triple<>(BuffStats.Concentration, 1, false));
                    break;
                case 4221054: // Flip the coin
                    statups.add(new Triple<>(BuffStats.FlipTheCoin, 1, false));
                    statups.add(new Triple<>(BuffStats.IndieDamR, ret.effects.getStats("indieDamR"), true));
                    statups.add(new Triple<>(BuffStats.IndieCr, ret.effects.getStats("x"), true));
                    break;
                case 5121054: // Stimulate
                    statups.add(new Triple<>(BuffStats.IndieDamR, ret.effects.getStats("indieDamR"), true));
                    statups.add(new Triple<>(BuffStats.Stimulate, 1, false));
                    break;
                case 5121055: // Unity of power
                    statups.add(new Triple<>(BuffStats.UnityOfPower, 1, false));
                    break;
                case 5321054: // Buck shot
                    statups.add(new Triple<>(BuffStats.BuckShot, level, false));
                    break;
                case 15121004: // Torpedo
                    statups.add(new Triple<>(BuffStats.ShadowPartner, 1, false));
                    break;
                case 15121054: // Heaven and earth
                    statups.add(new Triple<>(BuffStats.StrikerHyperElectric, ret.effects.getStats("x"), false));
                    statups.add(new Triple<>(BuffStats.IndieDamR, ret.effects.getStats("indieDamR"), true));
                    break;
                ...
                case 2121054: // Fire aura
                    statups.add(new Triple<>(BuffStats.FireAura, 1, false));
                    break;
                case 31121054: // Blue blood
                    statups.add(new Triple<>(BuffStats.ShadowPartner, ret.effects.getStats("x"), false));
                    break;
                case 2321055: // Heaven's Door
                    statups.add(new Triple<>(BuffStats.HeavensDoor, 1, false));
                    ret.time = Integer.MAX_VALUE;
                    break;
                case 2321054: //Vengeance of Angel

                    ret.absstats = false;
                    statups.add(new Triple<>(BuffStats.IgnoreTargetDEF, 0, false));
                    statups.add(new Triple<>(BuffStats.VengeanceOfAngel, 1, false));
                    statups.add(new Triple<>(BuffStats.IndieMAD, 50, true));
                    statups.add(new Triple<>(BuffStats.IndieBooster, -1, true));
                    statups.add(new Triple<>(BuffStats.IndieIgnoreMobpdpR, 20, true));
                    statups.add(new Triple<>(BuffStats.IndiePMdR, 30, true));

                    ret.time = Integer.MAX_VALUE;
                    break;
                case 5221021: // Quick draw
                    statups.add(new Triple<>(BuffStats.QuickDraw, ret.effects.getStats("damR"), false));
                    break;
                case 5221054: // Unweiring Nectar
                    statups.add(new Triple<>(BuffStats.IndieAsrR, ret.effects.getStats("y"), true));
                    statups.add(new Triple<>(BuffStats.IndieTerR, ret.effects.getStats("v"), true));
                    statups.add(new Triple<>(BuffStats.MaxHP, ret.effects.getStats("y"), false));
                    statups.add(new Triple<>(BuffStats.IndieCr, ret.effects.getStats("indieCr"), true));
                    break;
                ...
                case 3121054: // Preference
                    statups.add(new Triple<>(BuffStats.Stance, ret.effects.getStats("x"), false));
                    statups.add(new Triple<>(BuffStats.IndiePAD, ret.effects.getStats("indiePad"), true));
                    statups.add(new Triple<>(BuffStats.IndieBDR, ret.effects.getStats("y"), true));
                    ret.overTime = true;
                    break;
                case 3221054: // Bullseye
                    statups.add(new Triple<>(BuffStats.IndieCr, ret.effects.getStats("x"), true));
                    statups.add(new Triple<>(BuffStats.IndieCrMaxR, ret.effects.getStats("y"), true));
                    statups.add(new Triple<>(BuffStats.IndieIgnoreMobpdpR, ret.effects.getStats("indieIgnoreMobpdpR"), true));
                    statups.add(new Triple<>(BuffStats.IndieDamR, ret.effects.getStats("indieDamR"), true));
                    statups.add(new Triple<>(BuffStats.BullsEye, 1, false));
                    ret.overTime = true;
                    break;
                case 15120003: // Typhoon
                case 15111022: // Gale
                    statups.add(new Triple<>(BuffStats.DamR, ret.effects.getStats("y"), false));
                    break;
                ...
                case 13101024: // Silf's Ade
                    statups.add(new Triple<>(BuffStats.SoulArrow, 1, false));
                    statups.add(new Triple<>(BuffStats.CriticalBuff, ret.effects.getStats("x"), false));
                    statups.add(new Triple<>(BuffStats.IndiePAD, ret.effects.getStats("indiePad"), true));
                    break;
                ...
                case 13121004: // Wind blasting
                    statups.add(new Triple<>(BuffStats.IndiePADR, ret.effects.getStats("padR"), true));
                    statups.add(new Triple<>(BuffStats.IllusionStep, ret.effects.getStats("prop"), false));
                    break;
                case 13111023: // Albatross
                    ret.absstats = false;
                    statups.add(new Triple<>(BuffStats.Albatross, level, false));
                    statups.add(new Triple<>(BuffStats.IndiePAD, ret.effects.getStats("indiePad"), true));
                    statups.add(new Triple<>(BuffStats.IndieMHP, ret.effects.getStats("indieMhp"), true));
                    statups.add(new Triple<>(BuffStats.IndieBooster, ret.effects.getStats("indieBooster"), true));
                    statups.add(new Triple<>(BuffStats.IndieCr, ret.effects.getStats("indieCr"), true));
                ...
                case 13121054: // Storm Bringer
                    statups.add(new Triple<>(BuffStats.StormBringer, 1, false));
                    break;
                ...
                case 27111004: // Anti Magic Shell
                    statups.add(new Triple<>(BuffStats.AntiMagicShell, 3, false));
                    break;
                case 2201001: // Meditation
                    statups.add(new Triple<>(BuffStats.IndieMAD, ret.effects.getStats("indieMad"), true));
                    break;
                case 2201009: // Chilling Step
                    statups.add(new Triple<>(BuffStats.ChillingStep, 1, false));
                    break;
                case 1321015: // Sacrifice
                    statups.add(new Triple<>(BuffStats.IndieBDR, ret.effects.getStats("indieBDR"), true));
                    break;
                case 21121054: // Combo Unlimited
                    statups.add(new Triple<>(BuffStats.ComboUnlimited, ret.effects.getStats("cooltime"), false));
                    break;
                case 36121053: // Confine Entangle
                    monsterStatus.put(MonsterStatus.STUN, 1);
                    break;
                case 36121054: // Amaranth Generator
                    statups.add(new Triple<>(BuffStats.SurplusSupply, ret.effects.getStats("x"), false));
                    break;
                ...
                case 100000276: // Merchant Corps Special Elixir
                    statups.add(new Triple<>(BuffStats.TimeFastABuff, 1, false));
                    break;
                case 100000277: // Rapid Time (Combat)
                    statups.add(new Triple<>(BuffStats.TimeFastBBuff, 1, false));
                    break;
                case 100001263: // Divine Force
                    statups.add(new Triple<>(BuffStats.ZeroAuraStr, 1, false));
                    statups.add(new Triple<>(BuffStats.IndiePAD, ret.effects.getStats("indiePad"), true));
                    statups.add(new Triple<>(BuffStats.IndieMAD, ret.effects.getStats("indieMad"), true));
                    statups.add(new Triple<>(BuffStats.IndiePDD, ret.effects.getStats("indiePdd"), true));
                    statups.add(new Triple<>(BuffStats.IndieTerR, ret.effects.getStats("indieTerR"), true));
                    statups.add(new Triple<>(BuffStats.IndieAsrR, ret.effects.getStats("indieAsrR"), true));
                    break;
                case 100001264: // Divine Swift
                    statups.add(new Triple<>(BuffStats.ZeroAuraSpd, 1, false));
                    statups.add(new Triple<>(BuffStats.IndieACC, ret.effects.getStats("indieAcc"), true));
                    statups.add(new Triple<>(BuffStats.IndieEVA, ret.effects.getStats("indieEva"), true));
                    statups.add(new Triple<>(BuffStats.IndieJump, ret.effects.getStats("indieJump"), true));
                    statups.add(new Triple<>(BuffStats.IndieSpeed, ret.effects.getStats("indieSpeed"), true));
                ...
                case 100001272: // Time Rewind
                    statups.add(new Triple<>(BuffStats.PreReviveOnce, ret.effects.getStats("x"), false));
                    ret.time = Integer.MAX_VALUE;
                    break;
                ...
                case 80001427: // Rune Buff of Swiftnes
                    statups.add(new Triple<>(BuffStats.IndieJump, ret.effects.getStats("indieJump"), true));
                    statups.add(new Triple<>(BuffStats.IndieSpeed, ret.effects.getStats("indieSpeed"), true));
                    statups.add(new Triple<>(BuffStats.IndieEXP, ret.effects.getStats("indieExp"), true));
                    statups.add(new Triple<>(BuffStats.IndieBooster, ret.effects.getStats("indieBooster"), true));
                    ret.isRune = true;
                    break;
                case 80001428: // Rune Buff of Regeneration
                    statups.add(new Triple<>(BuffStats.DotHealHPPerSecond, 1, false));
                    statups.add(new Triple<>(BuffStats.IndieEXP, ret.effects.getStats("indieExp"), true));
                    statups.add(new Triple<>(BuffStats.IndieAsrR, ret.effects.getStats("indieAsrR"), true));
                    statups.add(new Triple<>(BuffStats.IndieTerR, ret.effects.getStats("indieTerR"), true));
                    ret.isRune = true;
                ...
                case 80001875: // Rune of Transcendenc
                    statups.add(new Triple<>(BuffStats.FixCoolTime, ret.effects.getStats("fixCoolTime"), false));
                    ret.isRune = true;
                    break;
                ...
                case 80001757: // Rune giant buff of the earthquake.
                    statups.add(new Triple<>(BuffStats.IndieJump, ret.effects.getStats("indieJump"), true));
                    statups.add(new Triple<>(BuffStats.IndieSpeed, ret.effects.getStats("indieSpeed"), true));
                    statups.add(new Triple<>(BuffStats.Inflation, ret.effects.getStats("x"), false));
                    ret.isRune = true;
                    break;
                ...
                case 80001218: // Rejuvenation
                    statups.add(new Triple<>(BuffStats.PAD, ret.effects.getStats("x"), true));
                    break;
                case 80001455: // List Ring
                    statups.add(new Triple<>(BuffStats.IndiePADR, ret.effects.getStats("indiePadR"), true));
                    break;
                case 80001456: // Ultimate Ring
                    statups.add(new Triple<>(BuffStats.ACC, 30000, true));
                    statups.add(new Triple<>(BuffStats.MAD, 30000, true));
                    break;
                case 80001457: // Limit ring
                    statups.add(new Triple<>(BuffStats.IndieBDR, ret.effects.getStats("indieBDR"), true));
                    break;
                case 80001458: // Hellcut Ring
                    statups.add(new Triple<>(BuffStats.MHPCutR, -ret.effects.getStats("x"), false));
                    statups.add(new Triple<>(BuffStats.IndieBDR, ret.effects.getStats("indieBDR"), true));
                    break;
                case 80001459: // Manacut ring
                    statups.add(new Triple<>(BuffStats.MMPCutR, -ret.effects.getStats("x"), false));
                    statups.add(new Triple<>(BuffStats.IndieIgnoreMobpdpR, ret.effects.getStats("indieIgnoreMobpdpR"), true));
                    break;
                ...
                case 80001469:
                case 80001470:
                    statups.add(new Triple<>(BuffStats.BuckShot, 1, false));
                    break;
                case 400011000: // Aura Weapon
                    statups.add(new Triple<>(BuffStats.IndieIgnoreMobpdpR, ret.effects.getStats("indieIgnoreMobpdpR"), true));
                    statups.add(new Triple<>(BuffStats.IndiePMdR, ret.effects.getStats("indiePMdR"), true));
                    statups.add(new Triple<>(BuffStats.AuraWeapon, ret.effects.getStats("z"), false));
                    break;
                ...
                case 400021006: // Union Aura
                    statups.add(new Triple<>(BuffStats.BMageAura, level, false));
                    statups.add(new Triple<>(BuffStats.UnionAuraBlow, 1, false));
                    break;
            ...
            if (GameConstants.isBeginnerJob(sourceid / 10000)) {
                switch (sourceid % 10000) {
                    ...
                    case 8003:
                        break;
                        case 8005:
                        statups.add(new Triple<>(BuffStats.AdvancedBless, 1, false));
                        break;
                    case 8006:
                        statups.add(new Triple<>(BuffStats.Inflation, ret.getSkillStats().getStats("x"), false));
                        break;
                    case 8002:
                        int valuez = 0;
                        valuez += ret.effects.getStats("y");
                        valuez |= ret.effects.getStats("x") << 8;
                        statups.add(new Triple<>(BuffStats.SharpEyes, valuez, false));
                        break;
                    case 1026:
                    case 1142:
                        ret.time = 600000;
        ...
        if (ret.overTime && ret.getSummonMovementType() == null && ret.absstats && sourceid != 32121010) {
            addBuffStatPairToListIfNotZero(statups, BuffStats.PAD, ret.effects.getStats("pad"), false);
            addBuffStatPairToListIfNotZero(statups, BuffStats.IndiePAD, ret.effects.getStats("indiePad"), true);
            addBuffStatPairToListIfNotZero(statups, BuffStats.PDD, ret.effects.getStats("pdd"), false);
            addBuffStatPairToListIfNotZero(statups, BuffStats.MAD, ret.effects.getStats("mad"), false);
            addBuffStatPairToListIfNotZero(statups, BuffStats.IndieMAD, ret.effects.getStats("indieMad"), true);
            addBuffStatPairToListIfNotZero(statups, BuffStats.ACC, ret.effects.getStats("acc"), false);
            addBuffStatPairToListIfNotZero(statups, BuffStats.EVA, ret.effects.getStats("eva"), false);
            addBuffStatPairToListIfNotZero(statups, BuffStats.Speed, ret.effects.getStats("speed"), false);
            addBuffStatPairToListIfNotZero(statups, BuffStats.Jump, ret.effects.getStats("jump"), false);
            addBuffStatPairToListIfNotZero(statups, BuffStats.EMHP, ret.effects.getStats("emhp"), false);
            addBuffStatPairToListIfNotZero(statups, BuffStats.EMMP, ret.effects.getStats("emmp"), false);
            addBuffStatPairToListIfNotZero(statups, BuffStats.EMAD, ret.effects.getStats("emad"), false);
            addBuffStatPairToListIfNotZero(statups, BuffStats.EPAD, ret.effects.getStats("epad"), false);
            addBuffStatPairToListIfNotZero(statups, BuffStats.EPDD, ret.effects.getStats("epdd"), false);
            ...
            addBuffStatPairToListIfNotZero(statups, BuffStats.ExpBuffRate, ret.effects.getStats("expBuff"), false);
            addBuffStatPairToListIfNotZero(statups, BuffStats.Inflation, ret.effects.getStats("inflation"), false);
            addBuffStatPairToListIfNotZero(statups, BuffStats.IndieEXP, ret.effects.getStats("indieExp"), true);
        }
        if (ret.isInflation()) {
            addBuffStatPairToListIfNotZero(statups, BuffStats.Inflation, ret.getInflation(), false);
        }
        if (!skill) {
            switch (sourceid) {
                case 2022747:
                    ...
                    ret.time = 600000;
                    ret.overTime = true;
                    statups.add(new Triple<>(BuffStats.PAD, 10, true));
                    statups.add(new Triple<>(BuffStats.MAD, 10, true));
                    statups.add(new Triple<>(BuffStats.RepeatEffect, 1, false));
                    break;
                case 2022746:
                case 2022764:
                    statups.clear();
                    ret.time = 600000;
                    ret.overTime = true;
                    statups.add(new Triple<>(BuffStats.PAD, 5, true));
                    statups.add(new Triple<>(BuffStats.MAD, 5, true));
                    statups.add(new Triple<>(BuffStats.RepeatEffect, 1, false));
                    break;
                case 2022823:
                    statups.clear();
                    ret.time = 600000;
                    ret.overTime = true;
                    statups.add(new Triple<>(BuffStats.PAD, 12, true));
                    statups.add(new Triple<>(BuffStats.MAD, 12, true));
                    statups.add(new Triple<>(BuffStats.RepeatEffect, 1, false));
                    break;
            }
        }
    ...
    private boolean applyTo(final MapleCharacter applyfrom, final MapleCharacter applyto, final boolean primary, Point pos) {
       ...
       if(sourceid ==400021069 ){
           pos = applyfrom.getPosition();
       }
        if( sourceid ==400001004) {
            statups.add(new Triple<>(BuffStats.CombatOrders,effects.getStats("x"),true));

            applyfrom.message("+1 for your skills!");
        }
        ...
        if (isDispel() && makeChanceResult()) {
            ...
            Objects.requireNonNull(SkillFactory.getSkill(21000000)).getEffect(applyto.getSkillLevel(21000000)).applyTo(applyto);
        ...
        if (applyto.isActiveBuffedValue(400021000)) {
            if (mpchange > 0) {
                mpchange += (int) (applyto.getStat().getCurrentMaxMp() * (Objects.requireNonNull(SkillFactory.getSkill(400021000)).getEffect(applyto.getSkillLevel(400021000)).getX() / 100.0D));
            } else if (hpchange > 0) {
                hpchange += (int) (applyto.getStat().getCurrentMaxHp() * (Objects.requireNonNull(SkillFactory.getSkill(400021000)).getEffect(applyto.getSkillLevel(400021000)).getX() / 100.0D));
            }
        }
        int recoveryUP = 0;
        /*
         ...
         */
        final List<Pair<PlayerStatList, Long>> hpmpupdate = new ArrayList<>(2);
        if (hpchange != 0) {
        ...
        if (mpchange != 0) {
            ...
            hpmpupdate.add(new Pair<>(PlayerStatList.MP, (long) stat.getMp()));
        }
        hpmpupdate.add(new Pair<>(PlayerStatList.HP, (long) stat.getHp()));
        ...
        if (isSoulStone()) {
            ...
            final List<MapleCharacter> chrs = new LinkedList<>();
            ...
            statups.clear();
            statups.add(new Triple<>(BuffStats.KinesisPsychicShield, effects.getStats("er"), false));
            statups.add(new Triple<>(BuffStats.Stance, effects.getStats("stanceProp"), false));
            statups.add(new Triple<>(BuffStats.IndiePDD, effects.getStats("indiePdd"), true));
            ...
            statups.add(new Triple<>(BuffStats.Barrier, (int) (applyto.getStat().getMaxHp() * ((float) effects.getStats("x") / 100)), false));
        } else if (sourceid == 80001428) { // Rune of Regeneration
            statups.clear();
            statups.add(new Triple<>(BuffStats.DotHealHPPerSecond, applyfrom.getStat().getMaxHp() / 10, false));
            statups.add(new Triple<>(BuffStats.IndieEXP, effects.getStats("indieExp"), true));
            statups.add(new Triple<>(BuffStats.IndieAsrR, effects.getStats("indieAsrR"), true));
            statups.add(new Triple<>(BuffStats.IndieTerR, effects.getStats("indieTerR"), true));
        } else if (sourceid == 21001008) { // Body pressure
            if (applyfrom.getBuffedValue(BuffStats.BodyPressure) == null) {
                statups.clear();
                statups.add(new Triple<>(BuffStats.BodyPressure, effects.getStats("x"), false));
            } else {
                return false;
            }
            ...
            if (applyfrom.getCooldownLimit(2321008) > 0 && applyfrom.getBuffedValue(BuffStats.KeyDownTimeIgnore, 2321001) == null) {
                SkillStatEffect cooltime = Objects.requireNonNull(Objects.requireNonNull(SkillFactory.getSkill(2321008))).getEffect(applyfrom.getSkillLevel(2321008));
                statups.clear();
                statups.add(new Triple<>(BuffStats.KeyDownTimeIgnore, 1, false));
            ...
            applyfrom.cancelEffect(sourceid == 100001263 ? Objects.requireNonNull(SkillFactory.getSkill(100001264)).getEffect(applyfrom.getSkillLevel(100001264)) : Objects.requireNonNull(SkillFactory.getSkill(100001263)).getEffect(applyfrom.getSkillLevel(100001263)), false, -1);
            ...
            applyfrom.cancelEffect(Objects.requireNonNull(SkillFactory.getSkill(skillid)).getEffect(applyfrom.getSkillLevel(skillid)), false, -1);
        } else if (sourceid == 11121005) { // Soluna Time
            statups.clear();
            statups.add(new Triple<>(BuffStats.GlimmeringTime, 1, false));
            int stateskill = Objects.requireNonNull(applyfrom.getBuffedSkillEffect(BuffStats.PoseType, -1)).getSourceId();
            SkillStatEffect stateeffect = Objects.requireNonNull(SkillFactory.getSkill(11121012)).getEffect(applyfrom.getSkillLevel(sourceid));
            stateeffect.applyTo(applyto);
        } else if (sourceid == 11121011) { // Soluna Time: Polling Moon
            statups.clear();
            statups.add(new Triple<>(BuffStats.IndieCr, effects.getStats("indieCr"), false));
            statups.add(new Triple<>(BuffStats.BuckShot, effects.getStats("x"), false));
        } else if (sourceid == 11121012) {// Soluna Time: Rising Sun
            if (applyto.isActiveBuffedValue(11121012)) {
                return true;
            } else {
                statups.clear();
                statups.add(new Triple<>(BuffStats.IndieBooster, effects.getStats("indieBooster"), true));
                statups.add(new Triple<>(BuffStats.IndieDamR, effects.getStats("indieDamR"), true));
            }
        } else if (sourceid == 5221021) { // Quick draw
            statups.clear();
            statups.add(new Triple<>(BuffStats.QuickDraw, effects.getStats("damR"), false));
            ...
            if (applyto.acaneAim > 0) {
                statups.add(new Triple<>(BuffStats.IndiePMdR, applyto.acaneAim, true));
            }
        }
        ...
        if (sourceid == 5201012) {
            ...
            if (skill != 5201012) {
                Objects.requireNonNull(SkillFactory.getSkill(skill)).getEffect(level).applyTo(applyto, pos);
                return false;
            }
        ...
        if (sourceid >= 33001007 && sourceid <= 33001015) {
            SkillStatEffect eff = Objects.requireNonNull(SkillFactory.getSkill(33001001)).getEffect(applyfrom.getSkillLevel(33001001));
        ...
        if (summonMovementType != null && pos != null) {
            if (sourceid == 400011001) {
                for (Entry<Integer, Pair<Integer, MapleSummon>> summons : applyfrom.getSummons().entrySet()) {
                    if (summons.getValue().left == 400011002) {
                        ...
                        final MapleSummon illusion = new MapleSummon(applyfrom, 400011001, pos, Objects.requireNonNull(SkillFactory.getSkill(400011001)).getEffect(level).getSummonMovementType(), summons.getValue().right.startTime);
                        ...
                        summons.getValue().right.removeSummon(applyfrom.getMap());
                        final MapleSummon illusion = new MapleSummon(applyfrom, 400011002, pos, Objects.requireNonNull(SkillFactory.getSkill(400011002)).getEffect(level).getSummonMovementType(), summons.getValue().right.startTime);
                        illusion.setPosition(pos);
                        applyfrom.getMap().spawnSummon(illusion, true, getDuration());
            ...
            if (sourceid == 20051085) {
                ItemInformation ii = ItemInformation.getInstance();
                Objects.requireNonNull(ii.getItemEffect(2022746)).applyTo(applyto);
            } else if (sourceid == 20051087) {
                ItemInformation ii = ItemInformation.getInstance();
                Objects.requireNonNull(ii.getItemEffect(2022747)).applyTo(applyto);
            }
            ...
            if (sourceid != 35111002 && sourceid != 400051023) {
                ...
                if (sourceid == 400021063) {
              applyfrom.message(" Templar Knight Summon Buffs");
               statups.add(new Triple<>(BuffStats.SUMMON,3,true));
            ...
            if (sourceid == 3111005) { // 
                try {
                    if (applyfrom.getSkillLevel(3120006) > 0) { // Spirit Link
                        Objects.requireNonNull(SkillFactory.getSkill(3120006)).getEffect(applyfrom.getSkillLevel(3120006)).applyTo(applyfrom, applyfrom.getPosition());
                    }
                } catch (Exception e) {
                    System.err.println("[Error] Support Monkey Twins Error");
                    if (!ServerConstants.realese) {
            ...
            if (sourceid == 3211005) { // Freezer
                try {
                    if (applyfrom.getSkillLevel(3220005) > 0) { // Spirit Link
                        Objects.requireNonNull(SkillFactory.getSkill(3220005)).getEffect(applyfrom.getSkillLevel(3220005)).applyTo(applyfrom, applyfrom.getPosition());
                    }
                } catch (Exception e) {
                    System.err.println("[Error] Support Monkey Twins Error");
                    if (!ServerConstants.realese) {
...
//                    }
            ...
            for (CoolDownValueHolder i : applyto.getAllCooldowns()) {
            ...
            int recover = applyto.getStat().getCurrentMaxHp() / 2; // Skill states you heal 50% of your hp so yeah its divided
        ...
        if (isPartyBuff() && (applyfrom.getParty() != null || isGmBuff()) && !isContainsRiding()) {
            ...
            for (final MapleMapObject affectedmo : affecteds) {
                ...
                if (affected != applyfrom && (isGmBuff() || applyfrom.getParty().equals(affected.getParty()))) {
                    ...
                    if (isTimeLeap()) {
                        for (CoolDownValueHolder i : affected.getAllCooldowns()) {
    ...
    public final void applyMonsterBuff(final MapleCharacter applyfrom) {
        final Rectangle bounds = calculateBoundingBox(applyfrom.getPosition(), applyfrom.isFacingLeft());
        final List<MapleMapObject> affected = sourceid == 35111005 ? applyfrom.getMap().getMapObjectsInRange(applyfrom.getPosition(), Double.POSITIVE_INFINITY, Collections.singletonList(MapleMapObjectType.MONSTER)) : applyfrom.getMap().getMapObjectsInRect(bounds, Arrays.asList(MapleMapObjectType.MONSTER));
        int i = 0;
    ...
    public final void cerpentScrew(final MapleCharacter chr) {
        ...
        for (final MapleMapObject object : chr.getMap().getMapObjectsInRect(bounds, Collections.singletonList(MapleMapObjectType.MONSTER))) {
    ...
    public final void applyAtom(final MapleCharacter applyfrom, final int type) {
        final Rectangle bounds = calculateBoundingBox(applyfrom.getPosition(), applyfrom.isFacingLeft());
        int i = 0;
        for (final MapleMapObject object : applyfrom.getMap().getMapObjectsInRect(bounds, Collections.singletonList(MapleMapObjectType.MONSTER))) {
            switch (type) {
                case 10:
    ...
    public final void applyMarkOf(final MapleCharacter applyfrom, final MapleMonster mob, final int kunai, final int skillid) {
        ...
        int i = 0;
        List<Integer> objectId = new ArrayList<>();
    ...
    public final void applyToStrikerStack(final MapleCharacter applyto, final int stack) {
        ...
        if (Objects.requireNonNull(SkillFactory.getSkill(sourceid)).notCancel()) {
            localDuration = Integer.MAX_VALUE;
        }
        if (sourceid == 2201009) {
            localDuration = Integer.MAX_VALUE;
        ...
        if (statups.size() > 0) {
            long overlap_magic = System.currentTimeMillis() % 1000000000;
            ...
            for (Triple<BuffStats, Integer, Boolean> statup : statups) {
                if (statup.getThird()) {
                    if (!stacked.containsKey(statup.getFirst())) {
                        stacked.put(statup.getFirst(), new ArrayList<>());
                    }
            ...
            applyto.getClient().getSession().writeAndFlush(MainPacketCreator.giveBuff((skill ? sourceid : -sourceid), localDuration, statups, this, stacked, Objects.requireNonNull(SkillFactory.getSkill(sourceid)).getAnimationTime(), applyto));
        }
    ...
    public void applySunfireBuff(final MapleCharacter applyto, boolean used, int attackSkill) {
        ...
        final List<Triple<BuffStats, Integer, Boolean>> stat = Collections.singletonList(new Triple<>(BuffStats.Larkness, 1, false));
        final long startTime = System.currentTimeMillis();
    ...
    public void applyEclipseBuff(final MapleCharacter applyto, boolean used, int attackSkill) {
        final int localDuration = 21000000;
        final List<Triple<BuffStats, Integer, Boolean>> stat = Collections.singletonList(new Triple<>(BuffStats.Larkness, 100, false));
        final long startTime = System.currentTimeMillis();
    ...
    public void applyequilibriumBuff(final MapleCharacter applyto, boolean sunfire) {
        for (CoolDownValueHolder m : applyto.getAllCooldowns()) {
        ...
        tools.Timer.EventTimer.getInstance().schedule(() -> {
        ...
        switch (sourceid) {
            ...
            case 400051001: // Lucky dice (the fifth)
            {
                ...
                if (loadeddiceid > 0) {
                    ...
                    final List<Triple<BuffStats, Integer, Boolean>> stat = Collections.singletonList(new Triple<>(BuffStats.Dice, loadeddiceid, false));
                    ...
                    final List<Triple<BuffStats, Integer, Boolean>> stat = Collections.singletonList(new Triple<>(BuffStats.Dice, doublediceid, false));
                    ...
                    final List<Triple<BuffStats, Integer, Boolean>> stat = Collections.singletonList(new Triple<>(BuffStats.Dice, diceid, false));
            ...
            case 5311004: {
                ...
                final List<Triple<BuffStats, Integer, Boolean>> stat = Collections.singletonList(new Triple<>(BuffStats.Roulette, Oakid, false));
                this.statups = stat;
                int overlap_magic = (int) (System.currentTimeMillis() % 1000000000);
                Map<BuffStats, List<StackedSkillEntry>> stacked = applyto.getStackSkills();
                ...
                applyto.getClient().getSession().writeAndFlush(MainPacketCreator.giveBuff(sourceid, localDuration, stat, this, stacked, Objects.requireNonNull(SkillFactory.getSkill(sourceid)).getAnimationTime(), applyto));
                normal = false;
                break;
            }
            case 3101009: { // Quiver cartridges
                ...
                applyto.getClient().getSession().writeAndFlush(UIPacket.showWZEffect("Skill/310.img/skill/3101009/modeStatus/" + (applyto.quivermode - 1) + "/" + (applyto.quivercount[applyto.quivermode - 1]), 1));
                ...
                applyto.getMap().broadcastMessage(applyto, UIPacket.broadcastWZEffect(applyto.getId(), "Skill/310.img/skill/3101009/modeStatus/" + (applyto.quivercount[applyto.quivermode - 1]), 1), applyto.getPosition());
                statups.clear();
                statups.add(new Triple<>(BuffStats.QuiverCatridge, (applyto.quivercount[0] * 10000) + (applyto.quivercount[1] * 100) + (applyto.quivercount[2]), false));
                break;
            }
            case 60001216: // Reshuffle Switch: Defense Mode
            case 60001217: { // Reshuffle Switch: Attack Mode
            ...
            case 61121217: {
                if (applyto.getCooldownLimit(61101002) == 0) {
                    SkillStatEffect effect = Objects.requireNonNull(SkillFactory.getSkill(61101002)).getEffect(applyfrom.getSkillLevel(61101002));
            ...
            case 61121053: { // Final Figure (4th)
                if (applyto.getJob() == 6112 && sourceid == 61111008) {
                    ...
                    Objects.requireNonNull(SkillFactory.getSkill(61120008)).getEffect(applyto.getSkillLevel(61111008)).applyTo(applyto);
                    return;
                }
                ...
                if (applyto.getBuffedValue(BuffStats.StopForceAtomInfo) != null) {
                    ...
                    if (sourceid == 61120008 || sourceid == 61121053) {
                        Objects.requireNonNull(SkillFactory.getSkill(61121217)).getEffect(applyto.getSkillLevel(61120007)).applyTo(applyto);
                    } else {
                        Objects.requireNonNull(SkillFactory.getSkill(61110211)).getEffect(applyto.getSkillLevel(61101002)).applyTo(applyto);
                    }
                }
                break;
            }
            case 400051044: {
                statups.clear();
                statups.add(new Triple<>(BuffStats.stack, applyto.aa, false));
                applyto.send(SecondaryStat.stack(statups, this, applyto));
                break;
            }
            case 400021071: {
                statups.clear();
                statups.add(new Triple<>(BuffStats.LightOrDark, applyto.bb, false));
            ...
            case 2120010:
                statups.clear();
                statups.add(new Triple<>(BuffStats.ArcaneAim, getSkillStats().getStats("x") * applyto.acaneAim, false));
                break;
            case 64120007:
                statups.clear();
                statups.add(new Triple<>(BuffStats.CriticalBuff, 2 * applyto.KADENA_WEAPON_STACK, false));
                statups.add(new Triple<>(BuffStats.EnrageCrDamMin, 5 * applyto.KADENA_WEAPON_STACK, false));
                break;
            case 1211006: // Other than Lightning Charge
            case 1211004:
            case 1221004:
                List<Triple<BuffStats, Integer, Boolean>> statt = Collections.singletonList(new Triple<>(BuffStats.WeaponCharge, 1, false));
                if (applyto.getBuffedValue(BuffStats.WeaponCharge) != null && applyto.getBuffedValue(BuffStats.WeaponCharge, 1211008) != null) {
                    ...
                    Objects.requireNonNull(SkillFactory.getSkill(1211008)).getEffect(applyto.getSkillLevel(1211008)).applyBuffEffect(applyto, applyto, primary, true);
                }
                break;
            case 1211008: // Lightning Charge
                if ((applyto.getBuffedValue(BuffStats.WeaponCharge) != null && applyto.getBuffedValue(BuffStats.WeaponCharge, 1211008) == null) || lightCharge) {
                    this.statups.add(new Triple<>(BuffStats.MAD, getSkillStats().getStats("mad"), false));
                }
                break;
            case 3220005: { // Spirit Link: Freezer
                SkillStatEffect eff_ = Objects.requireNonNull(SkillFactory.getSkill(3211005)).getEffect(applyto.getSkillLevel(3211005));
                time = eff_.time;
            ...
            case 3120006: { // Spirit Link: Phoenix
                SkillStatEffect eff_ = Objects.requireNonNull(SkillFactory.getSkill(3111005)).getEffect(applyto.getSkillLevel(3111005));
                time = eff_.time;
                localDuration = eff_.getDuration();
            ...
            case 20031210: { // Red Judgment-120725 Added
                ...
                final List<Triple<BuffStats, Integer, Boolean>> stat = Collections.singletonList(new Triple<>(BuffStats.DebuffTolerance, rand == 0 ? 5 : 10, false));
            ...
            case 27100003: { // Bless of Darkness
                final List<Triple<BuffStats, Integer, Boolean>> stat = Collections.singletonList(new Triple<>(BuffStats.BlessOfDarkness, (int) applyto.getBlessOfDark(), false));
                applyto.getClient().getSession().writeAndFlush(MainPacketCreator.giveBuff(sourceid, localDuration, stat, this, null, Objects.requireNonNull(SkillFactory.getSkill(sourceid)).getAnimationTime(), applyto));
                normal = false;
                break;
            }
            case 31011001: {
            ...
            case 27121054: {
                ...
                final SkillStatEffect a = Objects.requireNonNull(SkillFactory.getSkill(onoff ? 20040220 : 20040219)).getEffect(1);
            ...
            case 36111003: {
                ...
                statups.add(new Triple<>(BuffStats.StackBuff, applyto.dualBrid, false));
                statups.add(new Triple<>(BuffStats.PDD, 100 * applyto.dualBrid, false));
                break;
            }
            case 27121005: {
                statups.clear();
                applyto.acaneAim = applyto.acaneAim == 0 ? 1 : applyto.acaneAim;
                statups.add(new Triple<>(BuffStats.StackBuff, applyto.acaneAim, false));
            ...
            case 4221054: {
                ...
                statups.add(new Triple<>(BuffStats.FlipTheCoin, applyto.flipTheCoin, false));
                statups.add(new Triple<>(BuffStats.IndieDamR, effects.getStats("indieDamR") * applyto.flipTheCoin, true));
                statups.add(new Triple<>(BuffStats.IndieCr, effects.getStats("x") * applyto.flipTheCoin, true));
                break;
            }
            case 100000276: // Rapid time (detect)
                statups.clear();
                statups.add(new Triple<>(BuffStats.TimeFastABuff, applyto.RapidTimeCount, false));
                break;
            case 100000277: // Rapid Time (Combat)
                statups.clear();
                statups.add(new Triple<>(BuffStats.TimeFastBBuff, applyto.RapidTimeCount, false));
                break;
            case 5121055: {
                statups.clear();
                applyto.unitiyOfPower++;
                statups.add(new Triple<>(BuffStats.UnityOfPower, Math.min(applyto.unitiyOfPower, 4), false));
                break;
            }
            case 1310016: {
                statups.clear();
                statups.add(new Triple<>(BuffStats.IndieCr, effects.getStats("indieCr"), true));
                statups.add(new Triple<>(BuffStats.EPAD, effects.getStats("epad"), false));
                statups.add(new Triple<>(BuffStats.EPDD, effects.getStats("epdd"), false));
                break;
            }
            case 15120003:
            case 15111022: {
                statups.clear();
                statups.add(new Triple<>(BuffStats.DamR, applyto.lightning * getY(), true));
                applyto.lightning = 0;
            ...
            case 1200014: {
                ...
                statups.add(new Triple<>(BuffStats.ElementalCharge, effects.getStats("x"), false));
                //}
                break;
            }
            case 1301013: {
            ...
            case 100001268: { // Jeanne's Protection
                ...
                if (GameConstants.isZero(applyto.getJob())) {
                    statups.add(new Triple<>(BuffStats.BasicStatUp, effects.getStats("x"), false));
                }
            ...
            case 37120012: {
                if (sourceid == 37110009) {
                    if (applyto.getSkillLevel(37120012) > 0) {
                        Objects.requireNonNull(SkillFactory.getSkill(37120012)).getEffect(applyto.getSkillLevel(37120012)).applyTo(applyto);
                        return;
                    }
                }
                applyto.combination++;
            ...
            case 400011053: // Blasted Hammer (big one)
                ...
                if (applyto.getCooldownLimit(400011052) == 0) {
                    SkillStatEffect effect = Objects.requireNonNull(SkillFactory.getSkill(400011052)).getEffect(applyfrom.getSkillLevel(400011052));
            ...
            {
                statups.clear();
                statups.add(new Triple<>(BuffStats.StrikerHyperElectric, effects.getStats("x"), false));
                statups.add(new Triple<>(BuffStats.IndieDamR, effects.getStats("indieDamR") * applyto.lightning, true));
                break;
            }
            case 51001005: {
                int pad = 0;
            ...
            default:
                if (isMonsterRiding()) {
                    ...
                    if (sourceid == 33001001) { // Jaguar Riding
                        ...
                        statups.add(new Triple<>(BuffStats.IndieBooster, -1, true));
                        statups.add(new Triple<>(BuffStats.IndieSpeed, 30, true));
                        statups.add(new Triple<>(BuffStats.IndieMHPR, 30, true));
                        statups.add(new Triple<>(BuffStats.EPDD, effects.getStats("epdd"), false));
                        statups.add(new Triple<>(BuffStats.EPAD, effects.getStats("epad"), false));
                        statups.add(new Triple<>(BuffStats.EMMP, effects.getStats("emmp"), false));
                        statups.add(new Triple<>(BuffStats.EMHP, effects.getStats("emhp"), false));
                        statups.add(new Triple<>(BuffStats.Mechanic, 0, false));
                        statups.add(new Triple<>(BuffStats.MonsterRiding, mountid, false));
                        applyto.setKeyValue2("mountid", mountid);
                        applyto.setKeyValue2("mountskillid", sourceid);
                    } else if (sourceid == 35111003) { // Metal armor: tank
                        statups.add(new Triple<>(BuffStats.MonsterRiding, mountid, false));
                        statups.add(new Triple<>(BuffStats.Mechanic, 1, false));
                        applyto.setKeyValue2("mountid", mountid);
                        applyto.setKeyValue2("mountskillid", sourceid);
                        ...
                        if (Objects.requireNonNull(SkillFactory.getSkill(sourceid)).getDecs().contains("고공")) {
    ...
    public final void applyToBMDeath(MapleCharacter applyto) {
        ...
        if (statups.size() > 0) {
            ...
            applyto.getClient().getSession().writeAndFlush(MainPacketCreator.giveBuff((skill ? sourceid : -sourceid), localDuration, statups, this, stacked, Objects.requireNonNull(Objects.requireNonNull(SkillFactory.getSkill(sourceid))).getAnimationTime(), applyto));
        }
    ...
    public static int parseMountInfo(final MapleCharacter player, final int skillid) {
        ...
        if (SkillFactory.getSkill(skillid) != null) {
            veid = Objects.requireNonNull(SkillFactory.getSkill(skillid)).getVehicleID();
        }
        if (veid > 0) {
            return veid;
        }
        ...
        if (effects.getStats("hp") != 0) {
            if (!skill) {
                ...
                if (applyfrom.getSkillLevel(30000002) > 0) { // Efficiency
                    double percent = SkillFactory.getSkill(30000002).getEffect(1).getX() / 100.0D;
                    hpchange = (int) ((double) hpchange * percent);
                } else if (applyfrom.getSkillLevel(30010002) > 0) { // Efficiency
                    double percent = SkillFactory.getSkill(30010002).getEffect(1).getX() / 100.0D;
                    hpchange = (int) ((double) hpchange * percent);
                }
            } else {
                if (isHeal()) {
    ...
    private int calcMPChange(final MapleCharacter applyfrom, final boolean primary) {
        ...
        if (isTeleport()) {
            if (applyfrom.getBuffedValue(BuffStats.TeleportMasteryOn) != null) {
                mpchange -= applyfrom.getBuffedValue(BuffStats.TeleportMasteryOn);
            }
        }
    ...
    private int alchemistModifyVal(final MapleCharacter chr, int val, final boolean withX) {
        if (!skill) {
            final SkillStatEffect alchemistEffect = getAlchemistEffect(chr);
            if (alchemistEffect != null) {
                return (int) (val * ((alchemistEffect.getY()) / 100.0));
            }
        }
        return val;
    }

    private SkillStatEffect getAlchemistEffect(final MapleCharacter chr) {
        ISkill al;
        switch (chr.getJob()) {
            case 411:
            case 412:
    ...
    public final boolean isMonsterRiding_() {
        return skill && (sourceid == 1004 || sourceid == 10001004 || sourceid == 20001004 || sourceid == 20011004 || sourceid == 80001037 || sourceid == 80001038 || sourceid == 80001039 || sourceid == 80001044 || sourceid >= 80001082 && sourceid <= 80001090 || sourceid == 30011159 || sourceid == 30011109 || sourceid == 33001001 || sourceid == 35001002 || sourceid == 35111003 || sourceid == 22171080);
    }

    public final boolean isMonsterRiding() {
        return (skill && (isMonsterRiding_() || GameConstants.checkMountItem(sourceid) != 0)) || (SkillFactory.getSkill(sourceid) != null && Objects.requireNonNull(SkillFactory.getSkill(sourceid)).getVehicleID() > 0);
    }
    ...
    public final int getCooldown() { // Todo: This means that there is stuff fucking up. Same skillid being used for different return cooldowns.
        if (sourceid == 25101004 || sourceid == 25101003) {
            return 1;
    ...
    public final SummonMovementType getSummonMovementType() {
        switch (sourceid) {
            ...
            case 400011088:
            case 400011002:
                return SummonMovementType.ILLIUM_CRYSTAL;
            case 101100100:
            case 101100101:
         //   case 400011006: // Demon awakening
            //  case 400011007: // Demon awakening
    ...
    public static boolean isFreezStackSkill(int skillid) {
