...
public class InnerAbillity {
    ...
    public InnerSkillValueHolder renewSkill(int rank, int circulator, boolean ultimateCirculatorPos) {
        if (ultimateCirculatorPos && circulator == 2701000) { // Ultimate Circulator
            assert GameConstants.getInnerSkillbyRank(
                    rank) != null;
            ...
            int skillLevel = 0;
            if (random < 38) {
                skillLevel = Randomizer.rand(Objects.requireNonNull(SkillFactory.getSkill(randomSkill)).getMaxLevel() / 2,
                        Objects.requireNonNull(SkillFactory.getSkill(randomSkill)).getMaxLevel());
            } else if (random < 70) {
                skillLevel = Randomizer.rand(Objects.requireNonNull(SkillFactory.getSkill(randomSkill)).getMaxLevel() / 3,
                        Objects.requireNonNull(SkillFactory.getSkill(randomSkill)).getMaxLevel() / 2);
            } else {
                skillLevel = Randomizer.rand(Objects.requireNonNull(SkillFactory.getSkill(randomSkill)).getMaxLevel() / 4,
                        Objects.requireNonNull(SkillFactory.getSkill(randomSkill)).getMaxLevel() / 3);
            }
            return new InnerSkillValueHolder(randomSkill, (byte) skillLevel,
                    Objects.requireNonNull(SkillFactory.getSkill(randomSkill)).getMaxLevel(), (byte) 3);
        }
        ...
        return new InnerSkillValueHolder(randomSkill, (byte) skillLevel,
                Objects.requireNonNull(SkillFactory.getSkill(randomSkill)).getMaxLevel(), (byte) rank);
    }
