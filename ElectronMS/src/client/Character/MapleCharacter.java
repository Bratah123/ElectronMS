...
import client.ItemInventory.Inventory.MapleReward;
...
import client.Skills.PhantomStealSkill.PhantomStealSkill;
import client.Skills.SkillEntry;
import client.Skills.SkillFactory;
import client.Skills.SkillMacro;
import client.Skills.SkillStatEffect;
import client.Skills.StackedSkillEntry;
import client.Skills.VMatrix.VCore;
import client.Stats.Buffs.BuffStats;
import client.Stats.Buffs.BuffStatsValueHolder;
import client.Stats.Diseases.DiseaseStats;
import client.Stats.Diseases.DiseaseValueHolder;
import client.Stats.EquipStats.EnchantEquipStats;
...
import server.CustomEvents.MapleCarnivalParty;
import client.MapleClient;
import client.MapleKeyBinding;
import client.MapleKeyLayout;
import client.MapleProfession.MapleProfession;
import client.MapleProfession.MapleProfessionType;
...
import server.CustomEvents.SafiEvent;
import client.ItemInventory.Items.InventoryManipulator;
import client.ItemInventory.Items.ItemInformation;
import client.ItemInventory.Items.CashItem.MapleCashInventory;
import client.ItemInventory.Items.MapleRing;
...
import server.CustomEvents.MapleCarnivalChallenge;
...
import client.ItemInventory.Inventory.MapleQuickSlot;
...
public class MapleCharacter extends AnimatedHinaMapObjectExtend implements InventoryContainer, Serializable {
    ...
    private transient Map<Integer, Integer> linkMobIds = new HashMap<>();
    private transient Map<Integer, Integer> damageMeter = new ArrayMap<>();
    ...
    public short job;
    public short combo;
    private List<MapleSummon> mines = new ArrayList<>();
    ...
    public Map<ISkill, SkillEntry> matrixSkills = new LinkedHashMap<>();
    private List<InnerSkillValueHolder> innerSkills;
    private transient Map<BuffStats, List<BuffStatsValueHolder>> effects = new LinkedHashMap<>();
    private final transient Map<BuffStats, List<StackedSkillEntry>> stackedEffects = new LinkedHashMap<>();
    ...
    private Map<String, Integer> CustomValues3 = new HashMap<>();
    private transient Map<Integer, Pair<Integer, MapleSummon>> summons;
    private transient Map<Integer, CoolDownValueHolder> coolDowns = new LinkedHashMap<>(50);
    ...
    private transient Map<Integer, Integer> wpForce = new HashMap<>();
    private int wp = 0;
    ...
    private List<Pair<EnchantEquipStats, Integer>> enchant = new ArrayList<>();
    ...
    private Map<Integer, Pair<Integer, Integer>> link_skill = new HashMap<>();
    ...
    public static MapleCharacter ReconstructChr(final ChracterTransfer ct, final MapleClient client,
    ...
    public static MapleCharacter loadCharFromDB(int charid, MapleClient client, boolean channelserver) {
        ...
        try {
            ...
            if (channelserver) {
                ...
                while (true) {
                    //    System.out.println("[Notice] 캐릭터 인벤토리리 불러오는중"); // (it says: Loading character inventory)
                    try {
                ...
                rs = ps.executeQuery();
                while (rs.next()) {
                    ret.extendedSlots.add(rs.getInt("uniqueid"));
                }
                rs.close();
                ps.close();
    ...
    public static void saveNewCharToDB(final MapleCharacter chr) {
        ...
        try {
            ...
            assert JobType.getById(chr.job) != null;
    ...
    public void SkillSaveToDB(boolean dc) {
        ...
        try {
            ...
            if (getAllCooldowns().size() > 0) {
                for (final CoolDownValueHolder cooling : getAllCooldowns()) {
                    ps = con.prepareStatement(
    ...
    public void spawnWillMob1(long t, boolean istwo) {
        this.willtimer1 = tools.Timer.MapTimer.getInstance().register(() -> {
            if (willfirst != 0) {
                if (!istwo) {
    ...
    public void spawnWillMob2(long t, boolean istwo) {
        this.willtimer2 = tools.Timer.MapTimer.getInstance().register(() -> {
            if (willfirst2 != 0) {
                if (!istwo) {
                    client.getChannelServer().getMapFactory().getMap(450008250)
    ...
    public boolean isActiveBuffedValue(int skillid) {
        LinkedList<BuffStatsValueHolder> allBuffs = new LinkedList<>();
    ...
    public final void gainItem(final int id, final short quantity, final boolean randomStats, final long period,
        ...
        if (quantity >= 0) {
            ...
            if (type.equals(MapleInventoryType.EQUIP) && !GameConstants.isThrowingStar(id)
                ...
                item.setGMLog(gm_log + "on" + CurrentTime.getAllCurrentTime());
                InventoryManipulator.addbyItem(client, item);
            } else {
                InventoryManipulator.addById(client, id, quantity, "", null, period,
                        "Items obtained by the gainItem script called by" + getName() + "on" + CurrentTime.getAllCurrentTime());
            }
        } else {
    ...
    public void startMapTimeLimitTask(int time, final MapleMap to) {
        ...
        time *= 1000;
        mapTimeLimitTask = EtcTimer.getInstance().register(() -> changeMap(to, to.getPortal(0)), time, time);
    }
        ...
        if (getJob() == 510 || getJob() == 511 || getJob() == 512) {
            if (getSkillLevel(5100013) > 0) {
                if (InduerenseTask == null) {
                    final SkillStatEffect eff = Objects.requireNonNull(SkillFactory.getSkill(5100013)).getEffect(getSkillLevel(5100013));
                    Runnable r = () -> {
    ...
    public void checkSelfRecovery() {
        if (getSkillLevel(GameConstants.getRecoverySkill(getJob() / 10)) > 0) {
            if (selfRecoveryTask == null) {
                final SkillStatEffect eff = Objects.requireNonNull(SkillFactory.getSkill(GameConstants.getRecoverySkill(getJob() / 10)))
        ...
        if (GameConstants.isMercedes(getJob())) {
            if (getSkillLevel(20020109) > 0) {
                if (mercedesRecoveryTask == null) {
                    final SkillStatEffect eff = Objects.requireNonNull(SkillFactory.getSkill(20020109)).getEffect(getSkillLevel(20020109));
                    Runnable r = () -> {
                        int recoverhp = (int) ((eff.getX() / 100.0D) * getStat().getCurrentMaxHp());
                        int recovermp = (int) ((eff.getX() / 100.0D) * getStat().getCurrentMaxMp());
                        if (isAlive()) {
    ...
    public final void startInfinityRegen(final SkillStatEffect eff) {
        BuffTimer tMan = BuffTimer.getInstance();
        infinityReGenTask = tMan.register(() -> {
    ...
    public List<BuffStats> getBuffStats(final SkillStatEffect effect, final long startTime) {
        final List<BuffStats> bstats = new ArrayList<>();
    ...
    public void cancelEffect(final SkillStatEffect effect, final boolean overwrite, final long startTime) {
        try {
            ...
            effectsToCancel = new ArrayList<>(buffstats.size());
                        ...
                        for (final BuffStatsValueHolder bsvh : mbsvh) {
                            if (bsvh.effect.getSourceId() == effect.getSourceId()) {
                                ...
                                if (effect.getSourceId() == 152121005 && !overwrite) {
                                    ...
                                    SkillStatEffect a = Objects.requireNonNull(SkillFactory.getSkill(152001003)).getEffect(getSkillLevel(152001003));
                                    SkillStatEffect a1 = Objects.requireNonNull(SkillFactory.getSkill(152101008)).getEffect(getSkillLevel(152101008));
                                    a.applyTo(this);
                                    a1.applyTo(this);
                                }
            ...
            if (effect.getSourceId() == 152111003 && !overwrite) {
                SkillStatEffect a = Objects.requireNonNull(SkillFactory.getSkill(152101000)).getEffect(getSkillLevel(152101000));
                a.applyTo(this);
                SkillStatEffect a1 = Objects.requireNonNull(SkillFactory.getSkill(152101008)).getEffect(getSkillLevel(152101008)); // Machina
                a1.applyTo(this);
            }
            ...
            if (effect.getSourceId() == 400051033 && skillisCooling(400051033)
                ...
                if (eqpWeapon != null) {
                    ...
                    send(MainPacketCreator.giveBuff(400051033, localDuration, statups, effect, stacked,
                            Objects.requireNonNull(SkillFactory.getSkill(400051033)).getAnimationTime(), this));
    ...
    public void cancelBuffStats(int skill, BuffStats... statt) {
        final BuffStats[] buffStatList = statt;
    ...
    public void cancelEffectFromBuffStat(BuffStats stat, int skill) {
        final List<SkillStatEffect> toCancelEffects = new ArrayList<>();
        if (effects.containsKey(stat)) {
    ...
    public void dispelItem(int skillid) {
        final LinkedList<BuffStatsValueHolder> allBuffs = new LinkedList<>();
    ...
    public List<MapleBuffValueHolder> getAllBuffs() { // 후원 버프 (Donor (?) buffs)
        List<MapleBuffValueHolder> ret = new ArrayList<>();
        LinkedList<BuffStatsValueHolder> allBuffs = new LinkedList<>();
        for (List<BuffStatsValueHolder> holders : effects.values()) {
    ...
    public void enforceMaxHpMp() {
        try {
            List<Pair<PlayerStatList, Long>> statups = new ArrayList<>(2);
            stats.setMp(stats.getMp());
            statups.add(new Pair<>(PlayerStatList.MP, (long) stats.getMp()));
            stats.setHp(stats.getHp(), this);
            statups.add(new Pair<>(PlayerStatList.HP, (long) stats.getHp()));
            if (statups.size() > 0) {
    ...
    public void changeMapBanish(final int mapid, final String portal, final String msg) {
        ...
        changeMap(map, Objects.requireNonNull(map.getPortal(portal)));
    }
    ...
    private void changeMapInternal(MapleMap to, Point pos, byte[] warpPacket) {
        ...
        if (getQuickMoved()) {
            client.send(MainPacketCreator.getQuickMove(new ArrayList<>()));
            setQuickMoved(false);
        }
        ...
        if (getBuffedValue(BuffStats.PickPocket) != null) {
            this.mesoCount = 0;
            Objects.requireNonNull(SkillFactory.getSkill(4211003)).getEffect(this.getSkillLevel(4211003)).applyTo(this);
        }
    ...
    public void resetStats(final int str, final int dex, final int int_, final int luk) {
        List<Pair<PlayerStatList, Long>> stats = new ArrayList<>(2);
        ...
        stats.add(new Pair<>(PlayerStatList.STR, (long) str));
        stats.add(new Pair<>(PlayerStatList.DEX, (long) dex));
        stats.add(new Pair<>(PlayerStatList.INT, (long) int_));
        stats.add(new Pair<>(PlayerStatList.LUK, (long) luk));
        stats.add(new Pair<>(PlayerStatList.AVAILABLEAP, (long) total));
        client.getSession().writeAndFlush(MainPacketCreator.updatePlayerStats(stats, false, chr.getJob(), this));
    }

    public void startHurtHp() {
        hpDecreaseTask = EtcTimer.getInstance().register(() -> {
    ...
    public void changeJob(int newJob) {
        ...
        List<Pair<PlayerStatList, Long>> statup = new ArrayList<>(2);
        ...
        if (GameConstants.isKOC(getJob()) && getLevel() >= 100) {
            if (getSkillLevel(Integer.parseInt(getJob() + "1000")) <= 0) {
                teachSkill(Integer.parseInt(getJob() + "1000"), (byte) 0,
                        Objects.requireNonNull(SkillFactory.getSkill(Integer.parseInt(getJob() + "1000"))).getMaxLevel());
            }
        }
        if (!isGM()) {
            switch (job % 1000) {
        ...
        stats.setMp(stats.getCurrentMaxMp());
        stats.recalcLocalStats(this);
        statup.add(new Pair<>(PlayerStatList.MAXHP, (long) maxhp));
        statup.add(new Pair<>(PlayerStatList.MAXMP, (long) maxmp));
    ...
    public void gainSP(int sp) {
        ...
        if (sp > 0) {
            client.getSession().writeAndFlush(UIPacket.getSPMsg((byte) sp, job));
        }
    }
    ...
    public void spawnPet(byte slot, boolean lead, boolean broadcast) {
        ...
        if (pet != null) {
            ...
            if (getPetIndex(pet) != -1) {
                ...
                pet.setPos(pos);
                try {
                    pet.setFh(Objects.requireNonNull(getMap().getFootholds().findMaple(pos)).getId());
    ...
    public void changeSkillLevel_Skip(ISkill skil, int skilLevel, byte masterLevel) {
        final Map<ISkill, SkillEntry> enry = new HashMap<>(1);
    ...
    public void addMPHP(int hpDiff, int mpDiff) {
        List<Pair<PlayerStatList, Long>> statups = new ArrayList<>();
    ...
    public void updateSingleStat(PlayerStatList stat, long newval, boolean itemReaction) {
        List<Pair<PlayerStatList, Long>> statups = new ArrayList<>();
        statups.add(new Pair<>(PlayerStatList.AVAILABLESP, newval));
        ...
        Pair<PlayerStatList, Long> statpair = new Pair<>(stat, newval);
        client.getSession().writeAndFlush(
    ...
    public void gainExpMonster(final int gain, final boolean show, final boolean white, final byte pty, final int Class_Bonus_EXP) {
        ...
        if (getSkillLevel(20021110) > 0) { // Elf Blessing
            PsdBonusExpRate += (int) ((gain * rate)
                    * (Objects.requireNonNull(SkillFactory.getSkill(20021110)).getEffect(getSkillLevel(20021110)).getStat("expR") / 100.0D));
            total += PsdBonusExpRate;
        }
        else if (getSkillLevel(20021110) > 1) { // Elf Blessing 2
                PsdBonusExpRate += (int) ((gain * rate)
                    * (Objects.requireNonNull(SkillFactory.getSkill(20021110)).getEffect(getSkillLevel(20021110)).getStat("expR") / 50.0D));
            total += PsdBonusExpRate;
            ...
            PsdBonusExpRate += (int) ((gain * rate)
                    * (Objects.requireNonNull(SkillFactory.getSkill(20021110)).getEffect(getSkillLevel(20021110)).getStat("expR") / 5.0D));
            total += PsdBonusExpRate;
        ...
        if (getSkillLevel(80001040) > 0) { // link
            PsdBonusExpRate += (int) ((gain * rate)
                    * (Objects.requireNonNull(SkillFactory.getSkill(80001040)).getEffect(getSkillLevel(80001040)).getStat("expR") / 100.0D));
            total += PsdBonusExpRate;
        }
    ...
    public Map<Byte, Integer> getEquips(boolean paramBoolean) {
        Map<Byte, Integer> localHashMap = new HashMap<>();
        for (IItem item : this.getInventory(MapleInventoryType.EQUIPPED).list()) {
            ...
            if ((paramBoolean) && (((Equip) item).getPotential7() != 0)) {
                ...
                i2 = Integer.parseInt(str + ((Equip) item).getPotential7());
            }
    ...
    public final void expirationTask() {
        ...
        for (final MapleInventory inv : inventory) {
            final List<IItem> toberemove = new ArrayList<>();
        ...
        final List<Integer> toRemoveSkills = new ArrayList<>();
    ...
    public void controlMonster(MapleMonster monster, boolean aggro) {
        ...
        client.getSession().writeAndFlush(MobPacket.controlMonster(monster, false,
                monster.getId() != 8220028 && aggro, GameConstants.isAswanMap(mapid)));
    }
    ...
    public byte getSkillLevel(final ISkill skill) {
        ...
        if (Objects.requireNonNull(SkillFactory.getSkill(skill.getId())).getMaxLevel() == 1) {
            return 1;
        }
    ...
    public int getSkillLevel(int skill) {
        ...
        if (Objects.requireNonNull(SkillFactory.getSkill(skill)).getMaxLevel() == 1) {
            return 1;
        }
        ...
        byte skillLevel = (byte) Math.min(ret.skillevel + getStat().getIncAllSkill(),
                Objects.requireNonNull(SkillFactory.getSkill(skill)).getMaxLevel());
        if (Objects.requireNonNull(SkillFactory.getSkill(skill)).canCombatOrdered() && getBuffedValue(BuffStats.CombatOrders) != null) {
            skillLevel += getBuffedValue(BuffStats.CombatOrders).byteValue();
        }
        return skillLevel;
    }
    ...
    public void levelUp() {
       ...
       if (getStat().getStr() < 32000 || // If smaller than
        ...
        if (level >= 900 && !isGM()) {
            ...
            sb.append(getName());
            sb.append(" Level ").append(level).append("You have achieved! Please congratulate him everyone!");
        ...
        final List<Pair<PlayerStatList, Long>> statup = new ArrayList<>(8);
        statup.add(new Pair<>(PlayerStatList.MAXHP, (long) maxhp));
        statup.add(new Pair<>(PlayerStatList.MAXMP, (long) maxmp));
        statup.add(new Pair<>(PlayerStatList.HP, (long) getStat().getCurrentMaxHp()));
        statup.add(new Pair<>(PlayerStatList.MP, (long) getStat().getCurrentMaxMp()));
        statup.add(new Pair<>(PlayerStatList.EXP, exp));
        statup.add(new Pair<>(PlayerStatList.LEVEL, (long) level));
        ...
        statup.add(new Pair<>(PlayerStatList.AVAILABLEAP, (long) remainingAp));
    ...
    public void updateLevel(int levelp){
        exp = 0;
        setLevel(levelp);
        final List<Pair<PlayerStatList, Long>> statup = new ArrayList<>(2);
        statup.add(new Pair<>(PlayerStatList.EXP, exp));
        statup.add(new Pair<>(PlayerStatList.LEVEL, (long) level));
    ...
    public void changeKeybinding(int key, MapleKeyBinding keybinding) {
        if (keybinding.getType() != 0) {
            keylayout.Layout().put(key, keybinding);
        } else {
    ...
    public void disableDoor() {
        canDoor = false;
        BuffTimer.getInstance().schedule(() -> canDoor = true, 5000);
    }
    ...
    public void changeCooldown(int skillid, long changetime) {
        if (skillisCooling(skillid)) {
            for (CoolDownValueHolder mcdvh : getAllCooldowns()) {
    ...
    public List<CoolDownValueHolder> getAllCooldowns() {
        List<CoolDownValueHolder> ret = new ArrayList<>();
        for (CoolDownValueHolder mcdvh : coolDowns.values()) {
            ret.add(new CoolDownValueHolder(mcdvh.skillId, mcdvh.startTime, mcdvh.length));
        }
        return ret;
    }

    public final List<MapleDiseaseValueHolder> getAllDiseases() {
        final List<MapleDiseaseValueHolder> ret = new ArrayList<>(5);
    ...
    public void elementalAdep_fp() {
        ...
        Objects.requireNonNull(getBuffedSkillEffect(BuffStats.StackBuff, 2111011)).applyTo(this, getPosition());
        if (elementalAdep == 0) {
            elementalAdep = -1;
    ...
    public void SpiritGuard() {
        SpiritGuard--;
        Objects.requireNonNull(getBuffedSkillEffect(BuffStats.SpiritGuard, 25121209)).applyTo(this);
        if (SpiritGuard == 0) {
    ...
    public void giveDebuff(final DiseaseStats disease, MobSkill skill) {
        ...
        if (!hasDisease(disease) && diseases.size() < 2) {
            ...
            BuffTimer.getInstance().schedule(() -> dispelDebuff(disease), skill.getDuration());
    ...
    public final void giveSilentDebuff(final List<MapleDiseaseValueHolder> ld) {
        if (ld != null) {
            for (final MapleDiseaseValueHolder disease : ld) {
                ...
                BuffTimer.getInstance().schedule(() -> dispelDebuff(disease.disease), (disease.length + disease.startTime) - System.currentTimeMillis());
    ...
    public void unlockMaxDamage() {
        Objects.requireNonNull(SkillFactory.getSkill(1105)).getEffect(1).applyTo(this);
        if (isBuckshot()) {
            BuckShot();
        }
        if (isStance()) {
    ...
    public void BuckShot() {
        ...
        Map<BuffStats, List<StackedSkillEntry>> stacked = getStackSkills();
        for (Triple<BuffStats, Integer, Boolean> statup : Collections.singletonList(new Triple<>(BuffStats.BuckShot, 20, false))) {
            if (statup.getThird()) {
                if (!stacked.containsKey(statup.getFirst())) {
                    stacked.put(statup.getFirst(), new ArrayList<>());
                }
        ...
        getClient().getSession().writeAndFlush(MainPacketCreator.giveBuff(5321054, Integer.MAX_VALUE, Collections.singletonList(new Triple<>(BuffStats.BuckShot, 20, false)), SkillFactory.getSkill(5321054).getEffect(getSkillLevel(5321054)), null, SkillFactory.getSkill(5321054).getAnimationTime(), getClient().getPlayer()));
    }
    ...
    public void Stance() {
        int overlap_magic = (int) (System.currentTimeMillis() % 1000000000);
        Map<BuffStats, List<StackedSkillEntry>> stacked = getStackSkills();
        for (Triple<BuffStats, Integer, Boolean> statup : Collections.singletonList(new Triple<>(BuffStats.Stance, 1, false))) {
            if (statup.getThird()) {
                if (!stacked.containsKey(statup.getFirst())) {
                    stacked.put(statup.getFirst(), new ArrayList<>());
                }
        ...
        getClient().getSession().writeAndFlush(MainPacketCreator.giveBuff(80001140, Integer.MAX_VALUE, Collections.singletonList(new Triple<>(BuffStats.Stance, 1, false)), SkillFactory.getSkill(80001140).getEffect(getSkillLevel(80001140)), null, SkillFactory.getSkill(80001140).getAnimationTime(), getClient().getPlayer()));
    }
    ...
    public void HolySymbol() {
        int overlap_magic = (int) (System.currentTimeMillis() % 1000000000);
        Map<BuffStats, List<StackedSkillEntry>> stacked = getStackSkills();
        for (Triple<BuffStats, Integer, Boolean> statup : Collections.singletonList(new Triple<>(BuffStats.HolySymbol, 20, false))) {
            if (statup.getThird()) {
                if (!stacked.containsKey(statup.getFirst())) {
                    stacked.put(statup.getFirst(), new ArrayList<>());
                }
        ...
        getClient().getSession().writeAndFlush(MainPacketCreator.giveBuff(2311003, Integer.MAX_VALUE, Collections.singletonList(new Triple<>(BuffStats.HolySymbol, 20, false)), SkillFactory.getSkill(2311003).getEffect(getSkillLevel(2311003)), null, SkillFactory.getSkill(2311003).getAnimationTime(), getClient().getPlayer()));
    }
    ...
    public void SharpEyes() {
        int overlap_magic = (int) (System.currentTimeMillis() % 1000000000);
        Map<BuffStats, List<StackedSkillEntry>> stacked = getStackSkills();
        for (Triple<BuffStats, Integer, Boolean> statup : Collections.singletonList(new Triple<>(BuffStats.SharpEyes, 15, false))) {
            if (statup.getThird()) {
                if (!stacked.containsKey(statup.getFirst())) {
                    stacked.put(statup.getFirst(), new ArrayList<>());
                }
        ...
        getClient().getSession().writeAndFlush(MainPacketCreator.giveBuff(3121002, Integer.MAX_VALUE, Collections.singletonList(new Triple<>(BuffStats.SharpEyes, 15, false)), SkillFactory.getSkill(3121002).getEffect(getSkillLevel(3121002)), null, SkillFactory.getSkill(3121002).getAnimationTime(), getClient().getPlayer()));
    }
    ...
    public void PartyBooster() {
        int overlap_magic = (int) (System.currentTimeMillis() % 1000000000);
        Map<BuffStats, List<StackedSkillEntry>> stacked = getStackSkills();
        for (Triple<BuffStats, Integer, Boolean> statup : Collections.singletonList(new Triple<>(BuffStats.PartyBooster, 20, false))) {
            if (statup.getThird()) {
                if (!stacked.containsKey(statup.getFirst())) {
                    stacked.put(statup.getFirst(), new ArrayList<>());
                }
        ...
        getClient().getSession().writeAndFlush(MainPacketCreator.giveBuff(5121009, Integer.MAX_VALUE, Collections.singletonList(new Triple<>(BuffStats.PartyBooster, 20, false)), SkillFactory.getSkill(5121009).getEffect(getSkillLevel(5121009)), null, SkillFactory.getSkill(5121009).getAnimationTime(), getClient().getPlayer()));
    }
    ...
    public void ShadowPartner() {
        int overlap_magic = (int) (System.currentTimeMillis() % 1000000000);
        Map<BuffStats, List<StackedSkillEntry>> stacked = getStackSkills();
        for (Triple<BuffStats, Integer, Boolean> statup : Collections.singletonList(new Triple<>(BuffStats.ShadowPartner, 1, false))) {
            if (statup.getThird()) {
                if (!stacked.containsKey(statup.getFirst())) {
                    stacked.put(statup.getFirst(), new ArrayList<>());
                }
        ...
        getClient().getSession().writeAndFlush(MainPacketCreator.giveBuff(4331002, Integer.MAX_VALUE, Collections.singletonList(new Triple<>(BuffStats.ShadowPartner, 1, false)), SkillFactory.getSkill(4331002).getEffect(getSkillLevel(4331002)), null, SkillFactory.getSkill(4331002).getAnimationTime(), getClient().getPlayer()));
    }
    ...
    public void MagicArrow() {
        int overlap_magic = (int) (System.currentTimeMillis() % 1000000000);
        Map<BuffStats, List<StackedSkillEntry>> stacked = getStackSkills();
        for (Triple<BuffStats, Integer, Boolean> statup : Collections.singletonList(new Triple<>(BuffStats.PvPRaceEffect, 1, false))) {
            if (statup.getThird()) {
                if (!stacked.containsKey(statup.getFirst())) {
                    stacked.put(statup.getFirst(), new ArrayList<>());
                }
        ...
        getClient().getSession().writeAndFlush(MainPacketCreator.giveBuff(3100010, Integer.MAX_VALUE, Collections.singletonList(new Triple<>(BuffStats.PvPRaceEffect, 1, false)), SkillFactory.getSkill(3100010).getEffect(getSkillLevel(3100010)), null, SkillFactory.getSkill(3100010).getAnimationTime(), getClient().getPlayer()));
    }
    ...
    public void TrifleWorm() {
        int overlap_magic = (int) (System.currentTimeMillis() % 1000000000);
        Map<BuffStats, List<StackedSkillEntry>> stacked = getStackSkills();
        for (Triple<BuffStats, Integer, Boolean> statup : Collections.singletonList(new Triple<>(BuffStats.TriflingWhimOnOff, 1, false))) {
            if (statup.getThird()) {
                if (!stacked.containsKey(statup.getFirst())) {
                    stacked.put(statup.getFirst(), new ArrayList<>());
                }
        ...
        getClient().getSession().writeAndFlush(MainPacketCreator.giveBuff(13101022, Integer.MAX_VALUE, Collections.singletonList(new Triple<>(BuffStats.TriflingWhimOnOff, 1, false)), SkillFactory.getSkill(13101022).getEffect(getSkillLevel(13101022)), null, SkillFactory.getSkill(13101022).getAnimationTime(), getClient().getPlayer()));
    }
    ...
    public void Noir() {
        int overlap_magic = (int) (System.currentTimeMillis() % 1000000000);
        Map<BuffStats, List<StackedSkillEntry>> stacked = getStackSkills();
        for (Triple<BuffStats, Integer, Boolean> statup : Collections.singletonList(new Triple<>(BuffStats.PvPInvincible, 1, false))) {
            if (statup.getThird()) {
                if (!stacked.containsKey(statup.getFirst())) {
                    stacked.put(statup.getFirst(), new ArrayList<>());
                }
        ...
        getClient().getSession().writeAndFlush(MainPacketCreator.giveBuff(24120002, Integer.MAX_VALUE, Collections.singletonList(new Triple<>(BuffStats.PvPInvincible, 1, false)), SkillFactory.getSkill(24120002).getEffect(getSkillLevel(24120002)), null, SkillFactory.getSkill(24120002).getAnimationTime(), getClient().getPlayer()));
    ...
    public void Bling() {
        ...
        getClient().getSession().writeAndFlush(MainPacketCreator.giveBuff(24100003, Integer.MAX_VALUE, Collections.singletonList(new Triple<>(BuffStats.PvPRaceEffect, 1, false)), SkillFactory.getSkill(24100003).getEffect(getSkillLevel(24100003)), null, SkillFactory.getSkill(24120002).getAnimationTime(), getClient().getPlayer()));
    ...
    public void Kinesis() {
        int overlap_magic = (int) (System.currentTimeMillis() % 1000000000);
        Map<BuffStats, List<StackedSkillEntry>> stacked = getStackSkills();
        for (Triple<BuffStats, Integer, Boolean> statup : Collections.singletonList(new Triple<>(BuffStats.PvPScoreBonus, 1, false))) {
            if (statup.getThird()) {
                if (!stacked.containsKey(statup.getFirst())) {
                    stacked.put(statup.getFirst(), new ArrayList<>());
                }
        ...
        getClient().getSession().writeAndFlush(MainPacketCreator.giveBuff(142110011, Integer.MAX_VALUE, Collections.singletonList(new Triple<>(BuffStats.PvPScoreBonus, 1, false)), SkillFactory.getSkill(142110011).getEffect(getSkillLevel(142110011)), null, SkillFactory.getSkill(142110011).getAnimationTime(), getClient().getPlayer()));
    ...
    public void cancelRapidTime(byte type) {
        if (type == 1) {
            this.rapidtimer1 = tools.Timer.MapTimer.getInstance().schedule(() -> client.getPlayer().changeSkillLevel(100000277, (byte) 0, (byte) 0), 20000L);
        } else if (type == 2) {
            this.rapidtimer2 = tools.Timer.MapTimer.getInstance().schedule(() -> client.getPlayer().changeSkillLevel(100000277, (byte) 0, (byte) 0), 20000L);
        }
    }
    ...
    public void updateCombo(short combo, long curr) { //아란 콤보 (Aran Combo)
        if (combo > 1000) {
            combo = 1000;
        }
        ...
        switch (combo) {
            case 50:
                Objects.requireNonNull(SkillFactory.getSkill(21000000)).getEffect(getSkillLevel(21000000)).applyTo(this);
                setCombo((short) 50);
                break;
            case 100:
                Objects.requireNonNull(SkillFactory.getSkill(21000000)).getEffect(getSkillLevel(21000000)).applyTo(this);
            ...
            default:
                if (combo % 1000 == 0) {
                    Objects.requireNonNull(SkillFactory.getSkill(21110016)).getEffect(getSkillLevel(21110016)).applyTo(this);
                }
                break;
        }
    }
    ...
    public void useComboSkill(int skill) {
        if (getBuffedValue(BuffStats.ComboUnlimited) == null && getSkillLevel(skill) > 0) {
            SkillStatEffect effect = Objects.requireNonNull(SkillFactory.getSkill(skill)).getEffect(getSkillLevel(skill));
    ...
    public void checkSoulState(boolean useskill, int skillid) {
        SkillStatEffect skill = Objects.requireNonNull(SkillFactory.getSkill(skillid)).getEffect(getSkillLevel(skillid));
        if (useskill) {
    ...
    public void setSoulCount(int soulcount) {
        this.Soul = (Math.min(soulcount, 1000));
    }
    ...
    public void saveKeyValues() {
        try {
            ...
            System.err.println("[Error] Failed to save custom values.");
            if (!ServerConstants.realese) {
                e.printStackTrace();
            }
        }
        ...
        try {
            ...
            for (int i = 1; i <= 5; ++i) {
                for (StealSkillEntry sse : steelskills.getSkillEntrys(i)) {
                    ...
                    ps.setInt(4, StealSkillEntry.getJobIndex(sse.getSkillId()));
    ...
    public void send(Object ob) {
        getClient().getSession().writeAndFlush(ob);
    }
    ...
    public String printRewardsSaves() {
        ...
        } // KOOKIIE deleted the counting particle '개' from the line
        return str;
    }
    ...
    public void addInnerExp(int amount) {
        setInnerExp(Math.min(getInnerExp() + amount, Integer.MAX_VALUE));
    ...
    public void zeroSkillLevel() {
        ...
        for (int[] ints : skill) {
            if (level >= ints[1]) {
                if (getSkillLevel(ints[0]) <= 0) {
                    changeSkillLevel(ints[0], Objects.requireNonNull(SkillFactory.getSkill(ints[0])).getMaxLevel(), Objects.requireNonNull(SkillFactory.getSkill(ints[0])).getMaxLevel());
                }
            }
        }
    }
    ...
    public final void getSunfireBuffedValue(int skillid, int attackSkill, Integer Gauge) {
        ...
        if (skilllevel > 0) {
            ...
            if (getMinusOfGlassCTS_Morph() <= 1 && getBuffedValue(BuffStats.Larkness) == 20040216) {
                final SkillStatEffect Equili = Objects.requireNonNull(SkillFactory.getSkill(20040219)).getEffect(1);
                Equili.applyequilibriumBuff(this, true);
            }
        } else {
            assert sunfireid != null;
            this.changeSkillLevel(sunfireid, sunfireid.getMaxLevel(), (byte) sunfireid.getMasterLevel());
    ...
    public final void getEclipseBuffedValue(int skillid, int attackSkill, Integer Gauge) {
        ...
        if (skilllevel > 0) {
            ...
            if (getPlusOfGlassCTS_Morph() >= 10000 && getBuffedValue(BuffStats.Larkness) == 20040217) {
                final SkillStatEffect Equili = Objects.requireNonNull(SkillFactory.getSkill(20040220)).getEffect(1);
                Equili.applyequilibriumBuff(this, false);
            }
        } else {
            assert eclipseid != null;
            this.changeSkillLevel(eclipseid, eclipseid.getMaxLevel(), (byte) eclipseid.getMasterLevel());
    ...
    public static void crossChannelWarp(final MapleClient c, final int map, final byte channel) {
        String IP;
    ...
    public void giveBulletGauge(int skillid, boolean CloseR) {
        ...
        switch (skillid) {
            ...
            case 37000012:
            case 37000013:
                effect = Objects.requireNonNull(SkillFactory.getSkill(skillid)).getEffect(this.getSkillLevel(skillid));
    ...
    public void setRuneTimeStamp() {
        if (LastTouchedRune == null) {
            ...
            LastTouchedRune = EtcTimer.getInstance().register(() -> {
    ...
    public void handleForceGain(int oid, int skillid, int extraForce) {
        ...
        if (stats.mp <= stats.getCurrentMaxMp()) {
            final List<Pair<PlayerStatList, Long>> statupz = new ArrayList<>(8);
                        ...
                        if (MapleDataTool.getIntConvert("invisible", skillId, 0) == 0) { // Invisible skills are not posted
                            if (getLevel() >= MapleDataTool.getIntConvert("reqLev", skillId, 0)) {
                                try {
									...
									System.out.println(ex + "Skill-maxer error occurred!");
                                }
                            }
                        }
                    }
    ...
    public final void handleOrbgain() {
        int orbcount = getBuffedValue(BuffStats.ComboCounter);
        ISkill combo;
        ISkill advcombo;
        switch (getJob()) {
            case 1110:
        ...
        if (GameConstants.isPhantom(getJob())) {
            int comboid = Objects.requireNonNull(getBuffedSkillEffect(BuffStats.ComboCounter, -1)).getSourceId();
            if (comboid == 11111001) {
                combo = SkillFactory.getSkill(11111001);
                advcombo = SkillFactory.getSkill(11110005);
        ...
        if (orbcount < ceffect.getX() + 1) {
            ...
            List<Triple<BuffStats, Integer, Boolean>> stat = new ArrayList<>();
            stat.add(new Triple<>(BuffStats.ComboCounter, neworbcount, false));
            assert combo != null;
    ...
    public void handleOrbconsume(int amount) {
        ...
        List<Triple<BuffStats, Integer, Boolean>> stat = Collections.singletonList(new Triple<>(BuffStats.ComboCounter, (getBuffedValue(BuffStats.ComboCounter) - amount), false));
    ...
    public List<Integer> HeadTitle() {
        ...
        int ee = num - (aa * 10000 + bb * 1000 + cc * 100 + dd * 10);
        num_.add(aa);
        num_.add(bb);
        num_.add(cc);
        num_.add(dd);
    ...
    public void elementalChargeHandler(int Count) {
        ...
        if (getSkillLevel(400011052) > 0) {
            Objects.requireNonNull(SkillFactory.getSkill(400011052)).getEffect(getSkillLevel(400011052)).applyTo(this);
        } else if (isActiveBuffedValue(400011053)) {
            Objects.requireNonNull(SkillFactory.getSkill(400011053)).getEffect(getSkillLevel(400011052)).applyTo(this);
        }
    }
    ...
    public List<Pair<EnchantEquipStats, Integer>> enchant() {
        return enchant;
    }
    ...
    public void enchantThis(List<Pair<EnchantEquipStats, Integer>> a) {
        this.enchant = a;
    }
    ...
    public void removeLinkSkill(int paramInt1, int paramInt2) {
        ...
        try {
            ...
            try {
                assert localPreparedStatement != null;
                localPreparedStatement.close();
                assert localResultSet != null;
    ...
    public int getLinkSkillId(int paramInt) {
        ...
        for (MapleData skillIdData : data.getChildren()) {
            skillPairList.add(new Pair<>(Integer.parseInt(skillIdData.getName()),
    ...
    public void updateOneInfo(final int questid, final String key, final String value) {
        if (!questinfo.containsKey(questid) || key == null || value == null || MapleQuest.getInstance(questid).isPartyQuest()) {
            return;
        }
    ...
    public void recalcPartyQuestRank(final int questid) {
        ...
        if (MapleQuest.getInstance(questid).isPartyQuest()) {
            return;
        }
    ...
    public String getOneInfo(final int questid, final String key) {
        if (!questinfo.containsKey(questid) || key == null || MapleQuest.getInstance(questid).isPartyQuest()) {
            return null;
        }
        final String[] split = questinfo.get(questid).split(";");
        for (String x : split) {
    ...
    public void expandInventory(byte type, int amount) {
        final MapleInventory inv = getInventory(Objects.requireNonNull(MapleInventoryType.getByType(type)));
        inv.addSlot((byte) amount);
        client.getSession().writeAndFlush(MainPacketCreator.getSlotUpdate(type, inv.getSlotLimit()));
    }
    ...
    public String getTowerChairSetting() {
        if (TowerChairSetting == null || TowerChairSetting.equals("")) {
            return "#0";
        }
    ...
    public int getMesoChairCount() {
        return Math.min(MesoChairCount, 999999999);
    }
    ...
    public void performAngelOfLibra() {
        final MapleCharacter chr = this;
        final SkillStatEffect effect = Objects.requireNonNull(SkillFactory.getSkill(400021032)).getEffect(getSkillLevel(400021032));
        ...
        Runnable r = () -> {
            for (Pair<Integer, MapleSummon> summon : chr.getSummons().values()) {
                if (summon.getLeft() == 400021032) {
                    List<MapleCharacter> toBuff = new LinkedList<>();
                    toBuff.add(chr);
                    ...
                    if (chr.getPartyId() != -1) {
                        ...
                        for (MapleMapObject chrObj : affecteds) {
                            MapleCharacter chr1 = (MapleCharacter) chrObj;

                            if (chr1.getParty() != null) {
                                for (MapleMapObject mo : affecteds) {
                                    MapleCharacter hp = (MapleCharacter) mo;
                                    if (hp.getPartyId() == chr1.getPartyId()) {
                                        toBuff.add(hp);
                                    }
                                }
                            }
                    ...
                    statups.add(new Triple<>(BuffStats.IndieDamR,
                            Integer.max(0,
                                    Integer.min(effect.getDamage(), effect.getStat("w")
                                            + (chr.getStat().getInt() / effect.getStat("attackCount")))),
                            true));
                    ...
                    for (MapleCharacter target : toBuff) {
                        ...
                        target.getClient().getSession()
                                .writeAndFlush(MainPacketCreator.giveBuff(400021032, localDuration, statups, effect,
                                        stacked, Objects.requireNonNull(SkillFactory.getSkill(400021032)).getAnimationTime(), target));
                        final long starttime = System.currentTimeMillis();
    ...
    public void startMapSaveTimeLimitTask(int time, final MapleMap to) {
        ...
        savetimeTask = EtcTimer.getInstance().register(() -> {
            if (savetime > 0) {
                savetime--;
            } else {
    ...
    public void startSpiritGate(SkillStatEffect eff) { // Original: startSpiritGate
        ...
        r = new Runnable() {
            ...
            public void run() {
                if (chr.isActiveBuffedValue(400051022)) {
                    SkillStatEffect a = Objects.requireNonNull(SkillFactory.getSkill(400051023)).getEffect(chr.getSkillLevel(400051022));
                    MapleSummon summon = new MapleSummon(chr, 400051023, a.getDuration(), chr.summonxy, SummonMovementType.SPIRIT_GATE, System.currentTimeMillis());
    ...
    public void addpocket() {
        SkillStatEffect effect = Objects.requireNonNull(SkillFactory.getSkill(4211003)).getEffect(getSkillLevel(4211003));
        List<Triple<BuffStats, Integer, Boolean>> statups = new ArrayList<>();
        statups.add(new Triple<>(BuffStats.PickPocket, 1, false));
    ...
    public int magicDebris = 0;
    public List<Point> Pointxy = new ArrayList<>();
    ...
    public void GivePassPinderShape(int skillid) {
        ...
        SkillStatEffect Effect = Objects.requireNonNull(SkillFactory.getSkill(skillid)).getEffect(getSkillLevel(skillid));
    ...
    public void setRelicCount(int value) {
        ...
        if (!this.isActiveBuffedValue(3310006) && RelicValue == 1000) {
            SkillStatEffect effects = Objects.requireNonNull(SkillFactory.getSkill(3310006)).getEffect(this.getSkillLevel(3310006));
            effects.applyTo(this);
        }
    ...
    public void handleCristalCharge(int cristalLevel, final int n) {
        ...
        if (this.getCristalCharge() >= 150) {
            ...
            if (this.getBuffedValue(BuffStats.CrystalChargeMax, cristalLevel) == null) {
                SkillStatEffect a = Objects.requireNonNull(SkillFactory.getSkill(cristalLevel)).getEffect(this.getSkillLevel(cristalLevel));
                a.applyTo(this);
            }
        }
    ...
    private transient Map<Equip, Integer> equipcustomValue = new LinkedHashMap<>();
    ...
    public boolean checkBoSang() {
        return getKeyValue3("bosang__") != -1;
    }
    ...
    public void removeKeyValue_new(int type, String key) {
        ...
        for (String s : data) {
            if (s.startsWith(key + "=")) { // 湲곗〈??議댁???? ?곗?????? (looks like Japanese being wrongly decoded as Korean)
        ...
        stat.setCustomData(stat.getCustomData()); // 湲곗〈 ?ㅻ꺼瑜?? ???곗???異??. (looks like Japanese being wrongly decoded as Korean)
        updateInfoQuest(type, stat.getCustomData());
    }
    ...
    public void removeKeyValue_new(int type) {
        MapleQuest quest = MapleQuest.getInstance(type);
        final MapleQuestStatus stat = quests.get(quest);
        if (stat == null) { // key ?곗??곌?  null (looks like Japanese being wrongly decoded as Korean)
            return;
        }
        if (stat.getCustomData() == null) {
            return;
    ...
    public long getKeyValue_new(int type, String key) {
        ...
        for (String s : data) {
            if (s.startsWith(key + "=")) {
                ...
                long dd = Long.parseLong(newkey2);
                return dd;
            }
        }
        return -1; // 誘몄〈?ъ? -1 (looks like Japanese being wrongly decoded as Korean)
    }
    ...
    private int getCondemnationSkillId() {
        int skillId = 0;
        int CONDEMNATION = 32001014;
        int CONDEMNATION_II = 32110017;
        int CONDEMNATION_III = 32120019;
        if (getJob() == 0) {
