...
import client.ItemInventory.Items.MapleProfessionRecipe;
import client.ItemInventory.Items.MapleProfessionRecipeEntry;
...
public class ProfessionHandler {
    ...
    public enum CraftGrade {
        ...
        CraftGrade(int grade, int exp) {
            this.grade = grade;
            this.exp = exp;
        }
    }

    public static void spawnExtractor(ReadingMaple rh, MapleClient c) {
        MapleCharacter chr = c.getPlayer();
        ...
        chr.setExtractor(
                new MapleExtractor(chr, itemId, fee, Objects.requireNonNull(chr.getMap().getFootholds().findMaple(chr.getPosition())).getId())); // no
        // clue
        // about
        // time
        // left
    ...
    public static void useRecipe(ReadingMaple rh, MapleClient c) {
        ...
        ItemInformation ii = ItemInformation.getInstance();
        SkillStats spec = Objects.requireNonNull(ii.getItemEffect(itemid)).getSkillStats();
    ...
    public static void SwitchBag(final ReadingMaple slea, final MapleClient c) {
    ...
    public static void MoveBag(final ReadingMaple slea, final MapleClient c) {
        if (c.getPlayer().hasBlockedInventory()) { // hack
            return;
        }
        slea.readInt(); // c.getPlayer().updateTick(slea.readInt());
