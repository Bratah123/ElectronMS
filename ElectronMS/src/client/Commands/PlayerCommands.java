	...
	*/
	...
	public static class Mob extends PlayerCommand {
		...
		public int execute(MapleCharacter chr, String[] args) {
			...
			String mobHp = "Mob HP: " + chr.getMap().getMonsterByOid(oid).getHp();
	...
	@Command(names = {"callgm"}, parameters = "<message>", requiredType = AccountType.PLAYER)
	public static class CallGM extends PlayerCommand {
		@Override
		public int execute(MapleCharacter chr, String[] args) {
			if (args.length <= 1) {
			...
			if (message.length() >= 5) {
					...
					for (int i = 0; i < adminNames.length; i++) {
						...
						if (admins != null) {
							admins.dropMessage(6, "[CallGM] " + playerName + ": " + message);
