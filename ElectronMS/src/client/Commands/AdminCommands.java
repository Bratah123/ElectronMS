...
import constants.SystemsConstants.MedalRanking;
...
public class AdminCommands {
		...
		public int execute(MapleCharacter chr, String[] args) {
			...
			if (target != null) {
				...
				if (target.ban(sb.toString(), true, false)) {
					chr.dropMessage(6, target.getName() + " has been banned.");
	...
	public static class CM extends AdminCommand {
		@Override
		public int execute(MapleCharacter c, String[] splitted) {
			c.dropMessage(5, "You are currently in the following map: " + c.getMapId());
			return 1;
		}
	...
	public static class ReloadMap extends AdminCommand {
		...
		public int execute (MapleCharacter chr, String[] args)
		{
			...
			List<MapleMapObject> characters = chr.getMap().getAllPlayer();
			for (MapleMapObject playersInMap : characters) {
