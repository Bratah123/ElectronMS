...
import server.CustomEvents.MapleDojoAgent;
import server.CustomEvents.MapleEvent;
...
public abstract class AbstractPlayerInteraction {
    ...
    public final void warp(final int map, String portal) {
        ...
        if (map == c.getPlayer().getMapId()) { // test
            final Point portalPos = new Point(Objects.requireNonNull(c.getPlayer().getMap().getPortal(portal)).getPosition());
            if (portalPos.distanceSq(getPlayer().getTruePosition()) < 90000.0) { // estimation
                c.getPlayer().checkFollow();
                c.getSession().writeAndFlush(
                        MainPacketCreator.instantMapWarp((byte) Objects.requireNonNull(c.getPlayer().getMap().getPortal(portal)).getId()));
                c.getPlayer().getMap().movePlayer(c.getPlayer(),
                        new Point(Objects.requireNonNull(c.getPlayer().getMap().getPortal(portal)).getPosition()));
            } else {
                c.getPlayer().changeMap(mapz, Objects.requireNonNull(mapz.getPortal(portal)));
            }
        } else {
            c.getPlayer().changeMap(mapz, Objects.requireNonNull(mapz.getPortal(portal)));
        }
    ...
    public final void killAllMob2() {
        c.getPlayer().getMap().killAllMonsters2(true);
    }
    ...
    public final boolean canHold() {
        for (int i = 1; i <= 5; i++) {
            if (c.getPlayer().getInventory(Objects.requireNonNull(MapleInventoryType.getByType((byte) i))).getNextFreeSlot() <= -1) {
                return false;
            }
        }
        return true;
    ...
    public final boolean canHoldSlots(final int slot) {
        for (int i = 1; i <= 5; i++) {
            if (c.getPlayer().getInventory(Objects.requireNonNull(MapleInventoryType.getByType((byte) i))).isFull(slot)) {
                return false;
            }
        }
        return true;
    ...
    public final boolean canHoldEquip(final int slot) {
        return !c.getPlayer().getInventory(Objects.requireNonNull(MapleInventoryType.getByType((byte) 1))).isFull(slot);
    }
    ...
    public final void guildMessage(final int type, final String message) {
        if (getPlayer().getGuildId() > 0) {
            CommunityHandler.getInstance().guildPacket(getPlayer().getGuildId(),
    ...
    public final MapleGuild getGuild(int guildid) {
        return CommunityHandler.getInstance().getGuild(guildid);
    }
    ...
    public final void useSkill(final int skill, final int level) {
        ...
        Objects.requireNonNull(SkillFactory.getSkill(skill)).getEffect(level).applyTo(c.getPlayer());
    }

    public final void useItem(final int id) {
        Objects.requireNonNull(ItemInformation.getInstance().getItemEffect(id)).applyTo(c.getPlayer());
    ...
    public final void teachSkill(final int id, byte level) {
        ...
        if (getPlayer().getSkillLevel(skill) > level) {
            level = getPlayer().getSkillLevel(skill);
        }
        assert skill != null;
        getPlayer().changeSkillLevel(skill, level, skill.getMaxLevel());
    }
    ...
    public final int getSavedLocation(final String loc) {
        final int ret = c.getPlayer().getSavedLocation(SavedLocationType.fromString(loc));
        if (ret == -1) {
            return 100000000;
        }
        return ret;
    ...
    public void gainGP(final int gp) {
        ...
        CommunityHandler.getInstance().gainGP(getPlayer().getGuildId(), gp); // 1 for
    }
    ...
    public int getGP() {
        ...
        return Objects.requireNonNull(CommunityHandler.getInstance().getGuild(getPlayer().getGuildId())).getGP();
    }
    ...
    public final MapleInventory getInventory(int type) {
        return c.getPlayer().getInventory(Objects.requireNonNull(MapleInventoryType.getByType((byte) type)));
    }
    ...
    public final void startAswanOffSeason(final MapleCharacter leader) {
        ...
        EventTimer.getInstance().schedule(new Runnable() {
            @Override
            public void run() {
                if (leader.getMapId() == 955000100 || leader.getMapId() == 955000200 || leader.getMapId() == 955000300) {
            ...
            while (rs.next()) {
                ...
                for (String da : data) {
                    if (da.startsWith("dojo=")) {
                        ...
                        dataz = Integer.parseInt(valuez.replace(";", ""));
                    }
                    if (da.startsWith("dojo_time=")) {
                        String valuez = da.replace("dojo_time=", "");
                        timez = Integer.parseInt(valuez.replace(";", ""));
                    }
                }
        ...
        for (int i = 0; i < file.size(); i++) {
            for (int j = i + 1; j < file.size(); j++) {
                if (file.get(i).first < file.get(j).first) {
                    ...
                    file.set(j, temp);
                } else if (file.get(i).first.equals(file.get(j).first)) {
