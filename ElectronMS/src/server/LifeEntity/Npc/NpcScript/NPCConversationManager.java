...
import handlers.Global.InterServerHandler;
import client.ItemInventory.Inventory.InventoryHandler;
import static client.ItemInventory.Inventory.InventoryHandler.potential;
import provider.MapleData;
import provider.MapleDataProviderFactory;
import provider.MapleDataTool;
import server.CustomEvents.SafiEvent;
import server.Systems.MapleSlideMenu;
import server.Systems.MapleSlideMenu.SlideMenu0;
import server.Systems.MapleSlideMenu.SlideMenu1;
import server.Systems.MapleSlideMenu.SlideMenu2;
import server.Systems.MapleSlideMenu.SlideMenu3;
import server.Systems.MapleSlideMenu.SlideMenu4;
import server.Systems.MapleSlideMenu.SlideMenu5;
...
public class NPCConversationManager extends AbstractPlayerInteraction {
    ...
    public void safiEnd() {
        WorldBroadcasting.broadcastMessage(MainPacketCreator.getGMText(11, "Everyone i'm sorry, but the event is over ... See you next time~"));
        safi = false;
    }
	...
	{
		EtcTimer.getInstance().schedule(() -> sendPlayerToNpc(text), delay);
	}
	...
	{
		return MapleCharacterUtil.getIdByName(name);
	}
	public void getPlayerByName(String name)
	{
		getClient().getChannelServer().getPlayerStorage().getCharacterByName(name);
	}
	...
	public int getQuantityOfItem(int itemId)
	{
		...
		{
			System.out.println("Error: " + itemId + " does not exist");
			dispose();
		}
		else {
			itemQuantity = getPlayer().getItemQuantity(itemId, false);
		}
		return itemQuantity;
	}
   ...
   public int getCSPoints() {
    ...
    public void addFrozenMobCount(int a1) {
        int val = Math.min((getFrozenMobCount() + a1), 9999);
...
*/
	...
	// don't use this piece of shit I'll fix it later. Brandon
	public void consumeItemCash(int itemId)
	{
			getPlayer().getCashInventory().removeItemByCashId(itemId);
			getPlayer().dropMessage(5, "You lost 1 red cube.");
	}
	...
	/* public boolean createAlliance(String alliancename)
	{
		...
		try
		{
			return CommunityHandler.getInstance().createAlliance(alliancename, c.getPlayer().getId(), otherChar.getId(),
	...
	public boolean addCapacityToAlliance()
	{
		try
		{
			final MapleGuild gs = CommunityHandler.getInstance().getGuild(c.getPlayer().getGuildId());
			if (gs != null && c.getPlayer().getGuildRank() == 1 && c.getPlayer().getAllianceRank() == 1)
			{
				if (CommunityHandler.getInstance().getAllianceLeader(gs.getAllianceId()) == c.getPlayer().getId()
						&& CommunityHandler.getInstance().changeAllianceCapacity(gs.getAllianceId()))
				{
					gainMeso(-10000000);
					return true;
				}
	...
	public boolean disbandAlliance()
	{
		try
		{
			...
			{
				if (CommunityHandler.getInstance().getAllianceLeader(gs.getAllianceId()) == c.getPlayer().getId()
						&& CommunityHandler.getInstance().disbandAlliance(gs.getAllianceId()))
				{
					return true;
				}
			}
	...
	public final void maxAllSkills()
	{
		...
		{
			if (GameConstants.isApplicableSkill(skill.getId()) && skill.getId() < 90000000)
			{ // no
				// db/additionals/resistance
				// skills
				sa.put(skill, new SkillEntry(skill.getMaxLevel(), skill.getMaxLevel(),
						SkillFactory.getDefaultSExpiry(skill)));
			}
		}
	...
	public final void maxSkillsByJob()
	{
		...
		for (ISkill skill : SkillFactory.getAllSkills())
		{
			if (GameConstants.isApplicableSkill(skill.getId()) && skill.canBeLearnedBy(getPlayer().getJob())
					&& !skill.isInvisible())
	...
	public final void removeSkillsByJob()
	{
		...
		for (ISkill skill : SkillFactory.getAllSkills())
		{
			...
			{ // no
				...
				// skills
				sa.put(skill,
						new SkillEntry((byte) -1, skill.getMaxLevel(), SkillFactory.getDefaultSExpiry(skill)));
			}
		}
		getPlayer().changeSkillsLevel(sa);
	}
	...
	public final void maxSkillsByJobId(int jobid)
	{
		...
		{
			if (GameConstants.isApplicableSkill(skill.getId()) && skill.canBeLearnedBy(getPlayer().getJob())
					&& skill.getId() >= jobid * 1000000 && skill.getId() < (jobid + 1) * 1000000 && !skill.isInvisible())
			{
				sa.put(skill, new SkillEntry(skill.getMaxLevel(), skill.getMaxLevel(),
						SkillFactory.getDefaultSExpiry(skill)));
			}
	...
	public String checkDrop(MapleCharacter chr, int mobId)
	{
		...
		if (ranks != null && ranks.size() > 0)
		{
			...
			for (MonsterDropEntry rank : ranks) {
				de = rank;
				if (de.chance > 0 && (de.questid <= 0 || MapleQuest.getInstance(de.questid).getName().length() > 0)) {
					itemId = de.itemId;
					if (num == 0) {
	...
	public String EquippedListVertical(MapleClient c)
	{
		StringBuilder str = new StringBuilder();
	...
	{
		EtcTimer.getInstance().schedule(() -> sendNextNoESC(text), delay);
	}

	public void sendNextS(int deay, String text, int type)
	{
		EtcTimer.getInstance().schedule(() -> sendNextS(text, (byte) type), deay);
	}

	public void sendNextPrevS(int deay, String text, int type)
	{
		EtcTimer.getInstance().schedule(() -> sendNextPrevS(text, (byte) type), deay);
	}

	public void sendNextNoESC(int delay, String text, int npcid)
	{
		EtcTimer.getInstance().schedule(() -> sendNextNoESC(text, npcid), delay);
	}

	public void warp(int delay, int mapid, int portal)
	{
		EtcTimer.getInstance().schedule(() -> warp(mapid, portal), delay);
	}
	...
	public List<Triple<Short, String, Integer>> rankList(short[] ranks, String[] names, int[] values)
	{
		...
		if (names.length != values.length || values.length != ranks.length)
		{
			return null;
		}
	...
	public List<StructPotentialItem> getPotentialInfoById(int id)
	{
		ItemInformation ii = ItemInformation.getInstance();
    ...
    public String DamageMeterRank() {
        ...
        try {
            ...
            int i = 1;
            while (rs.next()) {
                text.append("#r#e").append(i != 10 ? "0" : "").append(i).append("#n#b위 #r닉네임#b ").append(rs.getString("name")).append(" #r누적 데미지#b ").append(Comma(rs.getLong("damage"))).append("\r\n");
                i++;
            }
            rs.close();
            ps.close();
        ...
        if (text.toString().equals("#b")) {
            text = new StringBuilder("#r아직까지 딜량 미터기를 갱신한 유저가 없습니다.");
        }
    ...
    public void installNamed(int RC, int selection)
    {
        if (isNamedTimeOver())
        {
        ...
        Named.install_named_char(getPlayer().getId(), 0, 0, (int) (RC * (selection == 0 || selection == 1 ? 1.9 : selection == 2 || selection == 3 || selection == 4 || selection == 5 ? 1.9 : selection == 6 || selection == 7 || selection == 8 || selection == 9 ? 3.6 : 0)), selection, Named.nextDate + "-" + Named.nextTime);
        sendOk("\n" + "Bets have been completed. I wish you good results");
        loseRC(RC);
        dispose();
    }
	...
	public String Comma(long r)
	{
		...
		for (int i = String.valueOf(r).length(); i >= 1; i--)
		{
			...
			re.append(String.valueOf(r).charAt(i - 1));
	...
	public String getCashEquipList()
	{
		...
		{
			if (Objects.requireNonNull(ItemInformation.getInstance().getEquipStats(item.getItemId())).get("cash") > 0)
			{
				text.append("#L").append(item.getPosition()).append("# #i").append(item.getItemId()).append("# #t").append(item.getItemId()).append("##l\r\n");
			}
		}
		return text.toString();
	}
	...
	public int getManneHair(int slot)
	{
		...
		return Integer.parseInt(qd.split("h" + slot + "=")[1].split(";")[0]);
	}
    ...
    public void StartBingoGame() {
        ...
        MapTimer.getInstance().schedule(() -> c.getPlayer().getBingoGame().StartGame(), 42 * 1000);
    }
