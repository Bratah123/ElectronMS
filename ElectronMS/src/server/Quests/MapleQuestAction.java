...
class MapleQuestAction implements Serializable
{
	...
	private List<QuestSkill> skills = new ArrayList<>();

	private Map<Integer, Byte> quests = new HashMap<>();
	private Map<Byte, List<Short>> sp = new HashMap<>();
	...
	protected MapleQuestAction(MapleQuest quest, ReadBin data) throws IOException
	{
		...
		int insightExp = data.readInt();
		int senseExp = data.readInt();
		int charismaExp = data.readInt();
		int charmExp = data.readInt();
		int craftExp = data.readInt();
		int willExp = data.readInt();
	...
	public void runStart(MapleCharacter c, Integer extSelection)
	{
		...
		if (buffItemID > -1)
		{
			...
			Objects.requireNonNull(ItemInformation.getInstance().getItemEffect(tobuff)).applyTo(c);
		}
