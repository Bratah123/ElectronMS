...
import client.Skills.VMatrix.MapleNodes.MapleNodeInfo;
import client.Skills.VMatrix.MapleNodes.MaplePlatform;
...
public class MapleWorldMapProvider {
            ...
            try {
                ...
                for (int i = 0; i < fieldgenmaps.size(); i++) {
                    if (fieldgenmaps.get(i).getName().equals(String.valueOf(i))) {
                        ...
                        if (templatecheck < mapid && mapid < templatecheck + count) {
                ...
                if (mapid >= 910000001 && mapid <= 910000022) {
                    Objects.requireNonNull(map.getPortal("out00")).setScriptName("out_fm01");
                }
                ...
                if (npcs) {
                    try {
                        ...
                        while (rs.next()) {
                            ...
                            assert pnpc != null;
                            pnpc.setFh(Objects.requireNonNull(map.getFootholds().findMaple(pnpc.getPosition())).getId());
                ...
                try {
                    map.setMapName(
                            MapleDataTool.getString("mapName", nameData.getChildByPath(getMapStringName(mapid)), ""));
                    map.setStreetName(MapleDataTool.getString("streetName",
                            nameData.getChildByPath(getMapStringName(mapid)), ""));
                } catch (Exception e) {
                    map.setMapName("");
                    map.setStreetName("");
                }
    ...
    private String getFieldGenerator() {
        return "Map/FieldGenerator.img";
    }

    private String getMapName(int mapid) {
        String mapName = StringUtil.getLeftPaddedStr(Integer.toString(mapid), '0', 9);

        mapName = "Map/Map" + mapid / 100000000 +
                "/" +
                mapName +
                ".img";
        return mapName;
