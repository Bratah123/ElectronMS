...
import server.Quests.MapleQuestStatus;
...
import client.ItemInventory.Inventory.InventoryHandler;
...
import server.Maps.MapleMapHandling.MapScript.MapScriptMethods;
import server.Maps.MapleMapHandling.MaplePortal.MaplePortal;
import server.Systems.MonsterCollection;
import server.Systems.MonsterCollectionRegion.CollectionField.CollectionGroup.CollectionMob;
...
import server.LifeEntity.Mob.MonsterEntity.MonsterDropEntry;
import server.LifeEntity.Mob.MonsterEntity.MonsterGlobalDropEntry;
import client.Stats.MonsterStatus.OverrideMonsterStats;
import server.LifeEntity.Mob.MobSpawnPoint;
import server.LifeEntity.Mob.BossEntity.SpawnPointAreaBoss;
import server.LifeEntity.Mob.MonsterEntity.MonsterSpawnEntry;
...
import handlers.Global.BossEvent.Damien.DemianSword;
...
import client.Skills.VMatrix.MapleNodes;
...
public class MapleMap {
    ...
    private final Collection<MonsterSpawnEntry> monsterSpawn = new LinkedList<>();
    ...
    private Map<Integer, MapleNPC> tempnpcs3 = new HashMap<>();
    private Map<Integer, MapleMonster> tempmonsters3 = new HashMap<>();
    private WeakReference<MapleCharacter> changeMobOrigin = null;
    private List<Integer> droppedItems = new LinkedList<>();
    ...
    public MapleMap(final int mapid, final int channel, final int returnMapId, final float monsterRate) {
        ...
        for (MapleMapObjectType type : MapleMapObjectType.values()) {
            mapobjects.put(type, new ConcurrentHashMap<>());
        }
    }
    ...
    public final void checkClockContact(final MapleCharacter chr, final MapleMonster monster) {
        if (this.getFieldType().equals("63")) {
            for (MapleMapObject object : this.getMapObjectsInRange(chr.getPosition(), Double.POSITIVE_INFINITY, Collections.singletonList(MapleMapObjectType.MIST))) {
    ...
    public final Point calcPointMaple(final Point initial) {
        ...
        if (!fh.isWall() && fh.getY1() != fh.getY2()) {
            ...
            if (fh.getY2() < fh.getY1()) {
                dropY = fh.getY1() - (int) (cos
                        * (Math.abs(initial.x - fh.getX1()) / cos1));
            } else {
                dropY = fh.getY1() + (int) (cos
                        * (Math.abs(initial.x - fh.getX1()) / cos1));
            }
    ...
    private void dropFromMonster(final MapleCharacter chr, final MapleMonster mob) {
        ...
        if (mob.isEliteMonster()) { // Elite Monsters
            dropEntry.add(new MonsterDropEntry(4001832, 300000, 1, 1, 0)); // Spell trace
            dropEntry.add(new MonsterDropEntry(2432970, 75000, 1, 1, 0)); // Honor badges
            dropEntry.add(new MonsterDropEntry(5062009, 3000, 1, 3, 0)); // Red cubes
        }
        /*  Arcane Symbols  */
        /*  Vanishing Journey */
        if (mob.getId() == 8641000 || mob.getId() == 8641001 || mob.getId() == 8641002 || mob.getId() == 8641003 || mob.getId() == 8641004 || mob.getId() == 8641005 || mob.getId() == 8641006
                || mob.getId() == 8641007 || mob.getId() == 8641012) {

            dropEntry.add(new MonsterDropEntry(1712001, 900, 1, 1, 0));

        }
        /*    Chuchu Symbol      */
        if(mob.getId() == 8642000 || mob.getId() ==  8642001 || mob.getId() ==  8642002 || mob.getId() ==  8642003 || mob.getId() ==  8642004 || mob.getId() ==  8642005 || mob.getId() ==  8642006
                || mob.getId() ==  8642007 ||mob.getId() ==  8642008 || mob.getId() ==  8642009 || mob.getId() ==  8642010 || mob.getId() ==  8642011 || mob.getId() ==  8642012 || mob.getId() ==  8642013
                || mob.getId() ==  8642014 || mob.getId() ==  8642015) // Chuchu arcane drop
        {

            dropEntry.add(new MonsterDropEntry(1712002, 700, 1, 1, 0));


        }
        /*    Lachelein Symbol      */
        if(mob.getId() == 8643001 || mob.getId() == 8643002 || mob.getId() == 8643003 || mob.getId() == 8643004 || mob.getId() == 8643005 || mob.getId() == 8643006 || mob.getId() == 8643007 || mob.getId() == 8643008 || mob.getId() == 8643009
                || mob.getId() == 8643010 || mob.getId() == 8643011 || mob.getId() == 8643012) // Lachelein arcane drop
        {

            dropEntry.add(new MonsterDropEntry(1712003, 500, 1, 1, 0));

        }
        /*    Arcana Symbol      */
        if(mob.getId() == 8644000 || mob.getId() == 8644001 || mob.getId() == 8644002 || mob.getId() == 8644003 || mob.getId() == 8644004 || mob.getId() == 8644005 || mob.getId() == 8644006
                || mob.getId() == 8644007 || mob.getId() == 8644008 || mob.getId() == 8644009 || mob.getId() == 8644010 || mob.getId() == 8644012) // Arcana arcane drop
        {

            dropEntry.add(new MonsterDropEntry(1712004, 300, 1, 1, 0));

        }
        /*    Morass Symbol      */
        if(mob.getId() == 8644400 || mob.getId() == 8644401 || mob.getId() == 8644402 || mob.getId() == 8644403 || mob.getId() == 8644404 || mob.getId() == 8644405 || mob.getId() == 8644406
                || mob.getId() == 8644407 || mob.getId() == 8644408 || mob.getId() == 8644409 || mob.getId() == 8644410 || mob.getId() == 8644411 || mob.getId() == 8644412) // Morass arcane drop
        {

            dropEntry.add(new MonsterDropEntry(1712005, 100, 1, 1, 0));

        }
        /*    Esfera Symbol      */
        if(mob.getId() == 8644500 || mob.getId() == 8644501 || mob.getId() == 8644502 || mob.getId() == 8644503 || mob.getId() == 8644504 || mob.getId() == 8644505 || mob.getId() == 8644506
                || mob.getId() == 8644507 || mob.getId() == 8644508 || mob.getId() == 8644509 || mob.getId() == 8644510) // Esfera arcane drop
        {

            dropEntry.add(new MonsterDropEntry(1712006, 50, 1, 1, 0));
        ...
        if (mob.isEliteBoss()) { // Elite Boss
            dropEntry.add(new MonsterDropEntry(4001832, 900000, 1, 10, 0)); //Traces of orders
            dropEntry.add(new MonsterDropEntry(2432970, 900000, 1, 5, 0)); //Special Medal of Honor
            dropEntry.add(new MonsterDropEntry(2630282, 900000, 1, 1, 0)); //Mysterious Core Gemstone Box
            dropEntry.add(new MonsterDropEntry(2433942, 900000, 1, 1, 0)); //Meso box

            int chance = Randomizer.rand(1, 100000);
            int rand = Randomizer.rand(1, 30);
            if (chance < 100) {
                chr.gainRC(rand);
                chr.send(MainPacketCreator.sendHint("Now #b#h ##kYou gained #r" + rand + " #kDP.", 350, 5));
            }

            //chr.modifyCSPoints(1, 500, true); // Kill Point
            //chr.send(MainPacketCreator.sendHint("Now #b#h ##kYour kill point is #r" + chr.getCSPoints(1) + " #kpoints.", 350, 5));
        }
        ...
        for (final MonsterDropEntry de : dropEntry) {
            ...
            GameConstants.getInventoryType(de.itemId);
                ...
                if (de.itemId == 0) {
                    ...
                    if (GameConstants.getInventoryType(de.itemId) == MapleInventoryType.EQUIP) {
                        ...
                        if (GameConstants.환생의불꽃아이템(idrop.getItemId())) {
                            idrop = InventoryHandler.Reincarnation((Equip) idrop);
                        }
                    } else {
    ...
    public final void killAllMonsters2(final boolean animate) {
    ...
    public final void killMonster(final MapleMonster monster, final MapleCharacter chr, final boolean withDrops, final boolean second, final byte animation) {
        ...
        String week = now.get(Calendar.WEEK_OF_YEAR) + "";
        ...
        if ((chr.getMapId() - 925070000) > 0 && (chr.getMapId() - 925070000) <= 6300) {
            ...
            if (floor / 10 == 3 && floor != 30 && floor != 33 && floor != 36 && floor != 39 && chr.mCount < 1) {
                ...
                chr.dropMessage(6, "Defeated the opponent. Timer stops for 10 seconds.");
                System.out.println("Test Tote");
        ...
        if (hpoint == 1) {
            boolean killp = false;
            int[] blockmob = {8881000, 8880150, 9440025, 8880302, 6500001, 8880503, 9100000, 9100001,8250000,8250001,8250002,2300100,4230102,3230101,2230101,2230131};  //Mobs that won't get kill points
            for (int value : blockmob) {
                if (monster.getId() == value) {
                    killp = true;
                    break;
                }
            }
            if (!killp) {
                if (Randomizer.isSuccess(50)) {
                    chr.modifyCSPoints(1, 1, true); // Kill Point
        ...
        int[] point2 = {30000, 15000, 10000, 7500, 6000, 5000};  //Kill point amount per party
        ...
        if (id == 8880150) {
            if (chr.getParty() != null) { //TODO : Take that example to fix boss party reward bug
                for (MaplePartyCharacter pc : chr.getParty().getMembers()) {
                    ...
                    if (ch != null) {
                        if (ch.getMapId() == 450004250) {
                            ...
                            ch.send(MainPacketCreator.sendHint("Congratulations!!\r\n#b" + monster.getStats().getName() + " #kKilled #r" + point2[ch.getParty().getMembers().size() - 1] + " 킬포인트#k를 얻으셨습니다.", 350, 50));
                        } else {
                            ch.send(MainPacketCreator.sendHint("\n" + "I didn't get a kill point because I didn't have it on the boss map", 350, 50));
                        }
                    }
                }
            }
                ...
                for (MaplePartyCharacter pc : chr.getParty().getMembers()) {
                    ...
                    if (ch != null) {
                        if (ch.getMapId() == 992040000) {
                            ...
                            ch.send(MainPacketCreator.sendHint("Congratulations!!\r\n#b" + monster.getStats().getName() + " #kKilled #r" + point3[ch.getParty().getMembers().size() - 1] + " 킬포인트#k를 얻으셨습니다.", 350, 50));
                        } else {
                            ch.send(MainPacketCreator.sendHint("I didn't get a kill point because I didn't have it on the boss map", 350, 50));
                        }
                    }
                }
            }
                ...
                for (MaplePartyCharacter pc : chr.getParty().getMembers()) {
                    ...
                    if (ch != null) {
                        if (ch.getMapId() == 450008350) {
                            ...
                            ch.send(MainPacketCreator.sendHint("Congratulations!!\r\n#b" + monster.getStats().getName() + " #kKilled #r" + point4[ch.getParty().getMembers().size() - 1] + " 킬포인트#k를 얻으셨습니다.", 350, 50));
                        } else {
                            ch.send(MainPacketCreator.sendHint("I didn't get a kill point because I didn't have it on the boss map", 350, 50));
                        }
                    }
                ...
                for (MaplePartyCharacter pc : chr.getParty().getMembers()) {
                    ...
                    if (ch != null) {
                        if (ch.getMapId() == 450010800) {
                            ...
                            ch.send(MainPacketCreator.sendHint("Congratulations!!\r\n#b" + monster.getStats().getName() + " #kKilled #r" + point5[ch.getParty().getMembers().size() - 1] + " 킬포인트#k를 얻으셨습니다.", 350, 50));
                        } else {
                            ch.send(MainPacketCreator.sendHint("I didn't get a kill point because I didn't have it on the boss map", 350, 50));
                        }
                    }
                ...
                for (MaplePartyCharacter pc : chr.getParty().getMembers()) {
                    ...
                    if (ch != null) {
                        if (ch.getMapId() == 500000011) {
                            ...
                            ch.send(MainPacketCreator.sendHint("Congratulations!!\r\n#b" + monster.getStats().getName() + " #kKilled #r" + point6[ch.getParty().getMembers().size() - 1] + " 킬포인트#k를 얻으셨습니다.", 350, 50));
                        } else {
                            ch.send(MainPacketCreator.sendHint("I didn't get a kill point because I didn't have it on the boss map", 350, 50));
                        }
                    }
       ...
       /* if (chr.getDateKey("questB4", false) != null) {
        ...
        if(monster.getId() == 8641000 || monster.getId() == 8641001 || monster.getId() == 8641002 || monster.getId() == 8641003 || monster.getId() == 8641004 || monster.getId() == 8641005 || monster.getId() == 8641006 || monster.getId() == 8641007 || monster.getId() == 8641012// Nodestone drop Vanishing journey
           || monster.getId() == 8642000 || monster.getId() ==  8642001 || monster.getId() ==  8642002 || monster.getId() ==  8642003 || monster.getId() ==  8642004 || monster.getId() ==  8642005 || monster.getId() ==  8642006 || monster.getId() ==  8642007 || monster.getId() ==  8642008 || monster.getId() ==  8642009 || monster.getId() ==  8642010 || monster.getId() ==  8642011 || monster.getId() ==  8642012 || monster.getId() ==  8642013 || monster.getId() ==  8642014 || monster.getId() ==  8642015 // Chuchu
           || monster.getId() == 8643001 || monster.getId() == 8643002 || monster.getId() == 8643003 || monster.getId() == 8643004 || monster.getId() == 8643005 || monster.getId() == 8643006 || monster.getId() == 8643007 || monster.getId() == 8643008 || monster.getId() == 8643009 || monster.getId() == 8643010 || monster.getId() == 8643011 || monster.getId() == 8643012 // Lachelein
           || monster.getId() == 8644000 || monster.getId() == 8644001 || monster.getId() == 8644002 || monster.getId() == 8644003 || monster.getId() == 8644004 || monster.getId() == 8644005 || monster.getId() == 8644006 || monster.getId() == 8644007 || monster.getId() == 8644008 || monster.getId() == 8644009 || monster.getId() == 8644010 || monster.getId() == 8644012 // Arcana
           || monster.getId() == 8644400 || monster.getId() == 8644401 || monster.getId() == 8644402 || monster.getId() == 8644403 || monster.getId() == 8644404 || monster.getId() == 8644405 || monster.getId() == 8644406 || monster.getId() == 8644407 || monster.getId() == 8644408 || monster.getId() == 8644409 || monster.getId() == 8644410 || monster.getId() == 8644411 || monster.getId() == 8644412 // Morass
           || monster.getId() == 8644500 || monster.getId() == 8644501 || monster.getId() == 8644502 || monster.getId() == 8644503 || monster.getId() == 8644504 || monster.getId() == 8644505 || monster.getId() == 8644506 || monster.getId() == 8644507 || monster.getId() == 8644508 || monster.getId() == 8644509 || monster.getId() == 8644510) // Esfera
        {
            int rand = Randomizer.rand(1, 15000);
            if(rand <= 20)
            {
                spawnMobDrop(new Item(2435719, (byte) 0, (short) 1, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
            }
            trace = 800;
        }
        ...
        if(trace != 0)
        {
            ...
            if(randtrace <= trace)
            {
                spawnMobDrop(new Item(4001832, (byte) 0, (short) 1, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
            }
        }
        ...
        if (!this.isEliteBossMap() && !monster.isEliteMonster() && !monster.isEliteBoss()
            ...
            broadcastMessage(MainPacketCreator.startMapEffect("The Azure energy doesn't go away, making this place gloomy.", 5120124, true));
            monster.setEliteMonster(false);
            timeAllPlayer();
            if(!EliteBossDuplicate)
            {
        ...
        if (this.EliteMobCommonCount >= 50000 && !EliteMobCommonCountHalf) {
            EliteMobCommonCountHalf = true;
            broadcastMessage(MainPacketCreator.startMapEffect("I can feel the Azure energy.", 5120124, true));
        }
            ...
            broadcastMessage(MainPacketCreator.startMapEffect("Powerful monster appears with Azure energy.", 5120124, true));
            timeAllPlayer();
        ...
        if (monster.getId() == 8220022) {// Elite 1
            int rand = Randomizer.rand(20, 50);
            spawnMobDrop(new Item(4031102, (byte) 0, (short) rand, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
        }
        ...
        if (monster.getId() == 8645009) { // Elite 5 Dunkel
            int rand = Randomizer.rand(100, 150);
            spawnMobDrop(new Item(4031102, (byte) 0, (short) rand, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
        }


        /* Jcoins Bosses */
        if (monster.getId() == 5220002) { // Faust
            spawnMobDrop(new Item(4310034, (byte) 0, (short) 10, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
        }
        if (monster.getId() == 5220000) {
            spawnMobDrop(new Item(4310034, (byte) 0, (short) 20, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
        }
        if (monster.getId() == 9300119) {
            spawnMobDrop(new Item(4310034, (byte) 0, (short) 30, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
        }
        if (monster.getId() == 8300006) {
            spawnMobDrop(new Item(4310034, (byte) 0, (short) 50, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
        }
        if (monster.getId() == 8300007) {
            spawnMobDrop(new Item(4310034, (byte) 0, (short) 60, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
        }
        if (monster.getId() == 9309205) {
            spawnMobDrop(new Item(4310034, (byte) 0, (short) 80, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
        }
        if (monster.getId() == 9309208) {
            spawnMobDrop(new Item(4310034, (byte) 0, (short) 110, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
        }
        if (monster.getId() == 8220011) {
            spawnMobDrop(new Item(4310034, (byte) 0, (short) 130, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
        }
        if (monster.getId() == 9010009) {
            spawnMobDrop(new Item(4310034, (byte) 0, (short) 150, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
        }
        // Box
        if (monster.getId() == 9390710) {
            spawnMobDrop(new Item(4032101, (byte) 0, (short) 10, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
        }
        if (monster.getId() == 9390711) {
            spawnMobDrop(new Item(4032101, (byte) 0, (short) 20, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
        }
        if (monster.getId() == 9390712) {
            spawnMobDrop(new Item(4032101, (byte) 0, (short) 30, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
        }
        if (monster.getId() == 8220003) {
            spawnMobDrop(new Item(4032101, (byte) 0, (short) 50, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
        }
        if (monster.getId() == 2600800) {
            spawnMobDrop(new Item(4032101, (byte) 0, (short) 70, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
        }
        if (monster.getId() == 8644011) {
            spawnMobDrop(new Item(4032101, (byte) 0, (short) 80, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
        }
        ...
        if(monster.getId() == 8800102) { // Zakum
            ...
            for (MaplePartyCharacter pchr : chr.getParty().getMembers()) {
                ...
                if (player.getMapId() == 280030000) {
                    ...
                    player.Message("You've gained " + q + " Zakum's Crystal(s)");
                } else
                    player.Message("You were not in the map, so no rewards sorry.");
        ...
        if(monster.getId() == 8810122) { // Horntail
            ...
            if (rand < 40)
                spawnMobDrop(new Item(4310034, (byte) 0, (short) 2, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);

            for (MaplePartyCharacter pchr : chr.getParty().getMembers()) {
                final MapleCharacter player = chr.getClient().getChannelServer().getPlayerStorage().getCharacterById(pchr.getId());
                int q = Randomizer.rand(1, 3);
                if (player.getMapId() == 240060201) {
        ...
        if(monster.getId() == 9101078) { // Flame Wolf
            ...
            if (rand < 40)
                spawnMobDrop(new Item(4310034, (byte) 0, (short) 4, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
        ...
        if(monster.getId() == 8870000) { // Hilla
            ...
            if (rand < 40)
                spawnMobDrop(new Item(4310034, (byte) 0, (short) 6, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);

            for (MaplePartyCharacter pchr : chr.getParty().getMembers()) {
                final MapleCharacter player = chr.getClient().getChannelServer().getPlayerStorage().getCharacterById(pchr.getId());
                int q = Randomizer.rand(1, 3);
                if (player.getMapId() == 262031300) {
        ...
        if(monster.getId() == 8840000) { // Von Leon
            ...
            if (rand < 40)
                spawnMobDrop(new Item(4310034, (byte) 0, (short) 8, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);

            for (MaplePartyCharacter pchr : chr.getParty().getMembers()) {
                final MapleCharacter player = chr.getClient().getChannelServer().getPlayerStorage().getCharacterById(pchr.getId());
                int q = Randomizer.rand(1, 3);
                if (player.getMapId() == 211070102) {
        ...
        if(monster.getId() == 8850011) { // Cygnus
            ...
            if (rand < 40)
                spawnMobDrop(new Item(4310034, (byte) 0, (short) 15, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
        ...
        if(monster.getId() == 8820212) { // Pink Bean
            ...
            if (rand < 40)
                spawnMobDrop(new Item(4310034, (byte) 0, (short) 40, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);

            for (MaplePartyCharacter pchr : chr.getParty().getMembers()) {
                final MapleCharacter player = chr.getClient().getChannelServer().getPlayerStorage().getCharacterById(pchr.getId());
                int q = Randomizer.rand(1, 3);
                if (player.getMapId() == 270051100) {
        ...
        if(monster.getId() == 8920000) { // Chaos Queen
            ...
            if (rand < 40)
                spawnMobDrop(new Item(4310034, (byte) 0, (short) 75, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);

            for (MaplePartyCharacter pchr : chr.getParty().getMembers()) {
                final MapleCharacter player = chr.getClient().getChannelServer().getPlayerStorage().getCharacterById(pchr.getId());
                int q = Randomizer.rand(1, 3);
                if (player.getMapId() == 105200710) {
        ...
        if(monster.getId() == 9801029) { // Lotus
            ...
            if (rand < 40)
                spawnMobDrop(new Item(4310034, (byte) 0, (short) 85, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);

            for (MaplePartyCharacter pchr : chr.getParty().getMembers()) {
                final MapleCharacter player = chr.getClient().getChannelServer().getPlayerStorage().getCharacterById(pchr.getId());
                int q = Randomizer.rand(1, 3);
                if (player.getMapId() == 350060200) {
        ...
        if(monster.getId() == 8880131) { // Damien
            ...
            if (rand < 40)
                spawnMobDrop(new Item(4310034, (byte) 0, (short) 90, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);


            for (MaplePartyCharacter pchr : chr.getParty().getMembers()) {
                final MapleCharacter player = chr.getClient().getChannelServer().getPlayerStorage().getCharacterById(pchr.getId());
                int q = Randomizer.rand(1, 3);
                if (player.getMapId() == 350160240) {
        ...
        if(monster.getId() == 8880150) { // Lucid
            ...
            if (rand < 40)
                spawnMobDrop(new Item(4310034, (byte) 0, (short) 95, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);
        ...
        if(monster.getId() == 9309207) { // Dorothy
            ...
            if (rand < 40)
                spawnMobDrop(new Item(4310034, (byte) 0, (short) 100, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);


            for (MaplePartyCharacter pchr : chr.getParty().getMembers()) {
                final MapleCharacter player = chr.getClient().getChannelServer().getPlayerStorage().getCharacterById(pchr.getId());
                int q = Randomizer.rand(1, 3);
                if (player.getMapId() == 992050000) {
        ...
        if(monster.getId() == 9440025) { // Cross
            ...
            if (rand < 40)
                spawnMobDrop(new Item(4310034, (byte) 0, (short) 115, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);

            for (MaplePartyCharacter pchr : chr.getParty().getMembers()) {
                final MapleCharacter player = chr.getClient().getChannelServer().getPlayerStorage().getCharacterById(pchr.getId());
                int q = Randomizer.rand(1, 3);
                if (player.getMapId() == 992040000) {
        ...
        if(monster.getId() == 8500002) { // Papulatus
            ...
            if (rand < 40)
                spawnMobDrop(new Item(4310034, (byte) 0, (short) 120, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);

            for (MaplePartyCharacter pchr : chr.getParty().getMembers()) {
                final MapleCharacter player = chr.getClient().getChannelServer().getPlayerStorage().getCharacterById(pchr.getId());
                int q = Randomizer.rand(1, 3);
                if (player.getMapId() == 220080001) {
        ...
        if(monster.getId() == 8880302) { // Will
            ...
            if (rand < 40)
                spawnMobDrop(new Item(4310034, (byte) 0, (short) 125, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);

            for (MaplePartyCharacter pchr : chr.getParty().getMembers()) {
                final MapleCharacter player = chr.getClient().getChannelServer().getPlayerStorage().getCharacterById(pchr.getId());
                int q = Randomizer.rand(1, 3);
                if (player.getMapId() == 450008350) {
        ...
        if(monster.getId() == 8880503) { // Black Mage
            ...
            if (rand < 40)
                spawnMobDrop(new Item(4310034, (byte) 0, (short) 200, (byte) 0), calcDropPos(new Point(monster.getPosition().x, monster.getPosition().y), monster.getPosition()), monster, chr, (byte) 1, (short) 0);

            for (MaplePartyCharacter pchr : chr.getParty().getMembers()) {
                final MapleCharacter player = chr.getClient().getChannelServer().getPlayerStorage().getCharacterById(pchr.getId());
                int q = Randomizer.rand(1, 3);
                if (player.getMapId() == 500000011) {
        ...
        if (chr.getQuestNoAdd(q) != null && chr.getQuestStatus(10546) == 1) {
        ...
        if (mobCollectionData != null) {
            if (Randomizer.nextInt(999999) <= 12000) {
    ...
    public MapleMonster makeEliteMonster(final MapleMonster monster) {
        ...
        ostats.setOExp(monster.getMobExp());
    ...
    private void time(final MapleCharacter chr) {
        MapTimer.getInstance().schedule(() -> chr.send(MainPacketCreator.removeMapEffect()), 5000L);
    }

    private void timeAllPlayer() {
        MapTimer.getInstance().schedule(() -> broadcastMessage(MainPacketCreator.removeMapEffect()), 5000L);
    }
            ...
            public final void run() {
                if (currentmap.isEliteBossMap()) {
                    MapleWorldMapProvider mwmp = ChannelServer.getInstance(player.getClient().getChannel())
                            .getMapFactory();
                    for (MapleMap target : mwmp.getMaps().values()) {
    ...
    public final void spawnTempNpc(final int id, final int x, final int y, final int owner) {
        ...
        npc.setRx0(x + 50);
        npc.setRx1(x - 50);
        npc.setFh(Objects.requireNonNull(getFootholds().findMaple(pos)).getId());
        npc.setTemp(true);
        addMapObject(npc);
    ...
    public final void removeNpc(final int id) {
        for (MapleMapObject mapleMapObject : getAllNPC()) {
    ...
    private void checkRemoveAfter(final MapleMonster monster) {
        ...
        if (ra > 0) {
            MapTimer.getInstance().schedule(() -> killMonster(monster), ra * 1000);
        }
    }
    ...
    public final void spawnMonsterWithEffect(final MapleMonster monster, final int effect, Point pos) {
        try {
            ...
            spawnAndAddRangedMapObject(monster, c -> c.getSession().writeAndFlush(MobPacket.spawnMonster(monster, -2, effect, 0,
                    GameConstants.isAswanMap(c.getPlayer().getMapId()))));
            updateMonsterController(monster);
    ...
    public final void spawnFakeMonster(final MapleMonster monster) {
        ...
        spawnAndAddRangedMapObject(monster, c -> c.getSession().writeAndFlush(MobPacket.spawnMonster(monster, -2, 0, 0, false)));
        updateMonsterController(monster);
    ...
    public final void spawnReactor(final MapleReactor reactor) {
        reactor.setMap(this);
        spawnAndAddRangedMapObject(reactor, c -> c.getSession().writeAndFlush(MainPacketCreator.spawnReactor(reactor)));
    }
    ...
    public final void spawnDragon(final MapleDragon summon) {
        spawnAndAddRangedMapObject(summon, c -> c.getSession().writeAndFlush(MainPacketCreator.spawnDragon(summon)));
    }

    public final void spawnSummon(final MapleSummon summon, final boolean animated, final int duration) {
        try {
            spawnAndAddRangedMapObject(summon, c -> c.getSession().writeAndFlush(MainPacketCreator.spawnSummon(summon, summon.getSkillLevel(), animated)));
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
    ...
    public final void spawnClockMist(final MapleMist clock) {
        spawnAndAddRangedMapObject(clock, c -> broadcastMessage(MainPacketCreator.spawnClockMist(clock)));
        MapTimer.getInstance().schedule(() -> {
    ...
    public final void spawnMist(final MapleMist mist, final int duration, boolean poison, boolean fake, boolean rv,
        ...
        mist.setMap(this);
        spawnAndAddRangedMapObject(mist, c -> c.sendPacket(MainPacketCreator.spawnMist(mist)));
        ...
        if (poison) {
            ...
            poisonSchedule = tMan.register(() -> {
                for (MapleMapObject mmo : getMapObjectsInRect(mist.getBox(),
                    ...
                    for (final MapleMapObject mistoo : chr.getMap().getMapObjectsInRange(chr.getPosition(),
                            Double.POSITIVE_INFINITY, Collections.singletonList(MapleMapObjectType.MIST))) {
                        final MapleMist check = (MapleMist) mistoo;
                        if (mist.getOwner() == check.getOwner() && mist.isTimeCapsule()) {
                            for (CoolDownValueHolder mcdvh : chr.getAllCooldowns()) {
            ...
            poisonSchedule = tMan.register(() -> mist.getSource().applyMonsterBuff(mist.getOwner()), 2000, 2500);
    ...
    public final void spawnMesoDrop(final int meso, final Point position, final MapleMapObject dropper,
        ...
        checkMaxItemInMap();
        spawnAndAddRangedMapObject(mdrop, c -> c.getSession().writeAndFlush(
    ...
    public final void spawnItemDrop(final MapleMapObject dropper, final MapleCharacter owner, final IItem item,
        ...
        spawnAndAddRangedMapObject(drop, c -> c.getSession().writeAndFlush(
    ...
    public boolean isLotusBoss() {
        return mapid == 350060160 || mapid == 350060180 || mapid == 350060200;
    }

    public void resetObtacleAtom() {
        if (!isLotusBoss()) {
            System.out.println("@MapleMap::resetObtacleAtom | not lotus boss map");
        }
        /*
    ...
    public final void addPlayer(final MapleCharacter chr) {
        ...
        if (chr.getKeyValue("fireaura2") != null) {
            Objects.requireNonNull(SkillFactory.getSkill(2121054)).getEffect(1).applyTo(chr, chr.getPosition());
        }
        if (chr.getKeyValue("scrue3") != null) {
            Objects.requireNonNull(SkillFactory.getSkill(65111007)).getEffect(1).applyTo(chr, chr.getPosition());
        }
        if (chr.getKeyValue("Donate_Trifle") != null) {
            Objects.requireNonNull(SkillFactory.getSkill(13120003)).getEffect(1).applyTo(chr, chr.getPosition());
        }
        if (chr.getKeyValue("magicg2") != null) {
            Objects.requireNonNull(SkillFactory.getSkill(2001002)).getEffect(1).applyTo(chr, chr.getPosition());
        }
        ...
        if (chr.getParty() != null) {
            ...
            chr.getClient().getSession().writeAndFlush(MainPacketCreator.updateParty(chr.getClient().getChannel(), chr.getParty(), MaplePartyOperation.SILENT_UPDATE, null));
        ...
        if (!chr.getSummons().isEmpty()) {
            for (Pair<Integer, MapleSummon> summon : chr.getSummons().values()) {
                if (summon.right.isStaticSummon()) {
                    ...
                    spawnSummon(summon.right, false, Objects.requireNonNull(SkillFactory.getSkill(summon.right.getSkill()))
    ...
    public final void removePlayer(final MapleCharacter chr) {
        ...
        List<MapleMapObject> removes = new ArrayList<>(this.getAllMist());
        ...
        if (getArrowFlatter(chr.getId()) != null) {
            chr.getMap().broadcastMessage(MainPacketCreator.cancelArrowFlatter(
                    Objects.requireNonNull(getArrowFlatter(chr.getId())).getObjectId(), Objects.requireNonNull(getArrowFlatter(chr.getId())).getArrow()));
            removeMapObject(Objects.requireNonNull(getArrowFlatter(chr.getId())));
        }
    ...
    public List<MapleWorldMapItem> getAllItemsThreadsafe() {
        ArrayList<MapleWorldMapItem> ret = new ArrayList<>();
    ...
    public final List<MapleMapObject> getAllPlayerNPC() {
        return getMapObjectsInRange(new Point(0, 0), Double.POSITIVE_INFINITY,
                Collections.singletonList(MapleMapObjectType.PLAYERNPC));
    }
    ...
    public final List<MapleMapObject> getAllDemianSword() {
        return getMapObjectsInRange(new Point(0, 0), Double.POSITIVE_INFINITY,
                Collections.singletonList(MapleMapObjectType.DemianSword));
    }
    ...
    public final List<MapleMapObject> getAllMist() {
        return getMapObjectsInRange(new Point(0, 0), Double.POSITIVE_INFINITY, Collections.singletonList(MapleMapObjectType.MIST));
    }
    ...
    public List<MapleMapObject> getAllHiredMerchant() {
        return getMapObjectsInRange(new Point(0, 0), Double.POSITIVE_INFINITY,
                Collections.singletonList(MapleMapObjectType.HIRED_MERCHANT));
    }
    ...
    public List<MapleMapObject> getAllSummon() {
        return getMapObjectsInRange(new Point(0, 0), Double.POSITIVE_INFINITY,
                Collections.singletonList(MapleMapObjectType.SUMMON));
    }

    public List<MapleMapObject> getAllRune() {
        return getMapObjectsInRange(new Point(0, 0), Double.POSITIVE_INFINITY, Collections.singletonList(MapleMapObjectType.RUNE));
    }
    ...
    public final void addMonsterSpawn(final MapleMonster monster, final int mobTime, final String msg) {
        ...
        monsterSpawn.add(new MobSpawnPoint(monster, newpos, mobTime, msg));
    }
    ...
    private class ExpireMapItemJob implements Runnable {
        ...
        public void run() {
            if (mapitem != null && mapitem == getMapObject(mapitem.getObjectId(), MapleMapObjectType.ITEM)) {
                if (droppedItems.contains(mapitem.getObjectId())) {
    ...
    public void respawn(final boolean force, final long now) {
        ...
        if (force) {
            ...
            if (numShouldSpawn > 0) {
                int spawned = 0;
                final List<MonsterSpawnEntry> randomSpawn = new ArrayList<>(monsterSpawn);
    ...
    public interface DelayedPacketCreation {
    ...
    public void startCatch() {
        if (catchstart == null) {
            ...
            catchstart = MapTimer.getInstance().schedule(() -> {
                broadcastMessage(
    ...
    public final List<ArrowFlatter> getArrowFlatterRange(final Point from, final double rangeSq) {
        ...
        final List<ArrowFlatter> arrows = new ArrayList<>();
        for (MapleMapObject mapobject : mapobjects) {
            if (mapobject.getType() == MapleMapObjectType.ARROWFLATTER) {
                arrows.add((ArrowFlatter) mapobject);
            }
        }
        return arrows;
    }
    ...
    public final void spawnArrowFlatter(final ArrowFlatter arrow) {
        spawnAndAddRangedMapObject(arrow, c -> {
    ...
    public final void spawnDemianSword(final DemianSword sword) {
        spawnAndAddRangedMapObject(sword, c -> {
    ...
    public final void resetSpawnLevel(int level) {
        for (MonsterSpawnEntry spawn : monsterSpawn) {
            if (spawn instanceof MobSpawnPoint) {
                ((MobSpawnPoint) spawn).setLevel(level);
            }
        }
    }
