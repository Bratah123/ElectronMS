...
public class EventManager {
    ...
    public EventInstanceManager readyInstance() {
        try {
            return (EventInstanceManager) (iv.invokeFunction("setup", (Object) null));
        } catch (ScriptException | NoSuchMethodException ex) {
            ex.printStackTrace();
        }
    ...
    public void warpAllPlayer(int from, int to) {
        ...
        if (tomap != null && frommap.getCharactersSize() > 0) {
    ...
    public boolean scheduleRandomEvent() {
        boolean event = false;
        for (int value : eventChannel) {
            event |= scheduleRandomEventInChannel(value);
        }
        return event;
    }
