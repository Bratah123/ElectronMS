 ...
 */
package server.Events.EventManager;
...
public class EventInstanceManager {
    ...
    private void unregisterPlayer_NoLock(final MapleCharacter chr) {
    ...
    public void monsterKilled(final MapleCharacter chr, final MapleMonster mob) {
        ...
        try {
            ...
            System.out.println("Event name" + em.getName() + ", Instance name : " + name + ", method Name : monsterValue:\n" + ex);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
    ...
    public void applyBuff(final MapleCharacter chr, final int id) {
        Objects.requireNonNull(ItemInformation.getInstance().getItemEffect(id)).applyTo(chr);
    ...
    public void applySkill(final MapleCharacter chr, final int id) {
        Objects.requireNonNull(SkillFactory.getSkill(id)).getEffect(1).applyTo(chr);
    }
