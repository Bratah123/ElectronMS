...
public class StarForceSystem {
        ...
        switch (a) {
            case 0:
                ...
                if (scroll.startsWith("순백")) {
                    ...
                    per = Integer.parseInt(scroll.split("%")[0]); // % 이전 맨 앞에서 값 출력
                }
                if (c.getPlayer().isGM()) {
                    per = 100;
                }
    ...
    public static Pair<String, Boolean> isSuperial(int itemid) {
        if ((itemid >= 1102471 && itemid <= 1102475) || (itemid >= 1072732 && itemid <= 1072736)
            ...
            return new Pair<>("MindPendent", true);
        }
    ...
    public static void StarForceStat(MapleClient c, Equip item, WritingPacket packet, int enhance) {
        c.getPlayer().enchant().removeAll(c.getPlayer().enchant());
        int stats = 0;
        MapleWeaponType a;
        final ItemInformation ii = ItemInformation.getInstance();
        final MapleData IData = ii.getItemData(item.getItemId());
        ...
        if (superial.getRight()) {
            if (enhance >= 5 && enhance < 15) {
                switch (superial.getLeft()) {
                    case "Helisium":
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.WATK, 3 + AddAllSmalls(enhance - 5)));
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.MATK, 3 + AddAllSmalls(enhance - 5)));
                        break;
                    case "Nova":
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.WATK, 6 + AddAllSmalls(enhance - 5)));
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.MATK, 6 + AddAllSmalls(enhance - 5)));
                        break;
                    case "Tilent":
                    case "MindPendent":
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.WATK, 9 + AddAllSmalls(enhance - 5)));
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.MATK, 9 + AddAllSmalls(enhance - 5)));
                        break;
                }
            } else {
                switch (superial.getLeft()) {
                    case "Helisium":
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.STR, 5 + AddAllSmalls(enhance)));
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.DEX, 5 + AddAllSmalls(enhance)));
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.INT, 5 + AddAllSmalls(enhance)));
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.LUK, 5 + AddAllSmalls(enhance)));
                        break;
                    case "Nova":
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.STR, 10 + AddAllSmalls(enhance)));
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.DEX, 10 + AddAllSmalls(enhance)));
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.INT, 10 + AddAllSmalls(enhance)));
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.LUK, 10 + AddAllSmalls(enhance)));
                        break;
                    case "Tilent":
                    case "MindPendent":
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.STR, 20 + AddAllSmalls(enhance)));
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.DEX, 20 + AddAllSmalls(enhance)));
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.INT, 20 + AddAllSmalls(enhance)));
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.LUK, 20 + AddAllSmalls(enhance)));
                        break;
                }
            }
        } else {
            if (GameConstants.isWeapon(item.getItemId())) {
                if (Objects.requireNonNull(ItemInformation.getInstance().getEquipStats(item.getItemId())).containsKey("reqLevel") && ItemInformation.getInstance().getEquipStats(item.getItemId()).get("reqLevel") > 100) {
                    if (enhance < 15) {
                        c.getPlayer().enchant().add(new Pair<>(Atk(item), Atk(item) == EnchantEquipStats.WATK ? ((item.getWatk() / 50) + 1) : ((item.getMatk() / 50) + 1)));
                    } else if (enhance < 22) {
                        c.getPlayer().enchant().add(new Pair<>(Atk(item), enhance - 7));
                    } else {
                       
                        switch (enhance) {
                            case 22:
                                c.getPlayer().enchant().add(new Pair<>(Atk(item), 31));
                                break;
                            case 23:
                                c.getPlayer().enchant().add(new Pair<>(Atk(item), 62));
                                break;
                            case 24:
                                c.getPlayer().enchant().add(new Pair<>(Atk(item), 124));
                                break;
                        }
                    }
                    if (enhance >= 4 && enhance < 10) {
                        if (Stat1(item, Job) != null) {
                            c.getPlayer().enchant().add(new Pair<>(Stat1(item, Job), 5));
                        }
                        if (Stat2(item, Job) != null) {
                            c.getPlayer().enchant().add(new Pair<>(Stat2(item, Job), 5));
                        }
                        if (Stat3(item, Job) != null) {
                            c.getPlayer().enchant().add(new Pair<>(Stat3(item, Job), 5));
                        }
                        if (Stat4(item, Job) != null) {
                            c.getPlayer().enchant().add(new Pair<>(Stat4(item, Job), 5));
                        }
                    } else if (enhance >= 10 && enhance < 15) {
                        if (Stat1(item, Job) != null) {
                            c.getPlayer().enchant().add(new Pair<>(Stat1(item, Job), 7));
                        }
                        if (Stat2(item, Job) != null) {
                            c.getPlayer().enchant().add(new Pair<>(Stat2(item, Job), 7));
                        }
                        if (Stat3(item, Job) != null) {
                            c.getPlayer().enchant().add(new Pair<>(Stat3(item, Job), 7));
                        }
                        if (Stat4(item, Job) != null) {
                            c.getPlayer().enchant().add(new Pair<>(Stat4(item, Job), 7));
                        }
                    } else if (enhance >= 15 && enhance < 20) {
                        if (Stat1(item, Job) != null) {
                            c.getPlayer().enchant().add(new Pair<>(Stat1(item, Job), 15));
                        }
                        if (Stat2(item, Job) != null) {
                            c.getPlayer().enchant().add(new Pair<>(Stat2(item, Job), 15));
                        }
                        if (Stat3(item, Job) != null) {
                            c.getPlayer().enchant().add(new Pair<>(Stat3(item, Job), 15));
                        }
                        if (Stat4(item, Job) != null) {
                            c.getPlayer().enchant().add(new Pair<>(Stat4(item, Job), 15));
                        }
                    } else if (enhance >= 20 && enhance < 25) {
                        if (Stat1(item, Job) != null) {
                            c.getPlayer().enchant().add(new Pair<>(Stat1(item, Job), 30));
                        }
                        if (Stat2(item, Job) != null) {
                            c.getPlayer().enchant().add(new Pair<>(Stat2(item, Job), 30));
                        }
                        if (Stat3(item, Job) != null) {
                            c.getPlayer().enchant().add(new Pair<>(Stat3(item, Job), 30));
                        }
                        if (Stat4(item, Job) != null) {
                            c.getPlayer().enchant().add(new Pair<>(Stat4(item, Job), 30));
                        }
                    } else {
                        if (Stat1(item, Job) != null) {
                            c.getPlayer().enchant().add(new Pair<>(Stat1(item, Job), 3));
                        }
                        if (Stat2(item, Job) != null) {
                            c.getPlayer().enchant().add(new Pair<>(Stat2(item, Job), 3));
                        }
                        if (Stat3(item, Job) != null) {
                            c.getPlayer().enchant().add(new Pair<>(Stat3(item, Job), 3));
                        }
                        if (Stat4(item, Job) != null) {
                            c.getPlayer().enchant().add(new Pair<>(Stat4(item, Job), 3));
                        }
                    }
                    if (Stat1(item, Job) != EnchantEquipStats.HP && Stat2(item, Job) != EnchantEquipStats.HP && Stat3(item, Job) != EnchantEquipStats.HP && Stat4(item, Job) != EnchantEquipStats.HP) {
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.HP, 10));
                    }
                    c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.MP, 10));
                } else {
                    c.getPlayer().enchant().add(new Pair<>(Atk(item), Atk(item) == EnchantEquipStats.WATK ? ((item.getWatk() / 50) + 1) : ((item.getMatk() / 50) + 1)));
                    if (Stat1(item, Job) != EnchantEquipStats.HP && Stat2(item, Job) != EnchantEquipStats.HP && Stat3(item, Job) != EnchantEquipStats.HP && Stat4(item, Job) != EnchantEquipStats.HP) {
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.HP, 5));
                    }
                    c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.MP, 5));
                    if (Stat1(item, Job) != null) {
                        c.getPlayer().enchant().add(new Pair<>(Stat1(item, Job), 2));
                    }
                    if (Stat2(item, Job) != null) {
                        c.getPlayer().enchant().add(new Pair<>(Stat2(item, Job), 2));
                    }
                    if (Stat3(item, Job) != null) {
                        c.getPlayer().enchant().add(new Pair<>(Stat3(item, Job), 2));
                    }
                    if (Stat4(item, Job) != null) {
                        c.getPlayer().enchant().add(new Pair<>(Stat4(item, Job), 2));
                    }
                }
            } else {
                if (Objects.requireNonNull(ItemInformation.getInstance().getEquipStats(item.getItemId())).get("reqLevel") > 100) {
                    if (enhance >= 4 && enhance < 10) {
                        c.getPlayer().enchant().add(new Pair<>(Stat1(item, Job), 5));
                        c.getPlayer().enchant().add(new Pair<>(Stat2(item, Job), 5));
                        if (Stat3(item, Job) != null) {
                        ...
                        if (enhance >= 17) {
                            c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.WATK, enhance - 4));
                            c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.MATK, enhance - 4));
                        }
                        c.getPlayer().enchant().add(new Pair<>(Stat1(item, Job), 15));
                        c.getPlayer().enchant().add(new Pair<>(Stat2(item, Job), 15));
                        if (Stat3(item, Job) != null) {
                        ...
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.WATK, enhance + 2));
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.MATK, enhance + 2));
                        c.getPlayer().enchant().add(new Pair<>(Stat1(item, Job), 30));
                        c.getPlayer().enchant().add(new Pair<>(Stat2(item, Job), 30));
                        if (Stat3(item, Job) != null) {
                            c.getPlayer().enchant().add(new Pair<>(Stat3(item, Job), 30));
                    ...
                    if (Stat1(item, Job) != EnchantEquipStats.HP && Stat2(item, Job) != EnchantEquipStats.HP && Stat3(item, Job) != EnchantEquipStats.HP && Stat4(item, Job) != EnchantEquipStats.HP) {
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.HP, 50));
                    }
                    c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.MP, 50));
                    if (enhance >= 4 && enhance < 10) {
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.WDEF, 15));
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.MDEF, 15));
                    } else if (enhance >= 10 && enhance < 15) {
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.WDEF, 25));
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.MDEF, 25));
                    } else if (enhance >= 15 && enhance < 20) {
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.WDEF, 55));
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.MDEF, 55));
                    } else if (enhance >= 20 && enhance < 25) {
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.WDEF, 95));
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.MDEF, 95));
                    } else {
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.WDEF, 9));
                        c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.MDEF, 9));
                    }
                } else {
                    c.getPlayer().enchant().add(new Pair<>(Stat1(item, Job), 2));
                    c.getPlayer().enchant().add(new Pair<>(Stat2(item, Job), 2));
                    ...
                    c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.MP, 5));
                    c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.WDEF, 5));
                    c.getPlayer().enchant().add(new Pair<>(EnchantEquipStats.MDEF, 5));
                }
            }
        }
        for (int i = 0; i < c.getPlayer().enchant().size(); i++) {
            stats |= c.getPlayer().enchant().get(i).left.getValue();
        }
        if (packet != null) {
            packet.writeInt(stats);
            for (int i = 0; i < c.getPlayer().enchant().size(); i++) {
                packet.writeInt(c.getPlayer().enchant().get(i).getRight());
            }
        }
    ...
    public static void StarSystemUpgrade(Equip equip, int success, MapleCharacter chr) {
        if (success == 1) {
            for (int i = 0; i < chr.enchant().size(); i++) {
                switch (chr.enchant().get(i).getLeft().getValue()) {
                    case 0x1:
                        equip.setWatk((short) (equip.getWatk() + chr.enchant().get(i).getRight()));
                        break;
                    case 0x2:
                        equip.setMatk((short) (equip.getMatk() + chr.enchant().get(i).getRight()));
                        break;
                    case 0x4:
                        equip.setStr((short) (equip.getStr() + chr.enchant().get(i).getRight()));
                        break;
                    case 0x8:
                        equip.setDex((short) (equip.getDex() + chr.enchant().get(i).getRight()));
                        break;
                    case 0x10:
                        equip.setInt((short) (equip.getInt() + chr.enchant().get(i).getRight()));
                        break;
                    case 0x20:
                        equip.setLuk((short) (equip.getLuk() + chr.enchant().get(i).getRight()));
                        break;
                    case 0x40:
                        equip.setWdef((short) (equip.getWdef() + chr.enchant().get(i).getRight()));
                        break;
                    case 0x80:
                        equip.setMdef((short) (equip.getMdef() + chr.enchant().get(i).getRight()));
                        break;
                    case 0x100:
                        equip.setHp((short) (equip.getHp() + chr.enchant().get(i).getRight()));
                        break;
                    case 0x200:
                        equip.setMp((short) (equip.getMp() + chr.enchant().get(i).getRight()));
                        break;
                    case 0x400:
                        equip.setAcc((short) (equip.getAcc() + chr.enchant().get(i).getRight()));
                        break;
                    case 0x800:
                        equip.setAvoid((short) (equip.getAvoid() + chr.enchant().get(i).getRight()));
                        break;
                }
            }
    ...
    public static void StarSystem(MapleCharacter chr, Equip equip, boolean isChance) {
        ...
        if (Randomizer.isSuccess(success)) {
            ...
            } else if (FailAndDown > 0) {// If there's a chance to go down
                if (zeros != null) {
                    StarSystemUpdate(equip, 0, chr, true);
                }
                StarSystemUpdate(equip, 0, chr, false);
            } else if (FailAndDestroy == 0) {// If grade is maintained
                if (zeros != null) {
                    StarSystemUpdate(equip, 3, chr, true);
                }
                StarSystemUpdate(equip, 3, chr, false);
            } else {// If there is a probability of popping
    ...
    public static void Itemupgrade(MapleClient c, Equip item, int scrollnumber, boolean success, MapleCharacter chr, boolean zeroWeapon) {//숫자별로
        ...
        if (success) {
        	if (scrollstring[scrollnumber].startsWith("이노센트")) {
                ...
                chr.gainItem(spelltrace, (short) -3000, false, 0, null);
                return;
        	} else {
            percent = Integer.parseInt(scrollstring[scrollnumber].split("%")[0]);
                switch (value) {
                	case "힘":
                		if (percent == 100) {
                			item.addStr((short) 3);
        ...
        if (!zeroWeapon) {
            chr.gainItem(spelltrace, (short) -usejuhun[percent == 100 ? 0 : percent == 70 ? 1 : percent == 30 ? 2 : 3], false, 0, null);
        }
    }
    ...
    public static byte[] AddItemOnSystem(MapleClient c, Equip item, int a, int FeverTime) {
        ...
        try {
            ...
            for (String s : scrollstring) {
                packet.writeInt(scrolltype(s));
                packet.writeMapleAsciiString(s);
                packet.writeInt(s.contains("순백") ? 2 : s.contains("이노센트") ? 1 : 0); // 특수 주문서
                packet.writeInt((s.contains("이노센트") || s.contains("순백")) ? 1 : 0); // 특수 주문서
                packet.writeInt(scrollstatic(s));
                switch (scrollstatic(s)) {
                    case 0: // Whiteness, Ino
                        break;
                    case 1: // ATT
                    case 2: // M.ATT
                    case 64: // Defense
                        packet.writeInt(s.startsWith("100%") || (itemtype == 108 && s.startsWith("70%")) ? 2 : 3);
                        break;
                    case 5: // Weapon (STR)
                    case 9: // Weapon (Dex)
                    case 33: // Weapons (Luck)
                        packet.writeInt(s.startsWith("30%") ? 7 : 9); // ATT
                        packet.writeInt(s.startsWith("30%") ? (s.contains("HP") ? 150 : 3) : (s.contains("HP") ? 200 : 4)); // Juice stat
                        break;
                    default:
                        packet.writeInt(s.startsWith("100%") ? 3 : s.startsWith("70%") ? 4 : 7);
                        packet.writeInt(s.startsWith("100%") ? 30 : s.startsWith("70%") ? 70 : 120);
                        if (scrollstatic(s) != 320)
                            packet.writeInt(s.startsWith("100%") ? 3 : s.startsWith("70%") ? 5 : 10);
                        break;
                }
                packet.writeInt(scrolljuhun(s));
                packet.writeInt(scrolljuhun(s));
                packet.write(s.startsWith("100%") ? 1 : 0);
            }
    	} catch (Exception e) {
    		e.printStackTrace();
    	}
