...
public class PotSystem {
    ...
    private static final Map<Integer, Pair<Integer, Integer>> pots = new HashMap<>();
    ...
    public static void addPot(int guildid, int rid, int exp) {
        synchronized (pots) {
            pots.put(guildid, new Pair<>(rid, exp));
        }
    }
        ...
        try {
            synchronized (pots) {
                ...
                if (pots.containsKey(guildid)) {
                    ...
                    if (potexp < 1000) {
                        int newExp = potexp + gain;
                        if (newExp > 1000) {
                            newExp = 1000;
                        }
                        pots.remove(guildid);
                        pots.put(guildid, new Pair<>(potid, newExp));
