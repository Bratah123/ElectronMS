...
public class AuctionHandler {
    ...
    public static final void Handle(final ReadingMaple rh, final MapleClient c, int op) {
        switch (op) {
            ...
            case 10: { // 아이템 등록
                ...
                Item item = (Item) c.getPlayer().getInventory(Objects.requireNonNull(MapleInventoryType.getByType(inv))).getItem(slot), copyItem;
            ...
            case 12: { // Unregister Item
                ...
                if (status != 1) {
                    item.setType(status == 3 ? item.getType(isOnwer, true) == 17 ? 28 : 18 : status == 4 ? 4 : 0);
                    item.setBid(0);
                }
                c.getSession().writeAndFlush(AuctionPacket.AuctionSell(WorldAuction.getSellItems(c.getPlayer().getId())));
		...
		ItemInformation ii = ItemInformation.getInstance();
                ...
                for (AuctionItemPackage aip : allItem) {
                    if (Objects.requireNonNull(ii.getName(aip.getItem().getItemId())).contains(str1) || Objects.requireNonNull(ii.getName(aip.getItem().getItemId())).contains(str2))
                        filter.add(aip);
                    if (filter.size() == 100)
                        break;
                }
    ...
    public static class WorldAuction {
        ...
        public static boolean isPremiumUser(MapleCharacter player) {
        ...
        public static void addItem(final AuctionItemPackage aitem) {
            aitem.getItem().setInventoryId(items.size() + 1);
            items.add(aitem);
        }

        public static AuctionItemPackage findByIid(final int id) {
