...
import server.CustomEvents.MapleMiniGame;
import tools.Pair;
...
public class PlayerInteractionHandler
{
	...
	public static void PlayerInteraction(final ReadingMaple rh, MapleClient c, final MapleCharacter chr)
	{
		...
		case CREATE:
		{
			...
			{ // trade
				MapleUserTrade.startTrade(chr, createType == 4, createType);
			}
		...
		case CHAT:
		{
			...
			else if (chr.getPlayerShop() != null)
			{
				...
				{
					ips.getMessages().add(new Pair<>(message, slot));
				}
		...
		case SET_ITEMS1:
		{
			...
			assert ivType != null;
			final IItem item = chr.getInventory(ivType).getItem(rh.readShort());
		...
		{ // Buy and Merchant buy
			...
			if (/* check > 32767 || */check <= 0 || check2 <= 0 || check3 > 32767 || check3 <= 0)
		...
		{
			final IMapleCharacterShop shop = chr.getPlayerShop();
			if (shop instanceof HiredMerchant && shop.isOwner(chr))
			{
	...
	public static byte getResult(byte rps1, byte rps2)
	{
		switch (rps1)
		{
		case 0:
