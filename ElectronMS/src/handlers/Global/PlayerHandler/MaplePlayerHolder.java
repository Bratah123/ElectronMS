...
public class MaplePlayerHolder {
    ...
    private final Map<Integer, Object> effects = new HashMap<>();
 ...
 try {
     ...
     effects.computeIfAbsent(chr.getId(), k -> new Object());
 } finally {
     mutex.unlock();
 }
    }
 ...
 try {
     for (MapleCharacter mapleCharacter : nameToChar.values()) {
         mapleCharacter.getClient().getSession().writeAndFlush(data);
     }
 } finally {
     mutex.unlock();
 }
