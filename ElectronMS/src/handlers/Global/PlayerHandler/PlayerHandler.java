...
import client.Skills.VMatrix.MatrixSkill;
...
import handlers.Global.AttackInfo;
import handlers.Global.AttackType;
import server.Events.AswanHandler;
import server.Events.FishingHandler;
import handlers.Global.MovementParse;
...
import client.ItemInventory.Items.MakerItemFactory;
import server.LifeEntity.Mob.MobAttackInfo;
import server.LifeEntity.Mob.MobAttackInfoFactory;
...
import server.LifeEntity.Mob.SummonAttackEntry;
...
public class PlayerHandler {
    ...
    public static void absorbingDF(ReadingMaple rh, MapleClient c) {
        ...
        for (int j = 0; j < v1; j++) {
            ...
            switch (skillid) {
                case 65111007:
                    if (Objects.requireNonNull(SkillFactory.getSkill(65111007)).getEffect(c.getPlayer().getSkillLevel(65111007)).makeChanceResult()) {
                        c.getPlayer().getMap().broadcastMessage(AngelicBusterSkill.SoulSeekerRegen(c.getPlayer(), m1));
                    }
                    break;
                case 65120011:
                    if (Objects.requireNonNull(SkillFactory.getSkill(65120011)).getEffect(c.getPlayer().getSkillLevel(65120011)).makeChanceResult()) {
                        c.getPlayer().getMap().broadcastMessage(AngelicBusterSkill.SoulSeekerRegen(c.getPlayer(), m1));
                    }
                    break;
                case 31221014:
    ...
    public static void ArrowFlatterAction(ReadingMaple rh, final MapleCharacter chr) {
        ...
        chr.addCooldown(3111013, System.currentTimeMillis(),
                Objects.requireNonNull(SkillFactory.getSkill(3111013)).getEffect(chr.getSkillLevel(3111013)).getCooldown() * 1000);
    ...
    public static void TakeDamage(ReadingMaple rh, MapleClient c, MapleCharacter chr) throws InterruptedException {
        ...
        if (type != -2 && type != -3 && type != -4 && type != -6) { // Not map damage
            ...
            if (chr.getBuffedSkillEffect(BuffStats.RoyalGuardPrepare) != null) {
                ...
                SkillStatEffect eff = Objects.requireNonNull(SkillFactory.getSkill(51001005)).getEffect(1);
                eff.applyTo(chr);
            ...
            if (chr.getJob() >= 433 && chr.getJob() <= 434 && damage != -1) {
                if (chr.getSkillLevel(4330009) > 0) {
                    if (Randomizer.rand(1, 100) <= Objects.requireNonNull(SkillFactory.getSkill(4330009)).getEffect(chr.getSkillLevel(4330009))
                            .getER()) {
                        damage = -1;
                        Objects.requireNonNull(SkillFactory.getSkill(4330009)).getEffect(chr.getSkillLevel(4330009)).applyTo(chr);
                    }
                }
            }
            if (damage != -1) {
                    ...
                    for (Pair<Integer, MapleSummon> s : chr.getSummons().values()) {
                        if (s.getLeft() == v1) {
                            if (attacker != null){
                                ...
                                List<Long> dmg = new ArrayList<>();
                                dmg.add((long) damage * 13);
                                allDamage.add(new SummonAttackEntry(attacker.getObjectId(), dmg));
                                SkillStatEffect e = Objects.requireNonNull(SkillFactory.getSkill(v1)).getEffect(chr.getSkillLevel(v1));
        ...
        if (GameConstants.isZero(chr.getJob())) {
            if (chr.getGender() == 0 && chr.getSecondGender() == 1) {
                if (chr.getSkillLevel(101120109) > 0) {
                    if (!chr.Barrier()) {
                        SkillStatEffect effects = Objects.requireNonNull(SkillFactory.getSkill(101120109)).getEffect(chr.getSkillLevel(101120109));
                        damage = -1;
                        effects.applyTo(chr);
                        chr.setBarrier(true);
                        Timer.MapTimer.getInstance().schedule(() -> chr.setBarrier(false), 1000 * 30);
                    }
                }
            }
        }
        ...
        if (chr.getBuffedValue(BuffStats.BlessingArmor, 1210016) != null) {
            int guardCount = chr.getBuffedValue(BuffStats.BlessingArmor, 1210016);
            if (guardCount == 0) {
        ...
        if (damage == 0) { // Guard
            if (chr.getSkillLevel(31110008) > 0) {
                SkillStatEffect effs = Objects.requireNonNull(SkillFactory.getSkill(31110008)).getEffect(chr.getSkillLevel(31110008));
        ...
        if (damage > 0) {
                ...
                if (chr.getBuffedValue(BuffStats.ACC) != null) {
                    ...
                    if (attacker != null) {
                        ...
                        if (GameConstants.isDemonSlayer(chr.getJob())) { // Dark Revenge Effect
                            if (chr.getSkillLevel(31101003) > 0) {
                                SkillStatEffect skills = Objects.requireNonNull(SkillFactory.getSkill(31101003))
                    ...
                    if (attacker != null) {
                        ...
                        if (chr.getBlessOfDark() == 0) {
                            chr.cancelEffect(Objects.requireNonNull(SkillFactory.getSkill(27100003)).getEffect(1), false, -1);
                        } else {
                            Objects.requireNonNull(SkillFactory.getSkill(27100003)).getEffect(chr.getSkillLevel(27100003)).applyTo(chr);
                        }
                    }
            ...
            if (chr.getBuffedValue(BuffStats.KinesisPsychicEnergeShield, 142001007) != null) {
                SkillStatEffect eff = Objects.requireNonNull(SkillFactory.getSkill(142001007)).getEffect(chr.getSkillLevel(142001007));
                int lessDaMper = eff.getX();
                if (chr.PPoint > 0) {
                    chr.PPoint--;
            ...
            if (chr.getBuffedValue(BuffStats.IndieDamR, 400011066) != null) {
                ...
                SkillStatEffect eff = Objects.requireNonNull(SkillFactory.getSkill(400011066)).getEffect(level);
                int maxStack = 10;
            ...
            if (chr.getSkillLevel(1210016) > 0 && (chr.getJob() == 121 || chr.getJob() == 122)) { // Blessing Armor
                SkillStatEffect effect = Objects.requireNonNull(SkillFactory.getSkill(1210016)).getEffect(chr.getSkillLevel(1210016));
        ...
        if (guardianSpiritActivated) {
            ...
            MapleMonster gsMob = chr.getMap().getMonsterByOid(gsOid);
            assert gsMob != null;
            if (!gsMob.getStats().isBoss()) {
                gsMob.applyStatus(chr,
                        ...
                        Objects.requireNonNull(SkillFactory.getSkill(1220006)).getEffect(chr.getSkillLevel(1220006)).getStatusDuration());
            }
        }
        if (!chr.isHidden()) {
    ...
    public static void CancelBuffHandler(int sourceid, MapleCharacter chr) {
        ...
        chr.getEffects().entrySet().stream().forEach((Map.Entry<BuffStats, List<BuffStatsValueHolder>> effect) -> {
        ...
        if (sourceid == 400041009) {
            ...
            if (jokerSkill == 400041015) {
                Objects.requireNonNull(SkillFactory.getSkill(400041011)).getEffect(chr.getSkillLevel(400041009)).applyTo(chr);
                Objects.requireNonNull(SkillFactory.getSkill(400041012)).getEffect(chr.getSkillLevel(400041009)).applyTo(chr);
                Objects.requireNonNull(SkillFactory.getSkill(400041013)).getEffect(chr.getSkillLevel(400041009)).applyTo(chr);
                Objects.requireNonNull(SkillFactory.getSkill(400041014)).getEffect(chr.getSkillLevel(400041009)).applyTo(chr);
            } else {
                Objects.requireNonNull(SkillFactory.getSkill(jokerSkill)).getEffect(chr.getSkillLevel(400041009)).applyTo(chr);
            }
    ...
    public static void SkillEffect(ReadingMaple rh, MapleCharacter chr) {
        ...
        if (skilllevel_serv > 0 && skilllevel_serv == entry.getLevel() && Objects.requireNonNull(skill).isChargeSkill() && entry.getLevel() > 0 || Objects.requireNonNull(skill).getId() == 35101009 || skill.getId() == 22171083 || skill.getId() == 400051334) {
            chr.setKeyDownSkill_Time(System.currentTimeMillis());
            chr.setSkillEffect(entry);

            if (skill.getId() == 155121341 || skill.getId() == 400051334 || skill.getId() == 155111306) {
                Map<SkillStatEffect, Long> bsvhs = new HashMap<>();
                chr.getEffects().entrySet().stream().forEach((Map.Entry<BuffStats, List<BuffStatsValueHolder>> effect1) -> effect1.getValue().stream().filter((bsvh) -> (!bsvhs.containsKey(bsvh.effect) && bsvh.effect.getSourceId() == 155101006)).forEach((bsvh) -> {
                    bsvhs.put(bsvh.effect, bsvh.startTime);
                }));
                if (bsvhs.size() == 0) {
                    Objects.requireNonNull(SkillFactory.getSkill(155101006)).getEffect(chr.getSkillLevel(155101006)).applyTo(chr);
                }
            }
            if (skill.getId() == 400011072) {
                Objects.requireNonNull(SkillFactory.getSkill(400011072)).getEffect(chr.getSkillLevel(400011072)).applyTo(chr);
            }
            if (skill.getId() == 22171083) {
                Objects.requireNonNull(SkillFactory.getSkill(22171083)).getEffect(chr.getSkillLevel(22171080)).applyTo(chr);
            }

            if (skill.getId() == 27101202 || skill.getId() == 35001001 || skill.getId() == 35101009 || skill.getId() == 400041009) {
                Objects.requireNonNull(SkillFactory.getSkill(400041009)).getEffect(chr.getSkillLevel(400041009)).applyTo(chr);
    ...
    public static void SpecialSkill(ReadingMaple rh, final MapleClient c, final MapleCharacter chr) {
        ...
        SkillStatEffect effect = Objects.requireNonNull(SkillFactory.getSkill(GameConstants.getLinkedBuffSkill(skillid))).getEffect(skillLevel);
        ...
        if (skillid == 400051010) {
            for (CoolDownValueHolder m : chr.getAllCooldowns()) {
        ...
        switch (skillid) {
            case 400031040: {
                ...
                SkillStatEffect effects = Objects.requireNonNull(SkillFactory.getSkill(400031040)).getEffect(400031040);
            ...
            case 22170070:
            case 22141017: {
                SkillStatEffect eff = Objects.requireNonNull(SkillFactory.getSkill(skillid)).getEffect(skillid);
                int max = eff.getV();
                final List<MapleMapObject> objs = chr.getMap().getMapObjectsInRange(chr.getTruePosition(), 200000, Collections.singletonList(MapleMapObjectType.MONSTER));
            ...
            case 21121058: {
                Objects.requireNonNull(SkillFactory.getSkill(21110016)).getEffect(chr.getSkillLevel(21110016)).applyTo(chr);
            ...
            case 3321003: {
                ...
                List<MapleMapObject> objs = c.getPlayer().getMap().getMapObjectsInRange(c.getPlayer().getTruePosition(), 200000, Collections.singletonList(MapleMapObjectType.MONSTER));
                rh.skip(13);
                Point xy = rh.readPos();
            ...
            case 400021030: { // Thunder brake
                final SkillStatEffect subEffect = Objects.requireNonNull(SkillFactory.getSkill(400021031)).getEffect(skillLevel);
            ...
            case 400041000: { // Venom Burst
                ...
                for (MapleMapObject mobs_object : mobs_objects) {
                    MapleMonster mob = (MapleMonster) mobs_object;
                ...
                for (MapleMonster mob : mobs) {
                    finalMobList.add(new Triple<>(mob.getObjectId(), 0, test));
                    test++;
                }
            ...
            case 400011016: {
                ...
                maha.removeMist();
                Objects.requireNonNull(SkillFactory.getSkill(400011016)).getEffect(skillLevel).applyTo(chr);
            ...
            case 400011015: { // Limit brake
                // Reducing Cool Time
                for (CoolDownValueHolder cooltime : new LinkedList<>(chr.getAllCooldowns())) {
            ...
            case 12101022: { // Burn and Rest
                int a = Objects.requireNonNull(SkillFactory.getSkill(12101022)).getEffect(chr.getSkillLevel(12101022)).getX();
            ...
            case 4211006: { // Meso Explosion
                rh.skip(3);
                List<MapleMapObject> drops = c.getPlayer().getMap().getMapObjectsInRange(c.getPlayer().getPosition(), 320000, Collections.singletonList(MapleMapObjectType.ITEM));
                ...
                final List<MapleMapObject> mobjects = c.getPlayer().getMap().getMapObjectsInRange(c.getPlayer().getPosition(), 320000, Collections.singletonList(MapleMapObjectType.MONSTER));
                final List<Integer> moids = new ArrayList<>();
                final int randmob_count = Randomizer.rand(mobjects.isEmpty() ? 0 : 1, Math.min(mobjects.size(), 10));
            ...
            case 33001016: {
                ...
                if (skillid == 33001025) {
                    ...
                    for (MapleMonster mob2 : monsters) {
                        SkillStatEffect eff = Objects.requireNonNull(SkillFactory.getSkill(33001025)).getEffect(chr.getSkillLevel(33001025));
        ...
        if (skill != null) {
            ...
            switch (skillid) {
                ...
                case 12111022: { //Maelstroom
                    ...
                    summon.setMaelstromId(Objects.requireNonNull(chr.getMap().getMonsterByOid(mobid)).getId());
                ...
                case 155101006: {
                    ...
                    Map<SkillStatEffect, Long> bsvhs = new HashMap<>();
                    c.getPlayer().getEffects().entrySet().stream().forEach((Map.Entry<BuffStats, List<BuffStatsValueHolder>> effect1) -> {
                ...
                case 152121005: {
                    ...
                    for (int i = 0; i < 5; ++i) {
                        SkillStatEffect a = Objects.requireNonNull(SkillFactory.getSkill(152121006)).getEffect(c.getPlayer().getSkillLevel(152121005));
                ...
                case 400051025: {
                    ...
                    SkillStatEffect a = Objects.requireNonNull(SkillFactory.getSkill(400051025)).getEffect(chr.getSkillLevel(400051024));
                    chr.getMap().spawnMist(new MapleMist(a.calculateBoundingBox(pos, chr.isFacingLeft()), chr, a), 1500, false, false, false, false, false);
                    SkillStatEffect b = Objects.requireNonNull(SkillFactory.getSkill(400051026)).getEffect(chr.getSkillLevel(400051024));
                ...
                default:
                    ...
                    if (effect.isMagicDoor()) {
                        ...
                        if (SkillStatEffect.parseMountInfo(c.getPlayer(), skill.getId()) != 0 && c.getPlayer().getBuffedValue(BuffStats.MonsterRiding) == null && c.getPlayer().getDragon() != null) {
...
//                                System.out.println(pet);
                        ...
                        if (skill.getId() == 400011012) {
                            Objects.requireNonNull(SkillFactory.getSkill(400011013)).getEffect(chr.getSkillLevel(400011012)).applyTo(c.getPlayer(),
                                    pos);
                            Objects.requireNonNull(SkillFactory.getSkill(400011014)).getEffect(chr.getSkillLevel(400011012)).applyTo(c.getPlayer(),
                                    pos);
                        }
                    }
                    break;
            ...
            for (InnerSkillValueHolder isvh : chr.getInnerSkills()) {
                if (isvh.getSkillId() == 70000045) {
                    hasNoCoolTime = Randomizer.isSuccess(Objects.requireNonNull(SkillFactory.getSkill(70000045)).getEffect(chr.getSkillLevel(70000045)).getSkillStats().getStats("nocoolProp"));
                    if (hasNoCoolTime) {
                        break;
                    }
                }
    ...
    public static void closeRangeAttack(ReadingMaple rh, final MapleClient c, final MapleCharacter chr,
        ...
        if(attack.skill == 155121306) {
            Map<SkillStatEffect, Long> bsvhs = new HashMap<>();
            chr.getEffects().entrySet().stream().forEach((Map.Entry<BuffStats, List<BuffStatsValueHolder>> effect1) -> {
                effect1.getValue().stream().filter((bsvh) -> (!bsvhs.containsKey(bsvh.effect) && bsvh.effect.getSourceId() == 155101006)).forEach((bsvh) -> {
                    bsvhs.put(bsvh.effect, bsvh.startTime);
                });
            });
        ...
        if (attack.skill > 0) {
            ...
            if(chr.gettimes() >= System.currentTimeMillis()) {
                if(chr.getsaveSkill() == 15001020 && attack.skill == 15001021) {
                    ...
                    Objects.requireNonNull(SkillFactory.getSkill(400051044)).getEffect(chr.getSkillLevel(400051044)).applyTo(chr);
            ...
            if (chr.getSkillLevel(5100015) > 0) {
                ...
                if (chr.에너지차지구분 == 0 && attack.skill != 400051015) {
                    if (현재에너지 >= 10000) {
                        ...
                        chr.send(MainPacketCreator.giveEnergyCharge(현재에너지, 10000, 5120018, 10000, chr.getBuffedValue(BuffStats.Stimulate) != null, false));
                    }
                } else if (chr.에너지차지구분 == 1 || attack.skill == 400051015) {
                    if (현재에너지 <= 0) {
                        chr.send(MainPacketCreator.giveEnergyCharge(0, 0, 5120018, 현재에너지, chr.getBuffedValue(BuffStats.Stimulate) != null, false));
                        chr.acaneAim = 0;
                        chr.에너지차지구분 = 0;
                    } else {
                        if (attack.skill != 400051015) {
                            chr.send(MainPacketCreator.giveEnergyCharge(0, 0, 5120018, 현재에너지, chr.getBuffedValue(BuffStats.Stimulate) != null, true));
        ...
        if (attack.skill == 21120018 || attack.skill == 21120019) {
            if (chr.royalGuard == 1) {
                SkillStatEffect effect = Objects.requireNonNull(SkillFactory.getSkill(chr.isActiveBuffedValue(21110016) ? 21110016 : 21121058)).getEffect(chr.getSkillLevel(chr.isActiveBuffedValue(21110016) ? 21110016 : 21121058));
        ...
        if (attack.skill != 0) {
            ...
            if (GameConstants.isDemonAvenger(c.getPlayer().getJob())) {
                ...
                if (skill.getName().contains("익시드 :")) {
                    ...
                    Objects.requireNonNull(SkillFactory.getSkill(30010230)).getEffect(1).applyTo(chr);
                }
            }
            ...
            if ((skill.getId() == 31011000 || skill.getId() == 31201000 || skill.getId() == 31211000 || skill.getId() == 31221000)) {
                ...
                SkillStatEffect realEffect = Objects.requireNonNull(SkillFactory.getSkill(61101002)).getEffect(c.getPlayer().getSkillLevel(61101002));
                ...
                chr.addCooldown(64111004, System.currentTimeMillis(),
                        Objects.requireNonNull(SkillFactory.getSkill(64111004)).getEffect(skillLevel).getCooldown() * 1000);
            } else if (attack.skill == 64121024) {
                chr.addCooldown(64121021, System.currentTimeMillis(),
                        Objects.requireNonNull(SkillFactory.getSkill(64121021)).getEffect(skillLevel).getCooldown() * 1000);
            }
            assert effect != null;
            attackCount = effect.getAttackCount();
            if (Objects.requireNonNull(SkillFactory.getSkill(attack.skill)).getEffect(skillLevel).getCooldown() > 0) {
                switch (attack.skill) {
                    case 1321013:
                    ...
                    case 11121055: {
                        chr.addCooldown(11121052, System.currentTimeMillis(),
                                Objects.requireNonNull(SkillFactory.getSkill(11121052)).getEffect(skillLevel).getCooldown() * 1000);
                        break;
                    }
                    case 33111013:
                    case 142121005: {
                        if (chr.getCooldownLimit(attack.skill) == 0) {
                            chr.addCooldown(attack.skill, System.currentTimeMillis(),
                                    Objects.requireNonNull(SkillFactory.getSkill(attack.skill)).getEffect(skillLevel).getCooldown() * 1000);
                        }
                        break;
                    }
                    case 400011032:
                    ...
                    case 400011037: {
                        chr.addCooldown(400011032, System.currentTimeMillis(),
                                Objects.requireNonNull(SkillFactory.getSkill(400011032)).getEffect(skillLevel).getCooldown() * 1000);
                        break;
                    }
                    ...
                    case 3321021: {
                        chr.addCooldown(3321014, System.currentTimeMillis(),
                                Objects.requireNonNull(SkillFactory.getSkill(3321014)).getEffect(skillLevel).getCooldown() * 1000);
                        break;
                    }
                    ...
                    case 3321040: {
                        chr.addCooldown(3321035, System.currentTimeMillis(),
                                Objects.requireNonNull(SkillFactory.getSkill(3321035)).getEffect(skillLevel).getCooldown() * 1000);
                        break;
                    }
                    ...
                    case 155120000: {
                        chr.addCooldown(155001102, System.currentTimeMillis(),
                                Objects.requireNonNull(SkillFactory.getSkill(155001102)).getEffect(skillLevel).getCooldown() * 1000);
                        break;
                    }
                    ...
                    case 155101214: {
                        chr.addCooldown(155101104, System.currentTimeMillis(),
                                Objects.requireNonNull(SkillFactory.getSkill(155101104)).getEffect(skillLevel).getCooldown() * 1000);
                        break;
                    }
                    case 155101100:  //Scarlet Charge Drive
                    case 155101101: {
                        chr.addCooldown(155101100, System.currentTimeMillis(),
                                Objects.requireNonNull(SkillFactory.getSkill(155101100)).getEffect(skillLevel).getCooldown() * 1000);
                        break;
                    }
                    case 400010030: {
                        chr.addCooldown(400011031, System.currentTimeMillis(),
                                Objects.requireNonNull(SkillFactory.getSkill(400011031)).getEffect(skillLevel).getCooldown() * 1000);
                        break;
                    }
                    default:
            ...
            if (attack.skill == 21121057) {
                Objects.requireNonNull(SkillFactory.getSkill(21121068)).getEffect(1).applyTo(chr, chr.getPosition());
            }
        }
        ...
        if (attack.skill == 27101101) {
            for (AttackPair atk : attack.allDamage) {
                Objects.requireNonNull(chr.getMap().getMonsterByOid(atk.objectid)).applyStatus(chr, new MonsterStatusEffect(Collections.singletonMap(MonsterStatus.STUN, 1), SkillFactory.getSkill(27101101), attack.skillLevel, null, false), (long) 3 * 1000);
            }
        }
        chr.checkFollow();
    ...
    public static void MagicDamage(ReadingMaple rh, MapleClient c, MapleCharacter chr, RecvPacketOpcode recv) {
        ...
        if (effect.getCooldown() > 0) {
            switch (attack.skill) {
                ...
                case 142110000: {
                    chr.addCooldown(attack.skill, System.currentTimeMillis(),
                            Objects.requireNonNull(SkillFactory.getSkill(142110001)).getEffect(skillLevel).getCooldown() * 1000);
                    break;
                }
                default: {
    ...
    public static void MoveAndroid(final ReadingMaple rh, final MapleClient ha, final MapleCharacter hp) {
        rh.skip(12);
    ...
    public static void RoomChange(ReadingMaple rh, MapleClient c, MapleCharacter player) {
        ...
        if (c.getChannel() != channel) {
            ...
            MapleCharacter.crossChannelWarp(c, targetMap, channel);
        } else {
            player.changeMap(to, Objects.requireNonNull(to.getPortal("sp")));
        }
    }
    ...
    public static void znfxkdladmfwnwkdyd(ReadingMaple rh, MapleClient c) {
        ...
        chr.addCooldown(id, System.currentTimeMillis(),
                Objects.requireNonNull(SkillFactory.getSkill(id)).getEffect(chr.getSkillLevel(id)).getCooldown() * 1000);
    }
    ...
    public static void OrbitalFlame(final ReadingMaple rh, final MapleClient c) {
        ...
        SkillStatEffect flame = Objects.requireNonNull(SkillFactory.getSkill(tempskill)).getEffect(chr.getSkillLevel(tempskill));
        if (flame != null && chr.getSkillLevel(elementid) > 0) {
            if (!chr.isActiveBuffedValue(elementid)) {
                SkillStatEffect element = Objects.requireNonNull(SkillFactory.getSkill(elementid)).getEffect(chr.getSkillLevel(elementid));
    ...
    public static void ChangeInner(ReadingMaple rh, MapleClient c) {
    ...
    public static void OnThrowGrenade(ReadingMaple rh, MapleClient c) {
        ...
        if (nSkillID == 12121001) {
            ...
            SkillStatEffect effect = Objects.requireNonNull(SkillFactory.getSkill(nSkillID)).getEffect(chr.getSkillLevel(nSkillID));
            ...
            SkillStatEffect effect = Objects.requireNonNull(SkillFactory.getSkill(400031003)).getEffect(chr.getSkillLevel(400031003));
    ...
    public static void mistSkill(ReadingMaple rh, MapleCharacter chr) {
        ...
        SkillStatEffect effect = Objects.requireNonNull(SkillFactory.getSkill(skillId)).getEffect(chr.getSkillLevel(skillId));
    ...
    public static void CreateKinesisPsychicArea(ReadingMaple rm, MapleClient c) {
        ...
        SkillStatEffect eff = Objects.requireNonNull(SkillFactory.getSkill(SkillID)).getEffect(SLV);
        c.getPlayer().getMap()
    ...
    public static void ReleasePsychicLock(ReadingMaple rm, MapleClient c) {
        int skillid = rm.readInt();
    ...
    public static void DoActivePsychicArea(ReadingMaple rm, MapleClient c) {
    ...
    public static void DebuffPsychicArea(ReadingMaple rm, MapleClient c) {
        int skillid = rm.readInt();
        short slv = rm.readShort();
        int v1 = rm.readInt();
        byte v2 = rm.readByte();
    ...
    public static void UserDamageSkinSaveRequest(final ReadingMaple rm, final MapleClient c) {
        byte v1 = rm.readByte();
        byte ds = rm.readByte();
    ...
    public static void HyperSkillRemove(final ReadingMaple rh, final MapleClient c) {
        ...
        final HashMap<ISkill, SkillEntry> hashMap = new HashMap<>();
        for (ISkill iSkill : c.getPlayer().getSkills().keySet()) {
            final ISkill skill;
            if ((skill = iSkill).getHyper() > 0) {
    ...
    public static void CashRemover(ReadingMaple rm, MapleClient c) {
        ...
        IItem item = c.getPlayer().getInventory(Objects.requireNonNull(MapleInventoryType.getByType((byte) invType))).getItem((short) slot);
    ...
    public static void DarkSpear(ReadingMaple rm, MapleClient c) {
        ...
        if (nSkillID == 400051003) {
            ...
            SkillStatEffect effect = Objects.requireNonNull(SkillFactory.getSkill(400051002)).getEffect(chr.getSkillLevel(400051002));
            ...
            c.send(MainPacketCreator.giveBuff(400051002, duration, statups, effect, stacked,
                    Objects.requireNonNull(SkillFactory.getSkill(400051002)).getAnimationTime(), chr));
            ...
            SkillStatEffect effect = Objects.requireNonNull(SkillFactory.getSkill(400051008)).getEffect(chr.getSkillLevel(400051008));
        ...
        if (nSkillID == 152001002 || nSkillID == 152120003) {
            ...
            Objects.requireNonNull(SkillFactory.getSkill(passiveid)).getEffect(chr.getSkillLevel(passiveid)).applyTo(chr);
            SkillStatEffect craft = Objects.requireNonNull(SkillFactory.getSkill(nSkillID)).getEffect(chr.getTotalSkillLevel(nSkillID));
            craft.applyTo(chr);
        }
        ...
        int cool = Objects.requireNonNull(SkillFactory.getSkill(mainskillid)).getEffect(chr.getSkillLevel(mainskillid)).getCooldown();
        if (cool > 0) {
    ...
    public static void IncSkillTime(ReadingMaple slea, MapleClient c) {
        ...
        if (skillId == 400051006) {
            SkillStatEffect effect = Objects.requireNonNull(SkillFactory.getSkill(400051006)).getEffect(c.getPlayer().getSkillLevel(400051006));
    ...
    public static void SymbolExp(ReadingMaple slea, MapleClient c) { //Arcane Symbol
        try {
            ...
            if (type == 0) {
                ...
                ArrayList<Short> removes = new ArrayList<>();
    ...
    public static void Joker(ReadingMaple slea, MapleClient c) {
        ...
        if (player.isActiveBuffedValue(400041009)) {
            Objects.requireNonNull(SkillFactory.getSkill(400041010)).getEffect(player.getSkillLevel(400041009)).applyAtom(player, 33);
        }
    }
    ...
    public static void XenonMegaSmahser(ReadingMaple slea, MapleClient c) {
        ...
        if (start) {
            SkillStatEffect effect = Objects.requireNonNull(SkillFactory.getSkill(400041007)).getEffect(chr.getSkillLevel(400041007));
            ...
            chr.send(MainPacketCreator.giveBuff(400041007, localDuration, statups, effect, stacked,
                    Objects.requireNonNull(SkillFactory.getSkill(400041007)).getAnimationTime(), chr));
            final long starttime = System.currentTimeMillis();
    ...
    public static void AndroidEar(MapleClient c, ReadingMaple rm) {
        ...
        client.ItemInventory.Items.InventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, rm.readShort(), (short) 1, true);
        c.getPlayer().ea();
    }
                        ...
                        if (mist.getSourceSkill() != null) {
                            if (mist.getSourceSkill().getId() == 12121005) {
                                ...
                                for (final MapleMapObject mo : c.getPlayer().getMap().getMapObjectsInRect(mist.getBox(), Collections.singletonList(MapleMapObjectType.PLAYER))) {
                                    ...
                                    SkillStatEffect effect = Objects.requireNonNull(SkillFactory.getSkill(12121005)).getEffect(c.getPlayer().getSkillLevel(12121005));
    ...
    public static void IceAura(final MapleClient c, final Point pos, final int skillid) {
        SkillStatEffect effect = Objects.requireNonNull(SkillFactory.getSkill(skillid)).getEffect(c.getPlayer().getSkillLevel(skillid));
        BuffTimer tMan = BuffTimer.getInstance();
        int i = 0;
        if (diabolicRecoveryTask != null) {
            diabolicRecoveryTask.cancel(true);
    ...
    public static void CrystalSkill(final ReadingMaple slea, final MapleClient c) {
        ...
        if (skillid == 152001002 || skillid == 152120003 && !c.getPlayer().skillisCooling(skillid)) {
            ...
            int passiveid = 152000007;
            Objects.requireNonNull(SkillFactory.getSkill(passiveid)).getEffect(c.getPlayer().getSkillLevel(passiveid)).applyTo(c.getPlayer());
            SkillStatEffect craft = Objects.requireNonNull(SkillFactory.getSkill(skillid)).getEffect(c.getPlayer().getTotalSkillLevel(skillid));
    ...
    public static void BlackAlter(ReadingMaple rm, MapleClient c) {
        ...
        int oid = rm.readInt();
        MapleSummon hs = null;
        if (c.getPlayer().getMap().getMapObject(oid, MapleMapObjectType.SUMMON) != null) {
    ...
    public static void GiveGrandCross(ReadingMaple slea, MapleClient c) {
        ...
        if (skillid == 400011072) {
            if (c.getPlayer().getKeyDownSkill_Time() + 3000 <= System.currentTimeMillis()) {
                ...
                SkillStatEffect effect = Objects.requireNonNull(SkillFactory.getSkill(400011072)).getEffect(c.getPlayer().getSkillLevel(400011072));
                List<Triple<BuffStats, Integer, Boolean>> statups = new ArrayList<>();
    ...
    public static void Obsidiunbarrier(final ReadingMaple slea, MapleClient c) {
        ...
        SkillStatEffect effect = Objects.requireNonNull(SkillFactory.getSkill(skill)).getEffect(skill);
        effect.applyTo(c.getPlayer());
        c.getPlayer().ea();
    }
