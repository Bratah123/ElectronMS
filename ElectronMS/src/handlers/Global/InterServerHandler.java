...
public class InterServerHandler {
    ...
    public static void EnterAuction(MapleClient c) {
    ...
    public static void EnterCS(final MapleClient c, final MapleCharacter chr, final boolean ScriptEnter) {
        if (!chr.isAlive()) {
            c.getSession().writeAndFlush(MainPacketCreator.resetActions(c.getPlayer()));
            return;
        }
...
/*        if (ServerConstants.chr != null) { //Electronic board
        ...
        if (!c.getPlayer().getStat().getJC()) {
            String[] text = {"welcome to, " + ServerConstants.serverName, " welcome to the server!"};
        ...
        if (GameConstants.isKOC(player.getJob()) && player.getLevel() >= 100) {
            if (player.getSkillLevel(Integer.parseInt(player.getJob() + "1000")) <= 0) {
                player.teachSkill(Integer.parseInt(player.getJob() + "1000"), (byte) 0, Objects.requireNonNull(SkillFactory.getSkill(Integer.parseInt(player.getJob() + "1000"))).getMaxLevel());
            }
        }
        ...
        player.send(MainPacketCreator.getQuickMove(new ArrayList<>()));
...
//        }
    ...
    public static void ChangeChannel(final ReadingMaple rh, final MapleClient c, final MapleCharacter chr) {
        if (!chr.isAlive()) {
            c.getSession().writeAndFlush(MainPacketCreator.resetActions(c.getPlayer()));
            return;
        }
