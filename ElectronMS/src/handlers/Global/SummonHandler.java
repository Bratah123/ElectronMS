...
public class SummonHandler {
    ...
    public static void MoveDragon(final ReadingMaple rh, final MapleCharacter chr) {
    ...
    public static void MoveSummon(final ReadingMaple rh, final MapleCharacter chr) {
    ...
    public static void DamageSummon(final ReadingMaple rh, final MapleCharacter chr) {
    ...
    public static void SummonAttack(final ReadingMaple rh, final MapleClient c, final MapleCharacter chr) {
        if (!chr.isAlive()) {
            return;
        }
...
//        }
        if(skillid == 400021071) {
            chr.setbb(0);
            Objects.requireNonNull(SkillFactory.getSkill(400021071)).getEffect(chr.getSkillLevel(400021071)).applyTo(chr);
        }
        ...
        if ((v2 == 152110002 || v2 == 152100002 ||  v2 == 152100001 || v2 == 152110001) && !chr.skillisCooling(v2)) {
            c.getPlayer().addCooldown(v2, System.currentTimeMillis(),
                    Objects.requireNonNull(SkillFactory.getSkill(v2)).getEffect(c.getPlayer().getSkillLevel(v2)).getCooldown() * 1000);
        }

        if (hits == 2 && skillid == 1301013) { 
            c.getPlayer().addCooldown(1311014, System.currentTimeMillis(),
                    Objects.requireNonNull(SkillFactory.getSkill(1311014)).getEffect(c.getPlayer().getSkillLevel(1311014)).getCooldown() * 1000);
        }
        if (v2 == 152101006) {
    ...
    public static void SummonSpecialAttack(final ReadingMaple rh, final MapleClient c, final MapleCharacter chr) {
        if (!chr.isAlive()) {
            return;
        }
        final MapleMap map = chr.getMap();
        ...
        switch (summonSkill.getId()) {
            case 152100001:
            case 152110001:
                c.getPlayer().addCooldown(summonSkill.getId(), System.currentTimeMillis(), Objects.requireNonNull(SkillFactory.getSkill(summonSkill.getId())).getEffect(c.getPlayer().getSkillLevel(summonSkill.getId())).getCooldown() * 1000);
                summon.removeSummon(map);
                Objects.requireNonNull(SkillFactory.getSkill(152101000)).getEffect(c.getPlayer().getSkillLevel(152101000)).applyTo(c.getPlayer(), summon.getPosition());
                break;
        ...
        switch (subId) {
            ...
            case 400021066:
                if (!chr.skillisCooling(subId))
                    c.getPlayer().addCooldown(subId, System.currentTimeMillis(), Objects.requireNonNull(SkillFactory.getSkill(subId)).getEffect(c.getPlayer().getSkillLevel(subId)).getCooldown() * 1000);
                break;
        }
         
    }

    public static void removeSummon(final ReadingMaple rh, final MapleClient ha) {
        int oid = rh.readInt();
        MapleSummon hs = null;
        if (ha.getPlayer().getMap().getMapObject(oid, MapleMapObjectType.SUMMON) != null) {
    ...
    public static void summonSkill(final ReadingMaple rh, final MapleClient ha, final MapleCharacter chr) {
        int oid = rh.readInt();
        rh.skip(4);
        int skillid = rh.readInt();
        MapleSummon hs = null;
        ...
        switch (skillid) {
            ...
            case 35111011: {
                ...
                ha.getPlayer().addHP((int) (ha.getPlayer().getStat().getCurrentMaxHp()
                        * Objects.requireNonNull(SkillFactory.getSkill(hs.getSkill())).getEffect(hs.getSkillLevel()).getHp() / 100.0));
            ...
            case 1310016: {
                ...
                if (!chr.isActiveBuffedValue(skillid)) {
                    ...
                    Objects.requireNonNull(SkillFactory.getSkill(skillid)).getEffect(chr.getSkillLevel(skillid)).applyTo(chr);
                }
                break;
            }
            case 400041038: {
                    final List<MapleMapObject> objs = chr.getMap().getMapObjectsInRange(chr.getTruePosition(), 800000, Collections.singletonList(MapleMapObjectType.MONSTER));
                ...
                if (GameConstants.isAngelicBlessBuff(skillid)) {
                    ...
                    switch (skillid % 1000) {
                        case 86: // Angelic bless
                            Objects.requireNonNull(ii.getItemEffect(2022746)).applyTo(ha.getPlayer());
                            effectid = 1085;
                            break;
                        case 88: // Dark Angel Bless
                            Objects.requireNonNull(ii.getItemEffect(2022747)).applyTo(ha.getPlayer());
                            effectid = 1087;
                            break;
                        case 91: // Snowflake Angelic Blessed
                            Objects.requireNonNull(ii.getItemEffect(2022764)).applyTo(ha.getPlayer());
                            effectid = 1090;
                            break;
                        case 180:// White Angel Bless
                            Objects.requireNonNull(ii.getItemEffect(2022823)).applyTo(ha.getPlayer());
                            effectid = 1179;
                            break;
                    }
                    ha.getPlayer().getMap()
