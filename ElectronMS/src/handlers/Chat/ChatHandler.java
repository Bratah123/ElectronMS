    ...
    public static void GeneralChat(String text, byte unk, MapleClient c, MapleCharacter chr) {
    	...
    	if (isCommand) {
            ...
            if(text.toLowerCase().startsWith("&find"))
            {
                ...
                if(name.length <= 1)
                {
                    chr.dropMessage(5,"Please provide a username");
                    return;
                }
                else {
                    MapleCharacter target = chr.getClient().getChannelServer().getPlayerStorage().getCharacterByName(name[1]);
                    if (target == null || target.isGM()) {
                        chr.dropMessage(5, "Player is currently offline.");
                        return;
                    } else {
                        int targetChannel = target.getClient().getChannel();
                        int playerChannel = chr.getClient().getChannel();
                        String map = target.getMap().getMapName();
                        if(playerChannel == targetChannel) {
                            chr.dropMessage(5, "Map: " + map);
                        }
                        else {
                            chr.dropMessage(5, "Channel: " + targetChannel);
                        }
                    }
                }
                return;
