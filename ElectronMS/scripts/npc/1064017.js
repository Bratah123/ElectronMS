...
var bossid = 8930000; // Boss mob code
var startmap = 105200810; // Boss map
var bossname = "Chaos Vellum"; // In Hangul
var limit = 3; // Daily Admissions
var time = 35; // Time limit
//1135 48
var x = -207, y = 443; // Boss coordinates
/*------------------------------*/
function start() {
	status = -1;
	action(1, 0, 0);
...
function action(mode, type, sel) {
	...
	if (status == 0) {
		var msg = "#fs11##d<#i2591418# :: #rChaos Vellum#k>#d"+enter;
        msg += "<Boss reward : #i4001922#>"+enter;
		msg += "<Time limit : #r"+time+" minute#k #dDaily entries#k #d: #b"+cm.GetCount(bossname+"c", limit)+"time#k / #r"+limit+"time#k>#k"+enter;
	    ...
	    if(!allPreqDone) // jank boolean rn but hey it works???
        {
            for(var i = 0; i < cm.getParty().getMembers().size(); i++)
            {
                ...
                if(partyMembers.getKeyValue("RedTierUnlock") == null)
                {
                    cm.sendOk("Make sure all your party members have done the Red Tier Prequest.");
                    return cm.dispose();
                }
                else
                {
		...
		if(!cm.CountCheck(bossname+"c", limit)) {
			cm.sendOk("You can only enter "+limit+" times per day");
			cm.dispose();
			return;
		}
