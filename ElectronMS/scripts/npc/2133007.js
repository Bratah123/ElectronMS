...
var bossid = 9390710; // 보스 몹 코드
var secondboss = 9999991; // (루시드, 스우만 적용. 2페 몹)
var startmap = 500000004; // 보스 맵
...
function action(mode, type, sel) {
	...
	if (status == 0) {
		var msg = "#fs11##d<#fUI/UIWindow2.img/MobGage/Mob/9390710# :: #r봄온라인 황소#k>#d"+enter;
        msg += "<보스 보상 : #i4032101##fs14##r10개#r#fs11#>"+enter;
		msg += "<[제한시간 :#k #r20분#k] #d일일 입장 횟수#k #d: #b"+cm.GetCount(bossname+"c", limit)+"회#k / #r"+limit+"회#k>#k"+enter;
        msg += "#d<데스 카운트 :#k #r5회#k>#k"+enter;
		msg += "#L1##r보스를 처치하러 이동하겠습니다.#k";
		cm.sendSimple(msg);
	} else if (status == 1) {
		/*if (cm.getPlayer().getParty() != cm.getPlayer().getKeyValue("TreasureUnlock")) {
        cm.sendOk("One of your party members didn't finish the treasure prequest.");
        cm.dispose();
        return;
        }*/
		if(!allPreqDone)
