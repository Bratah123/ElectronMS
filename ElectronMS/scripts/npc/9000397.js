...
var item = [4310061,4310014,4310218]; // Quest material code, if you want to increase the item type [4000000, 40000001]
var cost = [50,50,50]; // Number of quest materials, same as above
var gain = 1142973; // Quest reward code
...
function action(mode, type, selection) {
   ...
   // From here
   quest = cm.getQuestRecord(19021501);
   ...
   // Quest Declaration so far, don't touch
	...
	if (qstatus != 2) {
		...
		ms += "#b#L0#Let's become a jump king!.#l\r\n" // Quest Title Name
		if (qstatus != 0) {
			ms += "\r\n\r\n"
			ms += icon_etc + "\r\n"
		...
		cm.sendNext("You have already completed this quest."); // Quest message when completed
		cm.dispose();
	}
    } else if (status == 1) {
	sel = selection;
	if (sel == 0) {
		if (qstatus == 0) {
			cm.sendNext("Thank you for jumping instead of me! I will give you a title when you come!"); // Message on Accepting Quest First
		} else if (qstatus == 1 && !itemcheck) {
			var ms = ""
	...
	if (qstatus == 0) {
		cm.sendYesNo("You can collect the jump coins and bring them to me?"); // Message to ask if you want to accept the quest
	} else if (qstatus == 1 && itemcheck) {
